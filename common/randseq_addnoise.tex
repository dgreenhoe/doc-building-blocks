...
The problem here is that $\rvy$ and $\rvw$ are very much 
\prope{correlated}---in fact $\rvy$ is a \emph{function} of $\rvw$ in that $\rvy=\rvx+\rvw$.
...
\corbox{
  ...
  \implies
  \brb{\begin{array}{FrclD}
      (1).& \Szsy(z)      &=& \ZG(z)     \Szxy(z)      & and
    \\(2).& \Swsy(\omega) &=& \FG(\omega)\Swxy(\omega) &
  \end{array}}
  }
\\
\begin{proof}
