...
\corbox{
  ...
  \end{array}}
  \implies
  \brb{\begin{array}{F>{\ds}rc>{\ds}lMD}
       (1).&\Swxx^\ast(\omega) &=& \ds \Swxx(\omega)  & ($\Swxx(\omega)$ is \prope{real-valued}) & and
     \\(2).&\Swxy^\ast(\omega) &=& \ds \Swyx(\omega)  & ($\Swxy(\omega)$ is \prope{conjugate-symmetric}
  \end{array}}
  }
\end{corollary}
\begin{proof}
