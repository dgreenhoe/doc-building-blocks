...
Let $\opTrn$ and $\opDil$ be as in \prefp{def:opT}.\\
Let $\opLT$ be the \ope{Laplace transform} \xref{def:opLT}.
\propbox{
  \begin{array}{F>{\ds}rc>{\ds}l c>{\ds}lCD}
      1.& \opLT\opTrn^n      &=& e^{-s n} \opLT      & &        & \forall n\in\Z &
    \\2.& \opLT\opDil^j      &=& \opDil^{-j}\opLT    & &        & \forall j\in\Z &
    \\3.& \opDil\opLT        &=& \opLT\opDili        & &        & \forall n\in\Z &
    \\4.& \opLT\opDili\opLTi &=& \opLTi\opDili\opLT  &=& \opDil & \forall n\in\Z & ($\opDili$ is \prope{similar} to $\opDil$)
    \\5.& \opDil\opLT\opDil  &=& \opDili\opLT\opDili &=& \opLT  & \forall n\in\Z &
  \end{array}
  }
\end{proposition}
\begin{proof}
\begin{align*}
  \opLT\opTrn^n \ff(x)
    &= \opLT \ff(x-n)
    && \text{by definition of $\opTrn$} && \text{\xref{def:opT}}
  \\&= \int_\R \ff(x-n) e^{-sx} \dx
    && \text{by definition of $\opLT$}
    && \text{\xref{def:opLT}}
  \\&= \int_\R \ff(u) e^{-s (u+n)} \du
    && \text{where $u\eqd x-n$}
...
\end{corollary}
\begin{proof}
These results follow directly from \prefp{prop:wavstrct_BTD} with $\opFT = \left.\opLT\right|_{s=i\omega}$.
\end{proof}
