...
\qboxnpq
  ...
  \footnotetext{
    quote: \citePpc{weyl1935}{14}{H. Weyl, quoting himself from ``a conference on topology and abstract algebra as two ways of mathematical understanding, in 1931"}. 
...
%\citetblt{
%  quote: \citePc{weyl1935}{memorial address for Emmy Noether (1882--1935)},\\
%         \citeIgc{weyl1935p}{0486266931}{in a book of collected works of Hermann Weyl},\\
%         \citeIppgc{weyl1935d}{140}{141}{0486266931}{in a book by Auguste Dick about Emmy Noether}.\\
...
\footnote{
  ...
  \citor{riesz1909},
  \citer{hausdorff1914},
  \citeP{tietze1923}, % cited by Thron page 18
...
\defboxp{\indxs{\topT}\indxs{\sssTx}
  A family of sets $\topT\subseteq\psetX$ is a \structd{topology} on $\sid$ if
  \\\indentx$\begin{array}{F>{\ds}l>{\ds}lD}
      1. & \szero \in \topT
         &
        %& ($\szero$ is in $\topT$)
         & and
  ...
  The ordered pair $\topspaceX$ is a \structe{topological space} if $\topT$ is a \structe{topology} on $\setX$.
  A set $\setU$ is \propd{open} in $\topspaceX$ if $\setU$ is any element of $\topT$.
  A set $\setD$ is \propd{closed} in $\topspaceX$ if $\cmpD$ is \prope{open} in $\topspaceX$.
...
so also are these sets topologies on $\sid$ (next example):
%---------------------------------------
\begin{example}
...
%  \\\hline%
%   \footnotesize\input{../common/math/graphics/lat/lat5_m3_nontop.tex}%
%  &\footnotesize\input{../common/math/graphics/lat/lat5_n5_nontop.tex}%
%  &\footnotesize\input{../common/math/graphics/lat/lat5_l2onm2_top.tex}%
%  &\footnotesize\input{../common/math/graphics/lat/lat5_m2onl2_top.tex}%
%  &\footnotesize\input{../common/math/graphics/lat/lat5_l5_top.tex}%
%\end{tabular}}
%\end{example}
%\begin{proof}
%\begin{enumerate}
...
%  \begin{center}
%  \begin{fsL}
%  \input{../common/math/graphics/latwavxyz.tex}%
% % \setlength{\unitlength}{\tw/(400)}%
% % \begin{picture}(300,325)(-150,-25)%
...
% %     }%
% %   {\color{latdot}%
% %     \put(   0, 320){\makebox(0,0)[b] {$\setn{x,y,z}$}}%
% %     \put(- 65, 200){\makebox(0,0)[cr]{$\setn{x,y}$}}%
% %     \put( 120, 100){\makebox(0,0)[cl]{$\setn{z}$}}%
% %     \put(  20, 100){\makebox(0,0)[cl]{$\setn{y}$}}%
% %     \put(-120, 100){\makebox(0,0)[cr]{$\setn{x}$}}%
% %     \put(   0, -20){\makebox(0,0)[t] {$\szero$}}%
% %     }%
% % \end{picture}%
%  \end{fsL}
%  \end{center}
...
\defboxt{\indxs{\baseB}
  A set $\baseB\subseteq\psetx$ is a \structd{base} for $\topT$ if
  \\\indentx$\begin{array}{FlD}
    1.& \baseB \subseteq \topT
      & and
      \\
    2.& \forall\setU\in\topT,\quad \exists \setn{\setB_\gamma\in\baseB} \st \ds\setU = \setopu_{\gamma} \setB_\gamma
  \end{array}$
%  \\
%  An element in $\baseB$ is called a \hid{basic open set}.
}
\end{definition}
...
%---------------------------------------
Let $\topspaceX$ be a \structe{topological space}.
Let $\baseB$ be a subset of $\psetX$ such that $\baseB\subseteq\psetX$.
\thmbox{
  \brb{\text{$\baseB$ is a \structe{base} for $\topT$}}
  \quad\iff\quad
  %\brb{\begin{array}{FlclDD}
  %  \mc{5}{M}{There exists $\setA\in\baseB$ such that}\\
  %    \qquad1. & x\in\setX         &\implies& x\in\setA           & ($\baseB$ covers $\setX$) & and\\
  %    \qquad2. & x\in\setU\in\topT &\implies& \setA\subseteq\setU & %($\setU$ is an \structe{open set})
  %\end{array}}
  \brb{\begin{array}{M}
...
%\begin{proof}
%    \begin{align*}
%      &\text{\fbox{$\baseB$ is a \structe{base} for $\topT$}}
%      \\&\implies \text{if $\setU$ is an \structe{open set} then $\exists \set{\setB_\gamma\in\baseB}{\gamma\in\Gamma} \st \setU = \setopu_{\gamma\in\Gamma} \setB_\gamma$}
%        && \text{by definition of \structe{base} \xref{def:baseB}}
%      \\&\implies \text{if $x\in\setU$ then $\exists\set{\setB_\gamma\in\baseB}{\gamma\in\Gamma} \st x\in\setU = \setopu_{\gamma\in\Gamma} \setB_\gamma$}
%      \\&\implies \text{there exists $\setB_x\in\baseB \st x\in\setB_x\subseteq\setU$}
%        && \text{because $\setB_x\subseteq\setopu\setB_\gamma$}
%      \\&\implies \text{\fbox{right side hypothesis}}
%      \\&\implies \text{for every \structe{open set} $\setU\quad\exists\baseB\eqd\set{\setB_\gamma}{\gamma\in\Gamma}\st \setU=\setopu_{\gamma\in\Gamma}\setB_\gamma$}
%        && \text{because $\forall x\in\setU,\,x\in\setB_x\subseteq\setU$}
%      \\&\implies \text{\fbox{$\baseB$ is a \structe{base} for $\topT$}}
%        && \text{by definition of \structe{base} \xref{def:baseB}}
%    \end{align*}
%\end{proof}
...
\thmbox{
  ...
  \brbl{\begin{array}{FlclD}
    1.& x \in \setX                &\implies&  \exists \setB_x\in\baseB \st x\in\setB_x  & and \\
    2.& \setB_1,\setB_2\in\baseB   &\implies&  \setB_1\seti\setB_2\in\baseB
  \end{array}}
  }
\end{theorem}
...
\defbox{\begin{array}{M}
  $\topS$ is \propd{coarser} than $\topT$ and $\topT$ is \propd{finer} than $\topS$ if
  \\\indentx$\ds \topS\subseteq\topT$.
  \\
  $\topS$ is \propd{strictly coarser} than $\topT$ and $\topT$ is \propd{strictly finer} than $\topS$ if
  \\\indentx$\ds \topS\subset\topT$.
\end{array}}
\end{definition}
...
\defbox{\begin{array}{Mrc>{\ds}l}\indxs{\clsA}
    The set $\clsA$ is the \opd{closure}       of $\setA\in\psetX$ if & \hxs{\clsA} &\eqd& \Seti\set{\setD\in\psetX}{\text{$\setA\subseteq\setD$ and $\setD$ is \prope{closed}}}.
  \\[1ex]The set $\intA$ is the \structd{interior} of $\setA\in\psetX$ if & \hxs{\intA} &\eqd& \Setu\set{\setU\in\psetX}{\text{$\setU\subseteq\setA$ and $\setU$ is \prope{open}}}.
  \\\mc{4}{M}{A point $x$ is a  \structd{closure  point} of $\setA$ if $x\in \clsA$.}
  \\\mc{4}{M}{A point $x$ is an \structd{interior point} of $\setA$ if $x\in\intA$.}
  \\\mc{4}{M}{A point $x$ is an \structd{accumulation point} of $\setA$ if $x\in\clsp{\setA\setd\setn{x}}$}
  \\\mc{4}{M}{A point $x$ in $\clsA$ is a   \structd{point of adherence} in $\setA$ or is \propd{adherent} to $\setA$ if $x\in\clsA$.}
\end{array}}
\end{definition}
...
%    \\\hline
%      \input{../common/math/graphics/butterfly_setA.tex}
%    & \input{../common/math/graphics/butterfly_clsA.tex}
%    & \input{../common/math/graphics/butterfly_intA.tex}
%    & \input{../common/math/graphics/butterfly_bndA.tex}
%    & \input{../common/math/graphics/butterfly_drvA.tex}
%    \\
%    $\setA$ & $\clsA$ & $\intA$ & $\bndA$ & $\drvA$
%  \end{tabular}
%  \caption{examples of \structe{derived set}s (see \prefp{ex:derivedsets}) \label{fig:derivedsets}}
%\end{figure}
%%---------------------------------------
%\begin{example}
%\footnote{
%  \citerpg{mccarty1967}{90}{0486656330} %{\scshape closure, interior, and boundary}\\
%  }
%\label{ex:derivedsets}
%%---------------------------------------
%\prefpp{fig:derivedsets} illustrates some \structe{derived sets} of a set $\setA$ in a 
%\structe{topological space} $\topspaceX$.
%\end{example}
%
...
\lembox{\begin{array}{F rcl cM CC}
   ...
   %2. & \mc{5}{>{\ds}l}{\drvA\subseteq\clsA}                                                                         &                                                                 &\forall \setA\in\psetX.\\
    2. & \setA&=&\intA                                                           &\iff&     $\setA$ is \prope{open}   &                                                                 &\forall \setA\in\psetX.\\
    3. & \setA&=&\clsA                                                           &\iff&     $\setA$ is \prope{closed} &                                                                 &\forall \setA\in\psetX.\\
   % 5. & \mc{3}{l}{
   %        \brb{\begin{array}{MD}
...
%        && \text{by \thme{de Morgan's law}: \prefp{thm:algprop}}
...
% %\\4. & \bndA                         &=& \clsA \seti \cls{\cmpA}      & \forall \setA\in\psetX
% %\\5. & \intA                         &=& \cmpX[(\overline{\cmpA})]    & \forall \setA\in\psetX
%  \end{array}}
%\end{theorem}
%\begin{proof}
%\begin{enumerate}
...
%======================================
%\subsection{Functions on topological spaces}
%======================================
%%---------------------------------------
%\begin{definition}
%\footnote{
...
\label{def:clCxy}
%---------------------------------------
Let $\topspace{\setX}{\topT_x}$ and $\topspace{\setY}{\topT_y}$ be \structe{topological space}s \xref{def:tspace}.
Let $\ff$ be a function in $\clFxy$.
%\defboxp{
  A function $\ff\in\clFxy$ is \propd{continuous} if 
  \qquad$ %\\\indentx$
    \mcom{\setU\in\topT_y}{\prope{open} in $\topspace{\setY}{\topT_y}$} 
    \quad\implies\quad 
    \mcom{\ff^{-1}(\setU)\in\topT_x}{\prope{open} in $\topspace{\setX}{\topT_x}$}$ .
  \\
  A function is \propd{discontinuous} in $\clF{\topspace{\setX}{\topT_x}}{\topspace{\setX}{\topT_y}}$ 
  if it is not \prope{continuous} in $\clF{\topspace{\setX}{\topT_x}}{\topspace{\setX}{\topT_y}}$. 
  %\\The \hid{set of all continuous functions} $\hxs{\clCxy}$ in the function space $\clFxy$ is
...
\begin{figure}[h]
  ...
  \psset{unit=0.08mm}%
  \begin{tabular}{ccc}
    \input{../common/math/graphics/cont_yx.tex}%
  ...
  \end{tabular}%
  \caption{\prope{continuous}/\prope{discontinuous} functions \xref{ex:contxy}\label{fig:contxy}}
\end{figure}
%---------------------------------------
\begin{example}
\label{ex:contxy}
%---------------------------------------
Some \prope{continuous}/\prope{discontinuous} functions are illustrated in \prefpp{fig:contxy}.
\end{example}
...
\footnote{
  ...
  \citerpgc{searcoid2006}{130}{1846286271}{``Theorem 8.3.1 (Criteria for Continuity)", set in \structe{metric space}s}
  }
...
\thmbox{\begin{array}{M}
  ...
  \\\indentx$\begin{array}{FrclCc}
   ...
   %5. & \seqn{x_n}\to x   &\implies& \ff(\seqn{x_n})\to\ff(x)
  \end{array}$
\end{array}}
\end{theorem}
%\begin{proof}
...
%    \end{align*}
%
%  %\item Proof that (5) $\implies$ (2): Let $\setD$ be a 
%  %  \begin{align*}
%  %    
%  %  \end{align*}
%\end{enumerate}
...
\label{rem:conttop}
%---------------------------------------
A word of warning about defining \prope{continuity} in terms of topological spaces---\prope{continuity} 
is defined in terms of a pair of \structe{topological space}s, and whether function is \prope{continuous}
or \prope{discontinuous} in general depends very heavily on the selection of these spaces. 
This is illustrated in \pref{prop:conttop} (next).
The ramification of this is that when declaring a function to be \prope{continuous} or \prope{discontinuous}, 
one must make clear the assumed \structe{topological space}s.
\end{remark}

%---------------------------------------
\begin{proposition}
\footnote{
  \citerpgc{crossley2006}{18}{1846281946}{Proposition 3.9},
  \citerpgc{ponnusamy2002}{98}{0849317177}{2.64. Theorem.}
  }
\label{prop:conttop}
%---------------------------------------
Let $\topspaceXT$ and $\topspaceYS$ be \structe{topological space}s.
Let $\ff$ be a \structe{function} in $\clF{\topspaceXT}{\topspaceYS}$.
\propbox{\begin{array}{FMcMC}
  1. & $\topT$ is the \structe{discrete topology}   &\implies& $\ff$ is \prope{continuous}  & \forall \ff\in\clF{\topspaceXT}{\topspaceYS}\\
  2. & $\topS$ is the \structe{indiscrete topology} &\implies& $\ff$ is \prope{continuous}  & \forall \ff\in\clF{\topspaceXT}{\topspaceYS}
\end{array}}
\end{proposition}
...
\defbox{\begin{array}{M}\indxs{\setN_x}
  A set $\setN_x\in\psetX$ is a \structd{neighborhood} of an element $x\in\setX$ if 
  \\\indentx
    $\ds x \in \intt{\setN_x}$.
  \\
  A set $\setN_x$ is an \structd{open neighborhood} of an element $x\in\setX$ if 
  \\\indentx
...
\propbox{
  \brb{\begin{array}{M}
    A set $\hxs{\setN_x}$ is a \structd{neighborhood}\\ 
...
Let $\topspaceX$ be a \structe{topological space} \xref{def:topspace}.
\defbox{\begin{array}{M}
  A set $\setY\subseteq\setX$ is \propd{disconnected} if there exists $\quad\setA,\setB\subseteq\setX$ such that
  \\\qquad
  ...
  \end{array}$
  \\
  In this case, $\setY$ is said to be \propd{disconnected} by the sets $\setA$ and $\setB$,
  \\
  and the pair $\setA,\setB$ is a \structd{separation} of $\setY$.
  \\
  If a set is not disconnected, then it is \propd{connected}.
\end{array}}
\end{definition}
...
\defbox{\begin{tabular}{l}
  A \structe{topological space} is a \structd{Hausdorff space} if 
  \\\indentx
...
\defboxp{ %\begin{array}{M}
  A sequence $\seq{\setA_i}{i\in\setI}$ is a \structd{cover} of a set $\setA$ 
  in the topological space $\topspaceX$ if
    \[ \setA\subseteq \setopu_{i\in\setI} \setA_i.\]
  A sequence $\seq{\setB_i}{i\in\setJ}$ is a \structd{subcover} of 
  set $\setA$ with respect to a cover $\seq{\setA_i}{i\in\setI}$ if
    \[ \setn{\setB_i}_{i\in\setJ} \subset \setn{\setA_i}_{i\in\setI}. \]
  A sequence $\seq{\setM_i}{i\in\setK}$ is a \structd{minimal cover} of $\setA$ if
    $\seq{\setM_i}{i\in\setK}$ is a cover and
    $\seq{\setM_i}{i\in\setK\setd\setn{n}}$ is not a cover.
  \\
  A cover $\seq{\setA_i}{i\in\setI}$ is a \structd{proper cover} of $\setA$ if
    $\setA$ is not a member.
  \\
  A cover $\seq{\setA_i}{i\in\setI}$ is a \structd{open cover} of $\setA$ if
...
%In a \structe{metric space} \xref{def:metric}, a convergent sequence converges to an \prope{unique} limit \xref{prop:xn_to_xy}.
...
\footnote{
  ...
  \citerpc{leathem1905}{13}{``$\to$" symbol, section III.11}  % referenced by bromwich1955 page 3
  }
\label{def:converge}
\label{def:limit}
\label{def:limitpnt}
...
\defboxp{
  A sequence $\seqxZ{x_n}$ \propd{converges} in $\topspaceX$ to a point $x$ if 
  for each \structe{open set} \xref{def:openset}
  $\setU\in\topT$ that contains $x$ there exists $\xN\in\Zp$ such that 
  \\\indentx
  ...
  \end{array}$
  %\\& 5. & \metric{x_n}{x}\to 0
  \\A sequence that converges is \propd{convergent}.
    A sequence that does not converge is said to \propd{diverge}, or is \propd{divergent}.
...
\exboxp{
  ...
  \\In this space, the sequence $\seqn{x,x,x,\ldots}$ converges to $x$.
    But this sequence also converges to both $y$ and $z$ because $x$ is in every \structe{open set} \xref{def:ts_open} 
    that contains $y$
    and $x$ is in every \structe{open set} that contains $z$.
    So, the \structe{limit} \xref{def:limit} of the sequence is \prope{not unique}.
  }
\end{example}
...
\label{ex:seq_xt56}
%---------------------------------------
In contrast to the low resolution topological space of \pref{ex:seq_xt31}, 
the limit of the sequence $\seqn{x,x,x,\ldots}$ 
