...
Let $\fG(s)\eqd \brs{\opLT\fg(x)}$
Let the \structe{Region of Convergence} of $\fG(s)$ be $A\leq\Real(s)\leq B$.
\corbox{\setlength{\arraycolsep}{2pt}\begin{array}{>{\ds}l@{\hspace{2pt}}c@{\hspace{2pt}}>{\ds}r                                          Dl@{\hspace{2pt}}l                                                              C                    } %D
...
\exboxt{
  ...
  \\ initial voltage $\fv(t)=0$ for $t<0$, and finite voltage over all time, is
...
\begin{enumerate}
  ...
  \item lemma: Because $\fv(t)=0$ for $t<0$, then $\ds\lim_{t\to\infty}\brs{e^{st}\fv(-t)}=0$. \label{ilem:opLT_capZ1}
  \item lemma: Because voltage $\fv(t)$ is finite, $\ds\lim_{t\to\infty}\brs{e^{-st}\fv(t)}=0$. \label{ilem:opLT_capZ2}
  \item Then the impedance $\fZ(s)$ of the capacitor is \ldots
    \begin{align*}
      \fZ(s)
        &  \eqd \frac{\fV(s)}
                     {\fI(s)}
...
%--------------------------------------
\exboxt{
  The \ope{Laplace Transform} of the \prope{impedance} $Z(s)$ of an ideal inductor with \prope{inductance} $L$,
  \\ initial current $\fnct{i}(t)=0$ for $t<0$, and finite current over all time, is
  \\\indentx$\ds Z(s) = sL$.
  }
\end{example}
\begin{proof}
\begin{enumerate}
  \item An ideal inductor is modeled with the equation $\ds\fv(t) = L\ddt\fnct{i}(t)$.
  \item lemma: Because $\fnct{i}(t)=0$ for $t<0$, then $\ds\lim_{t\to\infty}\brs{e^{st}\fnct{i}(-t)}=0$. \label{ilem:opLT_indZ1}
  \item lemma: Because $\fnct{i}(t)$ is finite, $\ds\lim_{t\to\infty}\brs{e^{-st}\fnct{i}(t)}=0$. \label{ilem:opLT_indZ2}
  \item Then the impedance $\fZ(s)$ of the inductor is \ldots
    \begin{align*}
      \fZ(s)
        &  \eqd \frac{\fV(s)}
                     {\fI(s)}
        && \text{by definition of \prope{impedance} $\fZ(s)$}
      \\&  \eqd \frac{\ds\opLT \brs{\fv(t)} }
                     {\fI(s)}
        && \text{by definition of \ope{Laplace Transform} $\fI(s)$ of $\fnct{i}(t)$}
      \\&  \eqd \frac{\ds\opLT \brs{ L \ddt\fnct{i}(t)} }
                     {\fI(s)}
        && \text{by definition of \ope{ideal inductor} with \prope{inductance} $L$}
      \\&  =    \frac{\ds L\opLT \brs{ \ddt\fnct{i}(t)} }
                     {\fI(s)}
        && \text{by \prope{homogeneous} property of $\opLT$ \prefpp{thm:opLT_linear}}
      \\&  =    \frac{\ds Ls\brp{\opLT \brs{\fnct{i}(t)} + \lim_{t\to\infty}\brs{e^{-st}\fnct{i}(t) + e^{st}\fnct{i}(-t)} }}
                     {\fI(s)}
        && \text{by \prefp{thm:opLT_diff}}
      \\&  =    \frac{\ds Ls\opLT \brs{\fnct{i}(t)}}
                     {\fI(s)}
        && \text{by \pref{ilem:opLT_indZ1} and \pref{ilem:opLT_indZ2}}
      \\&  \eqd \frac{\ds sL\fI(s)}
                     {\fI(s)}
        && \text{by definition of $\fI(s)$}
      \\&  = sL
    \end{align*}
\end{enumerate}
\end{proof}
...
%--------------------------------------
Let $\fM_n$ be the \fncte{$n$th moment} \xref{def:moment_laplace} of a function $\ff(x)\in\spLLR$.
