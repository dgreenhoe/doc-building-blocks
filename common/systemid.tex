...
\corbox{
  \begin{array}{F>{\ds}rc>{\ds}l}
    (1).&\brb{\text{hypotheses of \prefp{thm:estHls}}}
        &\implies
        &\estHls(\omega) = \frac{\Swyx(\omega) + \Swyu(\omega)}{\Swxx(\omega)-\Swww(\omega)}
    \\
...
Let $\estHkp(\omega)$ be defined as in \prefpp{def:Hkappa}.
\propbox{
  \estHkp(\omega) = \frac{\Swyy(\omega)- s^2 \Swxx(\omega) + \sqrt{\brs{\Swyy(\omega) -  s^2 \Swxx(\omega)}^2 + 4 s^2 \abs{\Swxy(\omega)}^2}}
                            {2\Swxy(\omega)}
  }
\end{proposition}
