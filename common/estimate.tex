...
%20190726-see {def:ML}%   \opd{Bayesian}:                        &             & \estB   &\eqd& \arg\min_{\theta} C(\theta,\pdfp(s,r)) \\
%20190726-see {def:ML}%   \opd{Mean square}                      &(\opd{MS}):  & \estMS  &\eqd& \arg\min_{\theta} \pE\norm{C(\theta,\pdfp(s,r))}^2 \\
%20190726-see {def:ML}%   \opd{mini-max}                         &(\opd{MM}):  & \estMM  &\eqd& \arg\min_{\theta}\max_{\pdfp} C(\theta,\pdfp(s,r)) \\
%20190726-see {def:ML}%   \opd{maximum a-posteriori probability} &(\opd{MAP}): & \estMAP &\eqd& \arg\max_{\theta} \pP{s(t;\theta)|\fr(t)} \\
%20190726-see {def:ML}%   \opd{maximum likelihood}               &(\opd{ML}):  & \estML  &\eqd& \arg\max_{\theta} \pP{\fr(t)|s(t;\theta)}
