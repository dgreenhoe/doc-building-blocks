...
\prope{wide sense stationary}, and measurement noise being equal to $0$,
...
$\estH1$ is far from ideal in that it is \prope{biased} with respect to $\fH$;
...
Now with two estimators that in general provide two different results,
an obvious extension is to somehow combine them to produce new estimators (next three definitions).
%---------------------------------------
\begin{definition}[\fnctd{Arithmetic mean estimator}]
\footnote{
  \citePc{mitchell1982}{279}{``Frequency Response Calculation: The Average Method"},
