...
\begin{enumerate}
  ...
  \item lemma: \label{ilem:Hs_ineq}
    ...
    \\\iff
      ...
      && \brp{\begin{array}[t]{M}
           because $\ff(x)\eqd x^2$\\
           is \prope{strictly monotonic increasing}
         \end{array}}
    \\\text{true by \pref{ilem:Hs_ineq_sq}, so \pref{ilem:Hs_ineq} also true}
    \end{align*}

  \item lemma: \label{ilem:Hs_mono_ddp}
    \begin{align*}
      \boxed{\ddp\estHs}
      ...
      \\&= \frac{\frac{ \Swxx\brp{p\Swxx-\Swyy} + 2 \abs{\Swxy }^2 -  \Swxx\sqrt{\brp{p\Swxx-\Swyy}^2 + 4p\abs{\Swxy}^2}}
