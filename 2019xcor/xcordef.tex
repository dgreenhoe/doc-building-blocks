...
%---------------------------------------
\propbox{
  \brb{\text{$\Rxx(m)$ definition follows (1) or (2) or (3) or (4)}}
  \quad\implies\quad
  \brb{\begin{array}{rclM}
    \Swxx^\ast(\omega) &=& \Swxx(\omega) & (\prope{real-valued})
  \end{array}}
}
\end{proposition}
\begin{proof}
This follows from $\Rxx(m)$ (1)--(4) being \prope{conjugate symmetric} \xref{prop:Rxy}
and from \prefpp{prop:dtft_conjsym}.
\end{proof}
