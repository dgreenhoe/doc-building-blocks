...
\label{def:white_seq}
%---------------------------------------
Let $\rvx(n)$ be a \prope{wide sense stationary} \xref{def:wss} \fncte{random sequence} \xref{def:randseq}.
Let $\Rxx(n,m)$ be the \fncte{auto-correlation} \xref{def:Rxxm} of $\rvx(n)$.
\defboxt{
...
%If a random sequence $\rvx(m)$ is \prope{white} \xref{def:white_seq}
...
%$\inprod{n(n)}{\fpsi_n(n)}$ and $\inprod{n(n)}{\fpsi_m(n)}$ are \prope{uncorrelated}
%for $m\ne  n$.
%However, if $\rvx(n)$ is \propb{colored} (not white), then the innerproducts are not
...
%then by \prefpp{thm:Rxx_prop}, the set $\setn{\fpsi_n(n)}$ are \prope{orthogonal} and
%the innerproducts \textbf{are} \prope{uncorrelated} even though $\rvx(n)$ is
%not white.
...
\section{Examples}
%=======================================
Sunspot data --- see \prefpp{ex:sunspots}