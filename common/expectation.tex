...
\label{ex:jensen}
%--------------------------------------
Some examples of \thme{Jensen's Inequality} \xref{cor:jenson} applied to the \ope{expectation operator}
are the following:
\exbox{\begin{array}{>{\ds}rc>{\ds}l}
    \brp{\pE\rvX}^{-1} &<& \pE\brp{\rvX^{-1}}
  \\\pE\brp{\log\rvX}  &<& \log\brp{\pE\rvX}
  \\e^{-\pE\rvX}       &\le& \pE\brs{e^{-\rvX}}
\end{array}}
\end{example}
