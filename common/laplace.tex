...
\thmbox{\begin{array}{F >{\ds}lc>{\ds}r Dl C                 }
  ...
  \\[2ex]
    (2). & \opLT\brs{\step(-x)} &=& -\frac{1}{s} & for & \Real(s) < 0 & \forall x\in\R
\end{array}}
\end{theorem}
\begin{proof}
\begin{align*}
...
\begin{align*}
  ...
  \opLT\brs{e^{-\alpha x}\step(-x)}(s)
    ...
    && \text{\xref{thm:opLT_mod}}
  \\&= \frac{-1}{s+\alpha}
    && \mathrlap{\text{$\forall\Real(s) \in \intoo{-\infty-\Real(\alpha)}{0-(-\Real(\alpha))}$ by \prefp{thm:opLT_step}}}
...
\begin{enumerate}
  \item Proof for (1):
    \begin{align*}
      ...
      \\&= \frac{s+\alpha}{(s+\alpha)^2 + \omega_o^2}
        && \forall \Real(s) \in \intoo{-\Real(\alpha)}{\Real(\alpha)}
        && \mathrlap{\text{by \prefp{cor:opLT_step}}}
    \end{align*}
  ...
  \item Proof for (2):
    \begin{align*}
      ...
      \\&= \mathrlap{
           ...
           \quad\text{by \pref{cor:opLT_step} and \prefp{thm:opLT_mod}}}
    \end{align*}
...
\begin{proof}
  ...
  \begin{align*}
    \opLT \int_{u=-\infty}^{u=x} \fg(u) \du
      &\eqd \int_{x=-\infty}^{x=+\infty} \brs{\int_{u=-\infty}^{u=x} \fg(u) \du} e^{-s x} \dx
      && \text{by definition of $\opLT$}
      && \text{\xref{def:opLT}}
    \\&= \int_{x=-\infty}^{x=+\infty} \brs{\int_{u=-\infty}^{u=+\infty} \fg(u) \step(x-u) \du} e^{-s x} \dx
    \\&= \int_{v=-\infty}^{v=+\infty} \int_{u=-\infty}^{u=+\infty} \fg(u) \fh(v)  e^{-s (u+v)} \du \dv
      && \brp{\begin{array}{Mr@{\hspace{2pt}}c@{\hspace{2pt}}l}
           where      & v&\eqd&x-u\\
           $\implies$ & x&=&u+v
         \end{array}}
    \\&= \brs{\int_{v=-\infty}^{v=+\infty} \step(v) e^{-s v} \dv}
          \mcom{\brs{\int_{u=-\infty}^{u=+\infty} \fg(u)   e^{-s u} \du }}
               {\ope{Laplace Transform} of $\fg(x)$}
    \\&= \brs{\int_{v=0}^{v=\infty} e^{-s v} \dv} \brs{\opLT\fg}(s)
    \\&= \brlr{\frac{1}{-s}e^{-s v}}_{v=0}^{v=\infty} \brs{\opLT\fg}(s)
      && \mathrlap{\text{by \thme{Fundamental Theorem of Calculus}}}
    \\&= \frac{1}{s} \brs{\opLT\fg}(s)
      && \text{by definition of $\opLT$}
      && \text{\xref{def:opLT}}
  \end{align*}
\end{proof}
