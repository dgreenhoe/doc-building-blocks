...
\begin{figure}[th]
  ...
  \exbox{\begin{array}{NNN}
    ...
    \\\fncte{Daubechies-1}
     &\fncte{Daubechies-2}
     &\fncte{Daubechies-3}
    \\\ifxref{compactp}{ex:dau-p1}
     &\ifxref{compactp}{ex:dau-p2}
     &\ifxref{compactp}{ex:dau-p3}
  \end{array}}
  \caption{\fncte{Daubechies-$p$ wavelet function}s with $p$ \prope{vanishing moment}s 
           \xref{ex:wavstrct_Dp}\label{fig:wavstrct_Dp}}
\end{figure}
%--------------------------------------
\begin{example}
\label{ex:wavstrct_Dp}
%--------------------------------------
\prefpp{fig:wavstrct_Dp} illustrates some \fncte{Daubechies-$p$ wavelet function}s with $p$ \prope{vanishing moment}s.
Note the following \xref{thm:wavstrct_cqf}:
\\\indentx$\begin{array}{l>{\ds}rc>{\ds}lc>{\ds}l}
    p=1 & \sum_{n=0}^{n=1} g_n 
        &=& \frac{1}{2} \brp{ \sqrt{2} - \brp{-\sqrt{2}} } 
        &=& \sqrt{2}
    \\
    p=2 & \sum_{n=0}^{n=3} g_n 
        &=& \frac{\sqrt{2}}{8} \brp{ (1-\sqrt{3}) + (3-\sqrt{3}) + (3+\sqrt{3}) + (1+\sqrt{3}) }
        &=& \sqrt{2}
\end{array}$
\end{example}
