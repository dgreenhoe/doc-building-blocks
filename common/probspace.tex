...
%---------------------------------------
    ...
    \structe{orthocomplemented O$_6$ lattice}\ifsxref{ortholat}{def:o6}.
...
In analysis, arguably the most fundamental subset structure is the humble \structe{topology}\ifsxref{topology}{def:topology}.
...
called a \structe{\txsigma-algebra} (\structe{sigma-algebra})\ifsxref{measure}{def:sigalg}.
...
is called a \structe{measure space}\ifsxref{measure}{def:mspace}.
...
\defboxp{
  ...
  \\(2).& $\pse$ is a \structe{\txsigma-algebra} on $\pso$      & \ifsxref{measure}{def:sigalg} & and
  \\(3).& $\psP:\pse\to[0,1]$ is a \structe{measure} on $\pse$  & \ifsxref{measure}{def:measure}& .
  \end{array}$
  \\
...
\end{theorem}
\begin{proof}
$\psP$ is a \fncte{measure}\ifsxref{measure}{def:measure}.
\end{proof}
...
%---------------------------------------
\begin{theorem}[\thmd{Bayes' Rule}]
\footnote{
  \citerpgc{haykin2014}{95}{9781118476772}{``Bayes' Rule"}
  }
%---------------------------------------
\thmbox{
...
and where $\int$ is the \ope{Lebesgue integral}\ifsxref{integrat}{def:intL}.
Then $\opair{\psL}{\psP}$ is a \structd{probability space}.
\\\tbox{\includegraphics{../common/math/graphics/pdfs/pdf_Normal_0_1.pdf}}
\qquad\tbox{\includegraphics{../common/math/graphics/pdfs/pdf_Normal_1_quadRoot1over2pi.pdf}}
...
$\pso\eqd\setn{x,y,z}$.
There are five \structe{algebra of sets}\ifsxref{setstrct}{def:ss_algebra} on $\pso$
