...
\defbox{\begin{array}{M>{\ds}rc>{\ds}lC}
  The \hid{spectrum} of $x\in\algA$ is        & \oppSpec(x)        &\eqd& \set{\lambda\in\C}{\lambda e - x \text{ is not invertible}}.\\
  The \hid{resolvent} of $x\in\algA$ is       & \oppRes_x(\lambda) &\eqd& (\lambda e - x)^{-1}   & \forall\lambda\notin\oppSpec(x).\\
  The \hid{spectral radius} of $x\in\algA$ is & \oppRad(x)         &\eqd& \sup\set{\abs{\lambda}}{\lambda\in\oppSpec(x)}.
\end{array}}
\end{definition}
