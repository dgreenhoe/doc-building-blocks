...
\corbox{\begin{array}{>{\ds}rc>{\ds}lC}
    \pVar(\psdev\rvX+b) &=& \psdev^2\pVar(\rvX)           & \forall \psdev,b  \in\R
...
\\
  \pVar(\psdev\rvX+b)
    &=    \pEx(\psdev\rvX+b)^2  - [\pEx(\psdev\rvX+b)]^2
  \\&=    \pEx(\psdev^2\rvX^2+2\psdev b\rvX+b^2)  - [\psdev(\pEx\rvX)+b]^2
  \\&=    \psdev^2 \pEx\rvX^2  +2\psdev b\pEx\rvX + b^2 - \brs{\psdev^2[\pEx\rvX]^2 + 2\psdev b\pEx\rvX + b^2}
    &&    \text{by \prope{linearity} of $\pE$}
    &&    \text{\xref{thm:pE_linop}}
