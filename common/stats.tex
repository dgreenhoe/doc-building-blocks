...
\thmbox{\begin{array}{rclM}
  \pEx(aX+b)  &=& \brs{a\pEx\rvX} + b    & (\prope{linaer}) \\
  \pVar(aX) &=& a^2\pVar(X) \\
...
\defbox{
  \begin{array}{llrcll}
    \mbox{\bf \prope{convex}}           \mbox{ in } \intoo{a}{b} & \mbox{if} & \ff(x) &\le& \fy(x) & \mbox{for } x\in\intoo{a}{b} \\
    \mbox{\bf concave}          \mbox{ in } \intoo{a}{b} & \mbox{if} & \ff(x) &\ge& \fy(x) & \mbox{for } x\in\intoo{a}{b} \\
    \mbox{\bf strictly \prope{convex}}  \mbox{ in } \intoo{a}{b} & \mbox{if} & \ff(x) &<  & \fy(x) & \mbox{for } x\in\intoo{a}{b} \\
    \mbox{\bf strictly concave} \mbox{ in } \intoo{a}{b} & \mbox{if} & \ff(x) &>  & \fy(x) & \mbox{for } x\in\intoo{a}{b}
  \end{array}
  }
\end{definition}

The next theorem gives another form of \prope{convex} functions that is a little
...
\thmbox{
  \ff \mbox{ is \prope{convex} in $\intoo{a}{b}$ } \iff
        \ff(\lambda x_1+(1-\lambda)x_2) \le \lambda\ff(x_1) + (1-\lambda)\ff(x_2)
  }
\end{theorem}
\begin{proof}
\begin{enumerate}
  \item prove $\ff$ is \prope{convex} $\implies$
        $\ff(\lambda x_1+(1-\lambda)x_2) \le \lambda\ff(x_1) + (1-\lambda)\ff(x_2)$:
  \begin{align*}
  ...
  \end{align*}
  \item prove $\ff$ is \prope{convex} $\impliedby$
...
for whether that function is \prope{convex}.
%---------------------------------------
\begin{theorem}
\label{thm:convex_d2}
\citepp{cover}{24}{25}
%---------------------------------------
\thmbox{
  \ff''(x)>0 \implies \ff \mbox{ is \prope{convex}}
  }
\end{theorem}
\begin{proof}
\begin{align*}
...
By \prefpp{thm:convex_lambda}, $\ff(x)$ is \prope{convex}.
\end{proof}
...
And now for an extremely useful application of \prope{convex}ity to the
...
\thmbox{\begin{tabular}{Fl}
  1. & $V$ is a vector space. \\
  2. & $\inprod{\rvX}{\rvY}\eqd\pE(\rvX\rvY^\ast)$ is an inner product. \\
