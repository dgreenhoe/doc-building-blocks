...
\footnote{
  ...
  \citerpuc{betten2008L}{295}{https://link.springer.com/content/pdf/bbm:978-3-540-85051-9/1.pdf}{(B.1)}
  }
\label{def:opLT}
%--------------------------------------
...
\begin{remark}
%--------------------------------------
The scaling factor $\frac{1}{\sqrt{2\pi}}$ is not typically found references offering 
...
\footnote{
  \citerpgc{bracewell1978}{224}{007007013X}{Table 11.1 Theorems for the Laplace Transform},
  \citerpuc{betten2008H}{296}{https://link.springer.com/content/pdf/bbm:978-3-540-85051-9/1.pdf}{(B.6)}
  }
...
\corbox{\begin{array}{>{\ds}lc>{\ds}c     D     l                             C }
    \mc{3}{N}{Transform}                & \mc{2}{|N}{Radius of Convergence} & \mc{1}{|N}{Domain}
  \\\hline
    \opLT\brs{\step(x)\cos(\omega_o x)}  &=& \frac{s}{s^2+\omega_o^2}         & for & \Real(s) > 0 & x,\omega_o\in\R
  \\[2ex]\opLT\brs{\step(x)\sin(\omega_o x)}  &=& \frac{\omega_o}{s^2+\omega_o^2}  & for & \Real(s) > 0 & x,\omega_o\in\R
  \\[2ex]\opLT\brs{\step(-x)}                 &=& \frac{-1}{s}                     & for & \Real(s) < 0 & x\in\R
  \\[2ex]\opLT\brs{\step(-x)\cos(\omega_o x)} &=& \frac{-s}{s^2+\omega_o^2}        & for & \Real(s) < 0 & x,\omega_o\in\R
  \\[2ex]\opLT\brs{\step(-x)\sin(\omega_o x)} &=& \frac{-\omega_o}{s^2+\omega_o^2} & for & \Real(s) < 0 & x,\omega_o\in\R
\end{array}}
\end{corollary}
\begin{proof}
\begin{align*}
  ...
  \opLT\brs{\step(-x)} 
    &= \frac{-1}{s}
    && \Real(s) < 0
   %&& \text{by \ope{reversal} property \xref{cor:opLT_transversal} and \pref{thm:opLT_step}}
    && \text{by \prefpp{cor:opLT_transversal} and \pref{thm:opLT_step}}
  \\
  \opLT\brs{\step(-x)\cos(\omega_o x)} 
    &= \frac{1}{2}\brs{\frac{-1}{s-i\omega_o}} + \frac{1}{2}\brs{\frac{-1}{s+i\omega_o}}
    && \Real(s) < 0
    && \text{by \prefp{cor:opLT_mod} and \pref{thm:opLT_step}}
  \\&= \frac{-s}{s^2+\omega_o^2}
    && \Real(s) < 0
  \\
  \opLT\brs{\step(-x)\sin(\omega_o x)} 
...
\begin{theorem}
\footnote{
  \citerpuc{betten2008L}{301}{https://link.springer.com/content/pdf/bbm:978-3-540-85051-9/1.pdf}{(B.27)}
  }
\label{thm:opLT_diff}
\label{thm:opLT_int}
%---------------------------------------
