...
%=======================================
\section{Effects of additive noise and operators}
%=======================================
\begin{figure}[h]
  \centering
  \begin{tabular}{|l|l|l|}
...
\thmbox{
  \brb{\begin{array}{FMMD}
      (A).& $\rvx(n)$               is  & (\prope{WSS})          & and
    \\(B).& $\rvx(n)$ and $\rvu(n)$ are & (\prope{uncorrelated}) & and
    \\(C).& $\rvu(n)$               is  & (\prope{zero-mean})    &
  \end{array}}
  \implies
...
\prefpp{cor:ZSxy} demonstrates that if a random input $\rvx(n)$ is processed by an \prope{LTI} system
