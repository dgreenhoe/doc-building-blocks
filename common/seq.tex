...
\defboxp{%\begin{array}{M}
  A sequence $\seqxZ{x_n}$ \hid{converges} to a point $x$ if 
...
%======================================
One of the most important applications of \structe{metric space}s\ifsxref{metric}{def:metric} in analysis is the concept 
of \hie{convergence}.
...
%We say that a sequence $\seqn{x_n}$ \hie{converges} to a value $x$ in a 
%metric space $\opair{\spX}{\metricn}$
%when the distance $\metric{x_n}{x}$ between $x$ and $x_n$ approaches $0$ as $n$ approaces infinity.
%There are two basic kinds of sequences that ``converge":\\
...
\defbox{\begin{array}{l>{\ds}l}
  \mc{2}{l}{\text{A sequence $\seqxZ{x_n\in\setX}$
                  is a \hid{Cauchy sequence} in the metric space $\metspaceX$ if}} 
