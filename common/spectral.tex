...
Let $\oppN(\opA)$ be the \structe{null space} of $\opA$.
\defbox{\begin{array}{l}
  \text{An \vald{eigenvalue} of $\opA$ is any value $\lambda$ such that there exists $\vx$ such that $\opA\vx=\lambda\vx$.} \\
  \text{The \structd{eigenspace} $\spH_\lambda$ of $\opA$ at eigenvalue $\lambda$ is $\oppN(\opA-\lambda\opI)$.} \\
  \text{An \vald{eigenvector} of $\opA$ associated with eigenvalue $\lambda$ is any element of $\oppN(\opA-\lambda\opI)$.}
\end{array}}
\end{definition}
...
\defboxp{\indxs{\oppRes}\indxs{\oppSpec}
  The \structd{resolvent set} $\oppRes(\opA)$ of operator $\opA$ is defined as
  \[
    \oppRes(\opA) \eqd 
    \set{\lambda\in F}
        {\begin{array}{ll @{\qquad}D @{\qquad}>{$}l<{$}}
         ...
         \end{array}}
  \]
  The \structd{spectrum} $\oppSpec(\opA)$ of operator $\opA$ is defined as
...
\defbox{\indxs{\oppSpecp}\indxs{\oppSpecr}\indxs{\oppSpecc}
  \begin{array}{rcl}
  \mc{3}{l}{\text{The \structd{point spectrum} $\oppSpecp(\opA)$ of operator $\opA$ is defined as }} \\
  \qquad\oppSpecp(\opA) 
  &\eqd& 
  \set{\lambda\in F}
      {\begin{array}{ll @{\qquad}D @{\qquad}>{$}l<{$}}
      ...
      \end{array}}
  \\
  \mc{3}{l}{\text{The \structd{residual spectrum} $\oppSpecr(\opA)$ of operator $\opA$ is defined as }} \\
  \qquad\oppSpecr(\opA) 
  &\eqd& 
  \set{\lambda\in F}
      {\begin{array}{ll @{\qquad}D @{\qquad}>{$}l<{$}}
      ...
      \end{array}}
  \\
  \mc{3}{l}{\text{The \structd{continuous spectrum} $\oppSpecc(\opA)$ of operator $\opA$ is defined as }} \\
  \qquad\oppSpecc(\opA) 
  &\eqd& 
  \set{\lambda\in F}
      {\begin{array}{ll @{\qquad}D @{\qquad}>{$}l<{$}}
...
then that $\lambda$ part of the \structe{resolvent set} of $\opA$ and $\lambda$ is called
\prope{regular}.
However if for some $\lambda$ the operator $\opB(\lambda)$ 
fails any of these conditions, then that $\lambda$ part of the \structe{spectrum} of $\opA$.
...
\thmbox{\begin{array}{l}
  \left.\begin{array}{Fl}
    (A). & \mcom{\opNa\opN=\opN\opNa}{$\opN$ is \prope{normal}}\\
    (B). & \text{$\opN$ is \prope{compact}}
  \end{array}\right\}
  \qquad\implies\qquad
...
\defbox{\begin{array}{l@{\qquad}l}
  \mc{2}{l}{\text{A \opd{Fredholm operator} $\opK$ is defined as}}
  \\&
  \brs{\opK\ff}(t) 
  \eqd 
  \mcom{\int_a^b \mcom{\kernel(t,s)}{kernel} \ff(s) \dd{s} }
       {Fredholm integral equation of the first kind \footnotemark}
  \qquad \forall \ff\in \spL_2([a,b])
  \end{array}}
\footnotetext{
   The equation $\int_u \kernel(t,s) \ff(s) \dd{s}$ is a 
   \fnctd{Fredholm integral equation of the first kind} and
   $\kernel(t,u)$ is the {\bf kernel} of the equation.
   References: 
     \citer{fredholm1900},
...
\\
  4. & \text{autocorrelation operator}
     ...
     & \kernel(t,s)   &=& R(t,s) %\text{ (autocorrelation function)}
\end{array}
\]
\end{example}
...
%---------------------------------------
Let $\opK$ be a \prop{Fredholm operator} with kernel $\kernel(t,s)$ and adjoint $\opKa$.
\thmbox{
  \brs{\opK\ff}(t) = \int_\setA \kernel(t,s) \ff(s) \dds
  \qquad\iff\qquad
  \brs{\opKa\ff}(t) = \int_\setA \kernel^\ast(s,t) \ff(s) \dds
  }
\end{theorem}
\begin{proof}
\begin{align*}
...
%---------------------------------------
Let $\opK$ be an \prop{Fredholm operator} with kernel $\kernel(t,s)$ and adjoint $\opKa$.
\corbox{
  \mcom{\opK=\opKa}{$\opK$ is self-adjoint}
  \qquad\iff\qquad
  \mcom{\kernel(t,s) = \kernel^\ast(s,t)}{kernel is \prop{conjugate symmetric}}
  }
\end{corollary}
\begin{proof}
\begin{align*}
...
%---------------------------------------
\begin{theorem}[\thmd{Mercer's Theorem}]
\label{thm:mercer}
\footnote{
  \citerp{gohberg}{198},
...
%---------------------------------------
Let $\opK$ be an \prop{Fredholm operator} with kernel $\kernel(t,s)$ and
eigensystem $\seq{(\lambda_n,\phi_n(t))}{n\in\Z}$.
\thmbox{
  \brb{\begin{array}{FMD}
    (A). & $\ds\mcom{\int_a^b \int_a^b \kernel(t,s) \ff(t) \ff^\ast(s) \ds\dt \ge 0}{positive}$ & and\\
    (B). & $\kernel(t,s)$ is \prope{continuous} on
       \\& $\intcc{a}{b}\times\intcc{a}{b}$
  \end{array}}
  \implies
  \brb{\begin{array}{F>{\ds}lD}
    (1). & \kernel(t,s) = \sum_n \lambda_n \fphi_n(t) \fphi_n^\ast(s) & and\\
    (2). & \mc{2}{M}{$\kernel(t,s)$ \prope{converges absolutely}}
       \\& \mc{2}{M}{and \prope{uniformly} on}
       \\& \mc{2}{M}{$\intcc{a}{b}\times\intcc{a}{b}$}
  \end{array}}
  }
\end{theorem}
