...
$\estHs\to\estHa$ as $s\to\infty$  \xref{prop:Hs_lim}.
...
\begin{figure}[h]
  \begin{tabular}{|c|c|}
    \hline
     \includegraphics{graphics/transferHs_p.pdf}
    &\includegraphics{graphics/transferHs_ddp.pdf}
    \\
...
% ______________________________________
%|T|                                    |
%|H| Hs is monotonically decreasing in s|
%|M|                                    |
%|_|____________________________________|
\begin{theorem}                         %
\label{thm:Hs_mono}                     %
...
\thmbox{\begin{array}{M}
  $\ds\brb{s_1 < s_2} 
  \qquad\implies\qquad
  \estHs(\omega;s_2) \le \estHs(\omega;s_1)$
  \\
  ($\estHs$ is \propb{monotonically decreasing} in $s$)
\end{array}}
\end{theorem}
\begin{proof}
Proof not complete ... work in progress
\begin{enumerate}
  \item Let $p\eqd s^2$ \label{item:Hs_mono_p}
  \item lemma: \label{ilem:Hs_ineq}
      $4 \abs{\Swxy }^2   -2\Swxx\brp{\Swyy  -  p \Swxx} - 2\Swxx\sqrt{\brp{\Swyy  -  p \Swxx }^2 + 4 p \abs{\Swxy }^2} \le 0$.
      proof:
    \begin{align*}
      &4 \abs{\Swxy }^2   -2\Swxx\brp{\Swyy  -  p \Swxx} - 2\Swxx\sqrt{\brp{\Swyy  -  p \Swxx }^2 + 4 p \abs{\Swxy }^2} \le 0
    \\&\iff
      2 \abs{\Swxy }^2   -\Swxx\brp{\Swyy  -  p \Swxx} \le \Swxx\sqrt{\brp{\Swyy  -  p \Swxx }^2 + 4 p \abs{\Swxy }^2}
    \\&\iff
      \brs{2 \abs{\Swxy }^2   -\Swxx\brp{\Swyy  -  p \Swxx}}^2 \le \brs{\Swxx\sqrt{\brp{\Swyy  -  p \Swxx }^2 + 4 p \abs{\Swxy }^2}}^2
    \\&\iff
      4 \abs{\Swxy }^4   -4\Swxx\abs{\Swxy}\brp{\Swyy  -  p \Swxx} +\Swxx^2\brp{\Swyy  -  p \Swxx}^2 \le \Swxx^2\brs{\brp{\Swyy  -  p \Swxx }^2 + 4 p \abs{\Swxy }^2}
    \\&\iff
      4 \abs{\Swxy }^4   -4\Swxx\abs{\Swxy}\brp{\Swyy  -  p \Swxx} +\cancelto{0}{\Swxx^2\brp{\Swyy  -  p \Swxx}^2} \le \Swxx^2\brs{\cancelto{0}{\brp{\Swyy  -  p \Swxx }^2} + 4 p \abs{\Swxy }^2}
    \\&\iff
      4 \abs{\Swxy }^4   -4\Swxx\abs{\Swxy}\brp{\Swyy  -  p \Swxx}  \le 4p\Swxx^2\abs{\Swxy }^2
    \\&\iff
      4 \abs{\Swxy }^4   -4\Swxx\abs{\Swxy}\brp{\Swyy  -  \cancelto{0}{p \Swxx}}  \le \cancelto{0}{4p\Swxx^2\abs{\Swxy }^2}
    \\&\iff
      \abs{\Swxy }^4   \le \abs{\Swxy}^2\Swxx\Swyy
    \\&\iff
      \abs{\Swxy }^2   \le \Swxx\Swyy
    \\&\text{true by \thme{Cauchy Schwartz inequality} \xref{thm:cs}}
    \end{align*}
  ...
  \item lemma: derivative of $\estHs$ with respect to $p\eqd s^2$ \ldots
    {\begin{align*}
      \ddp\estHs
        ...
        && \text{by definition of $\estHs$}
        && \text{\xref{def:Hs}}
      \\&\eqd \ddp\frac{\Swyy - p \Swxx  +
                          \sqrt{\brp{\Swyy - p \Swxx }^2 + 4 p \abs{\Swxy }^2}
                       }{2\Swxy }
        && \text{by definition of $p$}
        && \text{\xref{item:Hs_mono_p}}
      \\&= \frac{\ds- \Swxx  + \frac{-2\Swxx\brp{\Swyy  -  p \Swxx } + 4 \abs{\Swxy }^2}
                                    {2\sqrt{\brp{\Swyy  -  p \Swxx }^2 + 4 p \abs{\Swxy }^2}}
                }{\ds2\Swxy }
      \\&= \frac{4 \abs{\Swxy }^2   -2\Swxx\brp{\Swyy  -  p \Swxx} - 2\Swxx\sqrt{\brp{\Swyy  -  p \Swxx }^2 + 4 p \abs{\Swxy }^2}}
                {4\Swxy\sqrt{\brp{\Swyy  -  p \Swxx }^2 + 4 p \abs{\Swxy }^2}}
      \\&\le 0
        &&\text{by \pref{ilem:Hs_ineq}}
%\\
%\\
%      \\&\le \frac{4 \abs{\Swxy }^2   -2\Swxx\brs{\brp{\Swyy  -  p \Swxx} + \sqrt{\brp{\Swyy  -  p \Swxx }^2 }}}
%                  {4\Swxy\sqrt{\brp{\Swyy  -  p \Swxx }^2 + 4 p \abs{\Swxy }^2}}
%      \\&=   \frac{\abs{\Swxy }^2   -\Swxx\brp{\Swyy  -  p \Swxx} }
%                  {\Swxy\sqrt{\brp{\Swyy  -  p \Swxx }^2 + 4 p \abs{\Swxy }^2}}
    \end{align*}}
\end{enumerate}
\end{proof}
