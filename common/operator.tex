...
\defboxt{
  ...
  %\mc{3}{M}{If $\spY=\F$, then $\opA$ is a \fnctd{functional}.}
  }
...
Two operators $\opA$ and $\opB$ in $\clOxy$ are \propd{equal} if 
...
%---------------------------------------
\begin{theorem}[\thmd{Fundamental theorem of linear equations}]
...
\propbox{\begin{array}{>{\qquad}F>{\ds}rc>{\ds}l @{\qquad}C @{\qquad}D @{\qquad}D}
  ...
  \\
    ...
    & (\prope{subadditive}).
  \\
  \mc{7}{M}{Moreover, $\opair{\clLxy}{\normopn}$ is a \structd{normed linear space}.}
\end{array}}
\end{proposition}
\begin{proof}
\begin{align*}
...
\defbox{\begin{array}{M}
  An operator $\opB$ is \propd{bounded} if $\normop{\opB} < \infty$.
  \\
  The quantity $\clBxy$ is the set of all \structd{bounded linear operators} on $\opair{\spX}{\spY}$ such that
...
%$\spX$ and $\spY$.
%\defbox{\begin{array}{M}
%  An operator $\opL$ is \propd{continuous} if\quad
%    for all $\epsilon>0$ there exists $\delta>0$ such that
%    \\\qquad$\ds\norm{\vx-\vy}<\delta \qquad\implies\qquad \norm{\opL\vx - \opL\vy} < \epsilon.$\\
%  %The set $\clCxy$ is defined to be the set of all \structd{continuous operators} on $\opair{\spX}{\spY}$ such that
...
%  An operator $\opM$ is \propd{isometric} if & \norm{\opM\vx-\opM\vy} &=& \norm{\vx-\vy} & \forall \vx,\vy\in\setX.\\
%  An operator $\opU$ is \propd{unitary}   if & \norm{\opU\vx}         &=& \norm{\vx}     & \forall \vx\in\setX.\\
%  An operator $\opN$ is \propd{normal}    if & \norm{\opNa\vx}        &=& \norm{\opN\vx} & \forall \vx\in\setX.
%\end{array}}
%\end{definition}
...
Let $\clOxy$ be the set of all operators from a linear space $\spX$ to a linear space $\spY$.
Let $\oppN(\opL)$ be the \hie{Null Space} of an operator $\opL$ in $\clOxy$
and $\oppI(\opL)$ the \structe{image set} of $\opL$ in $\clOxy$\ifsxref{relation}{def:rel_null}.
\thmbox{\begin{aligned}
...
\begin{definition}
\footnote{
  \citerpgc{rudinf}{126}{0070542252}{5.15 Projections},
  \citerpg{kubrusly2001}{70}{0817641742},
  \citerpg{bachman1966}{26}{0486402517},
  \citerpgc{halmos1958}{73}{9781461263876}{\textsection 41. Projections}
  }
\label{def:opP}
...
Let $\clBxy $ be the space of \structe{bounded linear operators} on normed linear spaces $\spX$ and $\spY$.
Let $\opP$ be a bounded linear operator in $\clBxy$
with \hie{Null Space} $\oppN(\opP)$ and \structe{image set} $\oppI(\opP)$\ifsxref{relation}{def:rel_null}.
\thmbox{
...
with adjoint $\opPa$, \structe{Null Space} $\oppN(\opP)$, and \structe{image set} $\oppI(\opP)$.%
...
\footnote{
  ...
  \citerpgc{bachman1966}{24}{9780486402512}{Theorem 2.1},
  \citerpgc{bertero1998}{225}{9781439822067}{\textsection ``9.2 SVD of a matrix \ldots If all eigenvectors are normalized\ldots"}
  }
...
Let $\oppN(\opN)$ be the \structe{Null Space} of an operator $\opN$ in $\clBhh$
and $\oppI(\opN)$ the    \structe{image set} of $\opN$ in $\clBhh$\ifsxref{relation}{def:rel_null}.
\thmbox{
...
Let $\clBhh $ be the space of \structe{bounded linear operators} on a \structe{Hilbert space} $\spH$.
Let $\oppN(\opN)$ be the \structe{Null Space} of an operator $\opN$ in $\clBhh$
and $\oppI(\opN)$ the    \structe{image set} of $\opN$ in $\clBhh$.
\thmbox{
...
\end{theorem}
\begin{proof}
The proof in (1) is flawed. This implies that (2) is also flawed. 
\citerp{rudinf}{313} claims both to be true. \problem
\begin{align*}
  \intertext{1. Proof that
...
\defbox{\begin{array}{M}
  An operator $\opM\in\clLxy$ is \propd{isometric} if
...
\defbox{\begin{array}{M rcl}
  The operator $\opU$ is \propd{unitary} if & \opUa\opU &=& \opU\opUa = \opI .
\end{array}}
\end{definition}
...
%---------------------------------------
Let $\clBhh $ be the space of \structe{bounded linear operators} on a \structe{Hilbert space} $\spH$.
Let $\oppI(\opU)$ be the \structe{image set}\ifsxref{relation}{def:rel_image} of $\opU$.
\thmbox{\begin{array}{>{\qquad}FlDlDc}
  \mc{6}{M}{If $\opU$ is a \structb{bounded linear operator} ($\opU\in\clBhh$), then the following conditions are \propb{equivalent}:}
  \\
   ...
   \\3. & \norm{\opU\vx-\opU\vy} = \norm{\opUa\vx-\opUa\vy} = \norm{\vx-\vy}
        & and
        & \oppI(\opU)=\setX
        & (\prope{isometric in distance})
        & \iff
   \\4. & \norm{\opU\vx} = \norm{\vx}
        & and
        & \oppI(\opU)=\setX
        & (\prope{isometric in length})
        &
\end{array}}
\end{theorem}
\begin{proof}
...
and $\oppI(\opU)$ the \structe{image set} of $\opU$.\ifdochas{relation}{\footnote{%
  \begin{tabular}{ll}
    \structe{Null Space}: & \ifxref{relation}{def:rel_null} \\
    \structe{image set}:  & \ifxref{relation}{def:rel_image}
  \end{tabular}}}
\thmbox{\begin{array}{l}
  \mcom{\opU\opUa=\opUa\opU=\opI}{$\opU$ is \propb{unitary}}
  \implies
  \brb{\begin{array}{rclclD}
      \opUi         &=& \opUa          & &               & and
    \\\oppI(\opU)   &=& \oppI(\opUa)   &=& \setX         & and
    \\\oppN(\opU)   &=& \oppN(\opUa)   &=& \setn{\vzero} & and
...
\begin{enumerate}
  \item Note that $\opU$, $\opUa$, and $\opUi$ are all both
        \prope{isometric} and \prope{normal}:
    \[\begin{array}{lclclll}
...
\mbox{}\\
\exbox{
  \mcom{\brb{\opR_\theta \eqd
    \brs{\begin{array}{rr}
      \cos\theta & -\sin\theta   \\
      \sin\theta &  \cos\theta
    \end{array}}
  }}{\opb{rotation matrix} $\opR_\theta:\R^2\to\R^2$}
  \qquad\implies\qquad
  \brb{\begin{array}{F rcl @{\qquad}M}
    (1). & \opRi_\theta &=& \opR_{-\theta} & and\\
...
\begin{example}
\footnote{
  \citerp{gelfand1963}{4},
  \citerpg{gelfand2018}{4}{9780486823850}
  }
%---------------------------------------
Let $\opA$ and $\opB$ be matrix operators.
\exbox{\mcom{\begin{array}{N|N}
  $\opA\eqd\brs{\begin{array}{rrc}
      \cos\phi     & -\sin\phi    & 0
    \\\sin\phi     &  \cos\phi    & 0
    \\\mc{1}{c}{0} & \mc{1}{c}{0} & 1
  \end{array}}$
  & 
  $\opB\eqd\brs{\begin{array}{rrr}
      -1 &  0 &  0
    \\ 0 & -1 &  0
    \\ 0 &  0 & -1
  \end{array}}$
  \\$\opA$ is a \ope{rotation operator}. & $\opB$ is a \ope{reflection operator}.
\end{array}}{\normalsize Both $\opA$ and $\opB$ are \propb{unitary}.}}
\end{example}
...
\exbox{\begin{array}{FMrclrcl}
  1. & \opb{Fourier Transform}
     & [\opFT \fx](f) &=& \int_{t\in\R} \fx(t) e^{-i2\pi ft}\dt
     & \kappa(t,f)    &=& e^{-i2\pi ft}
\\
  2. & \opb{Inverse Fourier Transform}
     & [\opFTi \Fx](t) &=& \int_{f\in\R} \Fx(f) e^{i2\pi ft}\df
     & \kappa(f,t)    &=& e^{i2\pi ft}
\\
  3. & \opb{Laplace operator}
     & [\opL \fx](s) &=& \int_{t\in\R} \fx(t) e^{-st}\dt
     & \kappa(t,s)   &=& e^{-st}
\end{array}}
\end{example}
...
\defbox{\begin{array}{M}
  $\opP$ is \propd{positive} if \qquad $\inprod{\opP\vx}{\vx} \ge 0$ $\forall\vx\in\spX$.
  \\
