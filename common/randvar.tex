...
see \exme{Irwin-Hall Distribution} \prefp{ex:pdf_uniform_sums}.}
...
\begin{figure}
  \centering
  \resizebox{\linewidth}{!}{$\ds\begin{array}{|ccccc|}
    \hline
...
\corbox{
  ...
  \quad\implies\quad
  \brb{\ppy(x) = \begin{array}{lM}
    \frac{1}{x^2} \ppx\brp{\frac{1}{x}} & for $x\neq0$
  \end{array}}
  }
\end{corollary}
\begin{proof}
...
\begin{figure}
  ...
  $\ds\begin{array}{|ccccc|}
    \hline
    \boxed{\tbox{\includegraphics{../common/math/graphics/pdfs/pdf_Uniform_1-2.pdf}}}
    &\quad\rightarrow\quad&
    \boxed{\tbox{\includegraphics{../common/math/graphics/pdfs/1_over_x_gt0.pdf}}}
    &\quad\rightarrow\quad&
    \boxed{\tbox{\includegraphics{../common/math/graphics/pdfs/pdf_Uniform12_1X.pdf}}}
    \\
    \text{\fncte{pdf} $\ppx(x)$ of $\rvX$}
    &&
    \ff(x)=\frac{1}{x}
    ...
    \\\hline
  \end{array}$
  \caption{Flow diagram for \prefpp{ex:ppy_N01_2x1}\label{fig:ppy_U12_1X}}
\end{figure}
%---------------------------------------
\begin{example}
\label{ex:ppy_U12_1X}
...
\exboxt{$\ds
  ...
  $
  \\\ldots as illustrated in \prefpp{fig:ppy_U12_1X}.
}
\end{example}
\begin{proof}
\begin{enumerate}
  ...
  \item By straight forward application of \prefp{cor:Yf1X}\ldots
    \begin{align*}
      \ppy(x)
    ...
    \end{align*}
  \item Note that the area under $\ppy(x)$ is $1$ (which is what we should expect \xref{thm:pdf1}):
    \begin{align*}
...
\corbox{
  ...
  \qquad\implies\qquad
  \brb{
    \ppy(x) = \frac{1}{2\sqrt{x}} \brs{\ppx\brp{-\sqrt{x}} + \ppx\brp{\sqrt{x}}}
    \quad\text{for $x\ge0$}
    }
  }
\end{corollary}
...
\begin{figure}
  ...
  \resizebox{\linewidth}{!}{$\ds\begin{array}{|ccccc|}
    ...
    &\quad\rightarrow\quad&
    \boxed{\tbox{\includegraphics{../common/math/graphics/pdfs/pdf_Uniform12_Xsquared.pdf}}}
    \\
    \text{\fncte{pdf} $\ppx(x)$ of $\rvX$}
    &&
    \ff(x)=x^2
    ...
    \\\hline
  \end{array}$}
  \caption{Flow diagram for \prefpp{ex:YX2}\label{fig:YX2}}
\end{figure}
%---------------------------------------
\begin{example}
\label{ex:YX2}
...
\exboxt{$\ds
  ...
  $
  \\\ldots as illustrated in \prefpp{fig:YX2}.
}
\end{example}
\begin{proof}
\begin{enumerate}
...
\begin{figure}
  ...
  $\ds\begin{array}{|ccccc|}
    ...
    \\\hline
  \end{array}$
  \caption{Flow diagram for \prefpp{ex:ChiSq_k1}\label{fig:ChiSq_k1}}
\end{figure}
%---------------------------------------
\begin{example}
\label{ex:ChiSq_k1}
...
\exboxt{$\ds
  ...
  \quad\implies\quad
  \brb{
    \rvY \eqd \ff(\rvX) \sim \chi(1) \eqd \frac{1}{\sqrt{2}\Gamma(1/2)} x^{-1/2} e^{-x/2}
    }
  $
  \\where $\chi(k)$ is the \fncte{chi-squared distribution with $k$ degrees of freedom} \xref{def:chisq},
  \\$\Gamma(x)$ is the \fncte{Gamma function} \xref{def:gamma}, and as illustrated in \prefpp{fig:YX2}.
}
\end{example}
\begin{proof}
See \prefpp{thm:Gamma=X^2}.
\end{proof}
...
\end{figure}
%---------------------------------------
\begin{example}[\exmd{Irwin-Hall Distribution}]
%[\exmd{Sum of Uniformly Distributed Random Variables}]
\footnote{
  \citeP{hall1927},
  \citeP{irwin1927},
  \citeP{bates1955}
