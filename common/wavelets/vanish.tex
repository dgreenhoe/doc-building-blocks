...
%--------------------------------------
Let $M_n$ be the \fncte{$n$th moment} \xref{def:Mn} and 
...
\lembox{\begin{array}{>{\ds}rc>{\ds}l@{\qquad}C}
  M_n  &=&  \brs{\sqrt{2\pi}(i)^{n}\opddwn  \Ff(\omega)}_{\omega=0} & \forall n\in\Znn,\,\ff\in\spLLR\\
  \brs{\opddwn  \Ff(\omega)}_{\omega=0}  &=& \cft(-i)^n M_n         & \forall n\in\Znn,\,\ff\in\spLLR
\end{array}}
\end{lemma}
\begin{proof}
  \begin{align*}
    ...
    \\&= \int_\R \ff(x) x^n \dx
    \\&\eqd M_n
      && \text{by definition of $M_n$ \xref{def:Mn}}
  \end{align*}
\end{proof}
...
%--------------------------------------
Let $\ff(x)$ be a function in $\spLLR$ and $M_n$ the \hie{$n$th moment} \xref{def:Mn} of $\ff(x)$.
...
$\fpsi(x)$ has $5$ vanishing moments.
\begin{footnotesize}
...
\exbox{\begin{array}{lclclclclclcl}
...
\begin{math}
%  \begin{array}{rr|rrrr@{\hspace{8ex}}rrrr|c}
...
%   \\&\vdots
%   \\&= \left[\prod_{n=1}^\infty \frac{1}{\sqrt{2}} \Dh\left(\frac{\omega}{2^n}\right)\right]
