...
%20190503%%--------------------------------------
%20190503%\begin{proposition}
%20190503%\label{prop:dtft_conjsym}
%20190503%%--------------------------------------
%20190503%Let $\Dx(\omega)\eqd\opDTFT\brs{\seqn{\fx[n]}}(\omega)$ be the \fncte{discrete-time Fourier transform} \xref{def:dtft}
%20190503%of a sequence $\seqxZ{x_n}$ in $\spllR$.
%20190503%\thmbox{
%20190503%  \brb{\begin{array}{rclD}
%20190503%    \fx(n) &=& \fx^\ast(-n) & (\prope{conjugate symmetric}) 
%20190503%  \end{array}}
%20190503%  \quad\implies\quad
%20190503%  \brb{\begin{array}{lD}
%20190503%    \FX(\omega) = \FX^\ast(\omega) & (\prope{real-valued})
%20190503%  \end{array}}
%20190503%  }
%20190503%\end{proposition}
%20190503%\begin{proof}
%20190503%\begin{align*}
%20190503%  \FX^\ast(\omega)
%20190503%    &\eqd \brs{\opDTFT\fx(n)}^\ast
%20190503%  \\&\eqd \brs{\sum_{n\in\Z} \fx(n) e^{-i\omega n}}^\ast && \text{by definition of \ope{DTFT}} && \text{\xref{def:dtft}}
%20190503%  \\&=         \sum_{n\in\Z} \fx^\ast(n) e^{i\omega n}   && \text{by \prope{antiautomorphic} property} && \text{\xref{def:staralg}}
%20190503%  \\&=         \sum_{n\in\Z} \fx(-n) e^{-i\omega(-n)}    && \text{by hypothesis}
%20190503%  \\&=         \sum_{-k\in\Z}\fx(k) e^{-i\omega k}      && \text{where $k\eqd-n$} && \text{$\implies$ $n=-k$}
%20190503%  \\&=         \sum_{k\in\Z} \fx(k) e^{-i\omega k}      && \text{by \prope{absolutely summable} property}
%20190503%  \\&\eqd      \FX(\omega)                              && \text{by definition of \ope{DTFT}} && \text{\xref{def:dtft}}
%20190503%\end{align*}
%20190503%\end{proof}

%20190503%%--------------------------------------
%20190503%\begin{theorem}
%20190503%\label{thm:dtft_conjneg_real}
%20190503%%--------------------------------------
%20190503%Let $\Dx(\omega)\eqd\opDTFT\brs{\seqn{\fx[n]}}(\omega)$ be the \fncte{discrete-time Fourier transform} \xref{def:dtft}
%20190503%of a sequence $\seqxZ{\fx[n]}$ in $\spllR$.
%20190503%\thmbox{
%20190503%  \brb{\begin{array}{FMD}
%20190503%      (1).&$\Fx(\omega) \eqd \opDTFT\seqn{\fx[n]}$ & and
%20190503%    \\(2).&$\seqn{\fx[n]}$ is \prope{real-valued}
%20190503%  \end{array}}
%20190503%  \quad\implies\quad
%20190503%  \brb{\begin{array}{F>{\ds}rc>{\ds}lc>{\ds}lD}
%20190503%      (1).&\opDTFT\seqn{\fx      [-n]} &=& \Fx      (-\omega) & &              & and
%20190503%    \\(2).&\opDTFT\seqn{\fx^\conj[ n]} &=& \Fx^\conj(-\omega) &=& \Fx( \omega) & and
%20190503%    \\(3).&\opDTFT\seqn{\fx^\conj[-n]} &=& \Fx^\conj( \omega) &=& \Fx(-\omega) & 
%20190503%  \end{array}}
%20190503%  }
%20190503%\end{theorem}
%20190503%\begin{proof}
%20190503%\begin{align*}
%20190503%  \opDTFT\seqn{\fx[-n]}
%20190503%    &\eqd \sum_{n\in\Z} \fx[-n] e^{-i\omega n}
%20190503%    && \text{by definition of \ope{DTFT}}
%20190503%    && \text{\xref{def:dtft}}
%20190503%  \\&= \sum_{m\in\Z} \fx[m] e^{i\omega m}
%20190503%     && \text{where $m\eqd-n$ $\implies$ $n=-m$}
%20190503%  \\&= \sum_{m\in\Z} \fx[m] e^{-i(-\omega)m}
%20190503%  \\&\eqd \Fx(-\omega)
%20190503%    && \text{by left hypothesis}
%20190503%    \\
%20190503%    \\
%20190503%  \boxed{\Fx^\conj(-\omega)}
%20190503%    &= \boxed{\opDTFT\seqn{\fx^\conj[n]}}
%20190503%    && \text{by \prefp{thm:dtft_conjneg}}
%20190503%  \\&= \opDTFT\seqn{\fx[n]}
%20190503%    && \text{by \prope{real-valued} hypothesis}
%20190503%  \\&= \boxed{\Fx(\omega)}
%20190503%    && \text{by definition of $\Fx(\omega)$}
%20190503%    && \text{\xref{def:dtft}}
%20190503%    \\
%20190503%    \\
%20190503%  \boxed{\Fx^\conj( \omega)}
