...
allows you to determine (predict) arbitrarily closely all the points $\ff(x)$
in the vicinity of $x=a$\ifsxref{taylor}{chp:taylor}.
But analytic functions are by definition functions for which all their derivatives exist.
Thus, if a function is \prope{discontinuous}, it is simply not a candidate for the Taylor Expansion.
...
$\msm$ is the standard \fncte{Borel measure} on $\borel$, and
...
\begin{align*}
  \opFT \brs{\ff(x-y)}(\omega)
    ...
    && \text{\ifxref{laplace}{def:opLT}}
  \\&= \brlr{e^{-sy} \;\brs{\opLT \ff(x)}(s)}_{s=i\omega}
    && \text{by Laplace \prope{translation} property}
    && \text{\ifxref{laplace}{thm:opLT_shift}}
  ...
  \\&= \brlr{\brs{\brs{\opLT\fg(x)}(s-r)}}_{s=i\omega}
    && \text{by Laplace \prope{dilation} property}
    && \text{\ifxref{laplace}{thm:opLT_shift}}
...
\lembox{\begin{array}{>{\ds}rc>{\ds}l@{\qquad}C}
  \fM_n  \eqd \int_{\R} x^n \ff(x) \dx &=&  \brlr{\sqrt{2\pi}(i)^{n}\opddwn  \Ff(\omega)}_{\omega=0} & \forall n\in\Znn,\,\ff\in\spLLR\\
...
Let $\opTrn$ be the \fncte{translation operator}\ifsxref{transop}{def:opT}.
\lembox{
...
\begin{proof}
\begin{enumerate}
  \item Proof using \ope{Laplace Transform}:
    \begin{align*}
      \sqrt{2\pi}\opFT\brs{e^{-\alpha\abs{x}}}
        &\eqd \brs{\sqrt{2\pi}}\frac{1}{\sqrt{2\pi}}\int_\R e^{-\alpha\abs{ x}} e^{-i\omega x}\dx
        && \text{by definition of $\opFT$}
        && \text{\xref{def:opFT}}
      \\&= \brs{\int_\R e^{-\alpha\abs{ x}} e^{-s x} \dx}_{s=i\omega}
      \\&= \brs{\frac{2\alpha}{\alpha^2 - s^2}}_{s=i\omega}
        && \forall\Real(s) \in \intoo{-\alpha}{\alpha}
        && \text{by \prefp{cor:opLT_expabs}}
      \\&= \frac{2\alpha}{\alpha^2 + \omega^2}
        && \text{because $s=i\omega$ is in $\intoo{-\alpha}{\alpha}$}
    \end{align*}

  \item Alternate proof:
    \begin{align*}
      \sqrt{2\pi}\opFT\brs{e^{-\alpha\abs{x}}}
        &\eqd \brs{\sqrt{2\pi}}\frac{1}{\sqrt{2\pi}}\int_{-\infty}^{\infty} e^{-\alpha\abs{ x}} e^{-i\omega x}\dx
        && \text{by definition of $\opFT$}
        && \text{\xref{def:opFT}}
      \\&= \int_{-\infty}^{0} e^{-\alpha(- x)} e^{-i\omega x}\dx
         + \int_{0}^{\infty} e^{-\alpha( x)} e^{-i\omega x}\dx
      \\&= \int_{-\infty}^{0} e^{ x(\alpha-i\omega)}\dx
         + \int_{0}^{\infty} e^{ x(-\alpha-i\omega)}\dx
      \\&= \brlr{\frac{e^{ x(\alpha-i\omega)}}{\alpha-i\omega}}_{-\infty}^{0}
         + \brlr{\frac{e^{ x(-\alpha-i\omega)}}{-\alpha-i\omega}}_{0}^{\infty}
        && \mathrlap{\text{by Fundamental Theorem of Calculus}}
      \\&= \brs{\frac{1}{\alpha-i\omega} - 0}
         + \brs{0- \frac{1}{-\alpha-i\omega}}
      \\&= \brs{\frac{1}{\alpha-i\omega}}\brs{\frac{\alpha-i\omega}{\alpha-i\omega}}
         + \brs{\frac{1}{\alpha+i\omega}}\brs{\frac{\alpha+i\omega}{\alpha+i\omega}}
      \\&= \frac{\alpha-i\omega}{\alpha^2+\omega^2}
         + \frac{\alpha+i\omega}{\alpha^2+\omega^2}
...
%    &\eqd \sqrt{2\pi}\brs{\frac{1}{\sqrt{2\pi}}}
%          \brlr{\opLT\brs{\cos(\beta x)e^{-\alpha\abs{x}}}}_{s=i\omega}
%    && \text{by definition Fourier Transform}
%    && \text{\xref{def:opFT}}
%  \\&= \brs{\frac{-2\alpha}{ (s -i\beta)^2 -\alpha^2}}_{s=i\omega}
%    && \text{by \prefp{cor:opLT_cosabs}}
%  \\&= \brs{\frac{-2\alpha}{ (i\omega -i\beta)^2 -\alpha^2}}
%    && \text{by $s=i\omega\in\intoo{-\alpha}{\alpha}$}
%  \\&= \brs{\frac{2\alpha}{(\omega - \beta)^2 + \alpha^2}}
%  \\&= \brs{\frac{2\alpha}
%                 {\omega^2 -2\beta\omega + (\alpha^2 + \beta^2)}}
%\end{align*}
%\end{proof}
...
%    &\eqd \sqrt{2\pi}\brs{\frac{1}{\sqrt{2\pi}}}
%          \brlr{\opLT\brs{\sin(\beta x)e^{-\alpha\abs{x}}}}_{s=i\omega}
%    && \text{by definition Fourier Transform}
%    && \text{\xref{def:opFT}}
%  \\&= \brs{\frac{-2\alpha}{ (s -i\beta)^2 -\alpha^2}}_{s=i\omega}
%    && \text{by \prefp{cor:opLT_cosabs}}
...
%    &\eqd \brs{\sqrt{2\pi}}\frac{1}{\sqrt{2\pi}}\int_{-\infty}^{\infty} \cos(\beta x)e^{-\alpha\abs{\tau}} e^{-i\omega\tau}\dtau
%    && \text{by definition Fourier Transform}
%  \\&= \int_{-\infty}^{\infty} \brs{\frac{e^{i\beta\tau}+e^{-i\beta\tau}}{2}} e^{-\alpha\abs{\tau}} e^{-i\omega\tau}\dtau
%    &&  \text{by \thme{Euler's identity}}&&\text{\xref{thm:eid}}
%  \\&= \frac{1}{2}\int_{-\infty}^{\infty} \brs{e^{-\alpha\abs{\tau}i\beta\tau-i\omega\tau}+e^{-\alpha\abs{\tau}-i\beta\tau-i\omega\tau}} \dtau
%  \\
%  \\
%  \\&= \frac{1}{\sqrt{2\pi}}\brs{\frac{\alpha}{\alpha^2+(\beta-\omega)^2}}
%\end{align*}
