...
Let $\setX$ be a set.
\defboxt{
  The \fnctd{indicator function} $\setind\in\clF{\psetx}{\setn{0,1}}$ is defined as
  \\\indentx$\ds
  \setind_{\setA}(x) =
    \brbl{\begin{array}{lMC}
...
%---------------------------------------
Let $\fstep(x)$ be the \fncte{step function} \xref{def:fstep}.
Let $\bcoef{n}{k}$ be the \fncte{binomial coefficient}\ifsxref{binomial}{def:bcoef}.
...
\begin{align*}
  ...
  \\&= \brs{\bcoef{1}{0}\fstep(x) - \bcoef{1}{1}\fstep(x-1)}
    ...
    && \text{\ifsxref{binomial}{def:bcoef}}
...
\begin{align*}
  ...
  \\&= \sum_{k=0}^{2} (-1)^k \bcoef{2}{k} (x-k) \fstep(x-k)
    && \text{by def. of $\bcoef{n}{k}$\ifsxref{binomial}{def:bcoef}}
  \\&= \brb{\begin{array}{lM}
...
or by using the free and open source software package \hie{Maxima}\ifsxref{src_code}{sec:src_bspline_max}.
\end{proof}
    ...
    \begin{align*}
      ...
      \\&=\mathrlap{
            \frac{1}{(n+1)!}\brp{\begin{array}{@{}r@{}>{\ds}l@{}l|DD}
              ...
              -&                  &(-1)^{n+2}\bcoef{n+1}{n+2}(x-n-2)^{n+1}\fstep(x-n-2)&(D)&$\bcoef{n+1}{n+2}=0$ \ifdochas{binomial}{by \prefp{prop:bcoef}}\\
...
\begin{proof}
\begin{enumerate}
  \item The coefficients for the sequence $\seqX$ may be computed with assistance from \hie{Maxima}\ifsxref{src_code}{sec:src_bspline_max}.
    ...
    \begin{enumerate}
      ...
      \item Proof that $n$ case $\implies$ $n+1$ case:
        \begin{align*}
          ...
          \\&= \int_0^1 \fN_{n-1}(x-\tau)\dtau
          \\&\quad -\int_0^1\fN_{n-1}(x-1-\tau) \dtau
          \\&= \fN_n(x) - \fN_n(x-1)
            && \text{by \prefp{lem:Nn}}
        \end{align*}
    \end{enumerate}
  ...
  \item Proof using \exme{rectangular pulse} example \xref{ex:ft_pulse} and \thme{Convolution Theorem} \xref{thm:conv}:
    \begin{align*}
      ...
      \\&= \frac{1}{\sqrt{2\pi}}\brs{\sqrt{2\pi}
        ...
        &&\text{\begin{tabular}{l}by \exme{rectangular pulse}\\example, $a=0$, $b=c=1$\end{tabular}}
        &&\text{\xref{ex:ft_pulse}}
      \\&= \frac{1}{\sqrt{2\pi}}\brs{e^{-i\brp{\frac{\omega}{2}}}
        \brp{\frac{\sin\brp{\frac{\omega}{2}}}{\brp{\sfrac{\omega}{2}}}}}^{n+1}
      \\&= \frac{1}{\sqrt{2\pi}}e^{-i\brp{\frac{(n+1)\omega}{2}}}
      \\&\quad\cdot\brp{\frac{\sin\brp{\frac{\omega}{2}}}{\brp{\sfrac{\omega}{2}}}}^{n+1}
    \end{align*}
%
...
\begin{proof}
    \begin{enumerate}
      ...
      \item Proof for this theorem follows from the $n=0$ case \xref{ilem:bspline_punity_n0},
            the definition of $\fN_n(x)$ \xref{def:Bspline}, and a corollary regarding partitions of unity\ifsxref{partuni}{cor:pun_pulse}.
...
%---------------------------------------
  ...
  \caption{
    \fncte{auto-power spectrum} $\Swn(\omega)$ plots of \fncte{B-splines} $\fN_n(x)$ \xref{lem:Nriesz}\protect\ifsxref{src_code}{sec:src_Nriesz}.
    \label{fig:Nriesz}
    }
\end{figure}
%---------------------------------------
...
%=======================================
In 1989, St{/'e}phane G. Mallat introduced the \structe{Multiresolution Analysis} (MRA) structure\ifsxref{mra}{def:mra}.
...
\begin{enumerate}
  ...
  \item Induction step\ldots proof that $n$ case $\implies$ $n+1$ case:
    \begin{align*}
      ...
      \\&=\mathrlap{\frac{1}{2^{n+1}}\brs{
                \mcom{\sum_{k=1}^{n+1}\bcoef{n+2}{k} \fN_n(2x-k)}{by \thme{Stifel formual}\ifsxref{binomial}{thm:stifel}}
      ...
      %    \end{array}}
      %  && \text{by \thme{Stifel formula}\ifsxref{binomial}{thm:stifel}}
...
\begin{enumerate}
  ...
  \item The completion of this proof follows directly from
        ...
        \thme{Scaling Function Sufficient Conditions}\ifsxref{mra}{thm:mra_rdc}.
\end{enumerate}
\end{proof}
...
each \fncte{B-spline} also satisfies the \fncte{dilation equation}\ifxref{mra}{thm:dilation_eq} such that
...
by the \fncte{order $n$ B-spline MRA scaling sequence} $\fphi(x)\eqd\fN_n(x)$.
...
\label{thm:Bsplineh}
%--------------------------------------
Let $\mrasys$ be an \structe{MRA system}\ifsxref{mra}{def:mra}.
...
\begin{enumerate}
  \item Proof that (1)$\implies$(3):
        By \prefp{thm:Nmra} we know that $\fN_n(x)$ is a \fncte{scaling function}\ifsxref{mra}{def:mra}.
        \begin{align*}
          \Fh(\omega)
            &= \sqrt{2}\frac{\Fphi(2\omega)}{\Fphi(\omega)}
            && \ifdochas{mra}{\text{by \prefp{lem:Fphi}}}
  ...
  \item Proof that (A)$\implies$(B):
    \begin{enumerate}
      \item lemma: Only $h_0$ and $h_1$ are \prope{non-zero}; All other coefficients $h_k$ are $0$. \label{ilem:N0_h_nonzero} Proof:
                   This follows from $\support\fphi(x)=\intcc{0}{1}$\ifsxref{mra}{def:support} and by a property of MRA systems\ifsxref{mra}{thm:mra_support}.

      \item lemma (equations for $\seqn{h_k}$):\label{ilem:N0_h_equ}
            Because $\seqn{h_k}$ is a \fncte{scaling coefficient sequence}\ifsxref{mra}{def:mra},
            it must satisfy the \prope{admissibility equation}\ifsxref{mra}{thm:admiss}.
            And because $\seqn{\fphi(x-k)}$ forms a \prope{partition of unity}, it must satisfy two important equations\ifsxref{partuni}{thm:pun_zero}.
            \pref{ilem:N0_h_nonzero} and
  ...
  \item Proof that (B)$\implies$(C):
    \begin{align*}
      ...
      \fphi(x)
        ...
        && \text{\ifxref{mra}{thm:dilation_eq}}
