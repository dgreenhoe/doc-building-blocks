...
\begin{align*}
  ...
  \\&= \Swww(\omega) - \Swwq^\ast(\omega) - \Swwq(\omega) + \Swqq(\omega)
    && \text{by \thme{Wiener-Kinchin relationship}}
  \\&= \Swxx(\omega)\abs{\estH(\omega)}^2 - \brs{\estH(\omega)\Swxy(\omega)}^\ast - \estH(\omega)\Swxy(\omega) + \Swqq(\omega)
    && \text{by \prefpp{cor:GHz}}
  \\
  0
    &= \pderiv{}{\estH}\fCost\brp{\estH} 
  \\&= \pderiv{}{\estH}
       \brp{\Swxx\abs{\estH}^2 - \estH\Swyx - \estH^\ast\Swyx^\ast + \Swqq}
  \\&= \pderiv{}{\estH}
       \brp{\Swxx\abs{\estH}^2 - \estH\Swyx - \estH^\ast\Swyx^\ast + \cancelto{0}{\Swqq}}
    && \text{because $\Swqq$ does not vary with $\estH$}
  ...
  \\&=\mathrlap{
     ...
     - \brs{\begin{array}{c}\pderiv{}{\estH_R}\estH_R\Re\Swxy\\\pderiv{}{\estH_I}(-\estH_I)(-\Im\Swxy)\end{array}}
     - \brs{\begin{array}{c}\pderiv{}{\estH_R}\estH_R\Re\Swxy\\\pderiv{}{\estH_I}  \estH_I   \Im\Swxy \end{array}}
     }
