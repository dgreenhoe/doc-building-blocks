...
For a \fncte{random variable} $\rvX$ on a \structe{probability space} $\ps$,
The $n$th \fncte{moment} and \fncte{central moments}, \fncte{mean}, \fncte{variance}, \fncte{skewness}, and \fncte{kurtosis}
are defined in \pref{def:cmom} and \prefpp{def:mean}.
...
be the \fncte{arithmetic mean} estimate \xref{def:am} of a sequence $\seqn{\rvx_n}$ of samples drawn from a
random variable $\rvX$. The \prope{quality} \xref{sec:quality} of this estimator is described as follows:
\thmbox{%
  \brb{\begin{array}{FMD}
      (A).& $\seqn{\rvx_n}$ is \prope{wide sense stationary} & and
    \\(B).& $\mean\eqd\pE\rvx_n$                             & and
    \\(C).& $\seqn{\rvx_n}$ is \prope{uncorrelated}          &
  \end{array}}
  \implies
...
%---------------------------------------
Let $\ds\meanest\eqd\frac{1}{\xN}\sum_{n=1}^\xN$ be the \fncte{average} \xref{def:average} of a random variable $\rvX$.
The \prope{quality} \xref{sec:quality} of this estimator is described as follows:
\corbox{
  \brb{\begin{array}{FMD}
      (A).& $\seqn{\rvx_n}$ is \prope{wide sense stationary} & and
    \\(B).& $\mean\eqd\pE\rvx_n$                             & and
...
\footnote{
  ...
  \citerpc{bajpai1967}{525}{Chapter XXVI Analysis of Variance}
  }
\label{def:varest}
%---------------------------------------
...
\begin{corollary}
\footnote{
  \citerpc{welford1962}{419}{(1)},
  \citerpc{simon2006}{90}{(3.53), (3.54)},
  \citerpc{pebay2008}{7}{(1.1)},
...
%---------------------------------------
\begin{example}
\label{ex:average_recursive}
%---------------------------------------
Let $X \eqd \seqn{1,2,3,4,5}$ be a finite sequence of numbers.
The \fncte{average} \xref{def:average} of $X$ is
\\\indentx$\ds
  \boxed{\meanest} \eqd \frac{1}{\xN}\sum_{n=1}^\xN x_n
                   ...
                   =    \boxed{3}
$.\\
Using the recursive method of \prefpp{cor:average_recursive} with $\meanest_n \eqd 0$ for all $n\le0$, yields
\exbox{\begin{array}{l|rclcl clcl cl}
...
\footnote{
  \citerpc{welford1962}{420}{(III) todo: make sure agrees}
  }
\label{thm:cmomest_recursive}
%---------------------------------------
\thmbox{
  \cmomest_{p,n}
    = \frac{1}{n} \sum_{k=1}^n \bcoef{p}{k}
      \brs{ -\frac{1}{n}\brp{\rvx_n-\meanest_{n-1}} }^k
      \brs{ (n-1) \cmomest_{p-k,n-1} + \brp{ \rvx_n - \meanest_{n-1} }^{p-k} }
  }
\end{theorem}
\begin{proof}
\begin{align*}
...
\footnote{
  \citerpc{welford1962}{420}{(I) where $\cmomest_{2,n}\eqd \frac{1}{n}S_n$},
  }
\label{thm:cmom2_recursive}
%---------------------------------------
\thmbox{
  \cmomest_{2,n} = \frac{n-1}{n}\mcom{\cmomest_{2,n-1}}{previous}
                 + \mcom{\brs{\frac{n-1}{n^2}}}{weight}
                   \mcom{\brp{ \rvx_n - \cmomest_{n-1} }^2}{square error}
  }
\end{theorem}
\begin{proof}
\begin{align*}
...
%---------------------------------------
\begin{example}
\label{ex:cmom2_recursive}
%---------------------------------------
Let $X \eqd \seqn{1,2,3,4,5}$ be a finite sequence of numbers.
The \fncte{second central moment estimate} of $X$ is
\begin{align*}
  \boxed{\cmomest_2} &\eqd \frac{1}{\xN}\sum_{n=1}^\xN \brp{x_n -\meanest}^2
                  ...
                  \\&=    \frac{1}{5}\brs{ 4 + 1 + 0 + 1 + 4 }
                     ...
                     =    \boxed{2}
\end{align*}
Using the recursive method of \prefpp{thm:cmom2_recursive} yields
\exbox{\begin{array}{l|rclcl clcl clcl cl}
...
%---------------------------------------
\corbox{\begin{array}{r c>{\ds}l M}
  \varest_n    %&=& \mcom{\brs{\frac{n}{n-1}}}{\fncte{Bessel's correction}}\cmomest_{2,n}
               &=& \mcom{ \brs{\frac{n-2}{n-1}} \varest_{n-1}            }{weighted previous}
                 + \mcom{      \frac{1}{n}      \brp{ \rvx_n - \meanest_{n-1} }^2 }{weighted squared error}
                 & (\fncte{sample variance estimation})
                \\
  \popvarest_n %&=& \cmomest_{2,n}
               &=& \mcom{ \brs{\frac{n-1}{n}}  \popvarest_{n-1}            }{weighted previous}
...
%---------------------------------------
Let $X \eqd \seqn{1,2,3,4,5}$ be a finite sequence of numbers.
The two standard \fncte{variance} estimates of $X$ are
\begin{align*}
  ...
  \boxed{\varest} &\eqd \frac{1}{\xN-1}\sum_{n=1}^\xN \brp{x_n -\meanest}^2
                     ...
                     =    \boxed{2.5}
\end{align*}
Using the recursive method of \prefpp{cor:var_recursive} along with results from
\prefpp{ex:average_recursive} and \prefpp{ex:cmom2_recursive} yields
\exbox{\begin{array}{ rc>{\ds}lclM}
  \popvarest_5 &=& \cmomest_{2,5}                     &=& 2    & (\fncte{population variance})\\
  \varest_5    &=& \brs{\frac{5}{5-1}} \cmomest_{2,5} &=& 2.5  & (\fncte{sample variance})
\end{array}}
\end{example}
