...
\begin{figure}[h]
  ...
  \begin{tabular}{|c|c|}
    \hline
     \tbox{\includegraphics{graphics/opT_estG_estH_cnoise.pdf}}
    ...
    \\    \xref{thm:estHls_A}                            &    \xref{thm:estHls_B} %\xref{cor:H1LSb}
    \\\hline
  \end{tabular}
...
\pref{thm:estHls} then demonstrates optimal solutions with respect to these definitions.
In particular, the theorem demonstrates
the optimal \prope{LTI} \prope{least-squares} estimates of $\opT$ are
\\\indentx
...
\thmbox{
  ...
  \brbl{\begin{array}{F>{\ds}r>{\ds}c>{\ds}l}
      (1).&\argmin_{\estG}\fCostrq(\estG) &=& \frac{\Swyx(\omega)+\Swyu(\omega)}{\Swxx(\omega)+\Swuu(\omega)}
    \\(2).&\argmin_{\estH}\fCostsy(\estH) &=& \frac{\Swyx(\omega)+\Swuy(\omega)}{\Swxx(\omega)}
    \\
  \end{array}}
  }
\end{theorem}
...
\begin{enumerate}
  ...
  \item lemma (expanded forms for cost functions): \label{ilem:H1LS_cost}
    {\begin{align*}
      ...
      \\&= \boxed{
         ...
         - \estH(\omega)\brs{\Swxy(\omega)+\Swuy(\omega)}
         - \estH^\ast(\omega)\brs{\Swxy(\omega)+\Swuy(\omega)}^\ast
         + \Swyy(\omega)
         }
        &&\text{by (A)--(D) and}
        &&\text{\prefpp{cor:GHz}}
  ...
  \item lemma: \label{ilem:G1LS_R}
    \begin{align*}
      ...
      \\&= 2\estG_R\Swpp - 2\Real\Swyx - 2\Real\Swyu
        && \text{by \prefp{thm:opT_cnoise}}
      \\&\implies \boxed{\estG_R(\omega) = \frac{\Real\Swyx(\omega)+\Real\Swyu(\omega)}{\Swpp(\omega)}}
    \end{align*}
  ...
  \item lemma: \label{ilem:G1LS_I}
    \begin{align*}
      ...
      \\&= 2\estG_I\Swpp + 2i(i\Imag\Swyp)
        && \text{by \prefp{cor:Swxy_sym}}
      \\&= 2\estG_I\Swpp + 2i(i\Imag\Swyx+i\Imag\Swyu)
        && \text{by \prefp{thm:opT_mnoise}}
      \\&= 2\estG_I\Swpp - 2\Imag(\Swyx+\Swyu)
      \\&\implies \boxed{\estG_I(\omega) = \frac{\Imag\Swyx(\omega)+\Imag\Swyu(\omega)}{\Swpp(\omega)}}
     \end{align*}
  ...
  \item lemma: \label{ilem:H1LS_I}
    \begin{align*}
      ...
      \\&= \pderiv{}{\estH_I}
           \brp{\Swxx\abs{\estH}^2 - \estH(\Swxy+\Swuy) - \estH^\ast(\Swxy^\ast+\Swuy^\ast) + \Swyy}
        && \text{by \prefp{ilem:H1LS_cost}}
      \\&= \mathrlap{
           \pderiv{}{\estH_I}\brs{
           \Swxx\brs{\estH_R^2+\estH_I^2}
         - (\estH_R+i\estH_I)(\Swxy+\Swuy)
         - (\estH_R-i\estH_I)(\Swxy+\Swuy)^\ast
         + \Swyy}
         }
      \\&= 2\estH_I\Swxx - i(\Swxy+\Swuy) + i(\Swxy+\Swuy)^\ast + \cancelto{0}{\pderiv{}{\estH_R}\Swyy}
        && \text{because $\rvq$ does not vary with $\estH$}
      \\&= 2\estH_I\Swxx - 2i(i\Imag\Swxy) - 2i(i\Imag\Swuy)
      \\&= 2\estH_I\Swxx + 2i(i\Imag\Swyx) - 2i(i\Imag\Swyu)
        && \text{by \prefp{cor:Swxy_sym}}
      \\&= 2\estH_I\Swxx - 2\Imag\Swyx - 2\Imag\Swyu
      \\&\implies \boxed{\estH_I(\omega) = \frac{\Imag\Swyx(\omega)+\Imag\Swyu(\omega)}{\Swxx(\omega)}}
     \end{align*}
  ...
  \item Proof for $\estG\eqd\estGls$ and $\estH\eqd\estHls$ expressions:
    \begin{align*}
      ...
      \\&= \frac{\Swyx(\omega)}{\Swpp(\omega)}
        && \text{by \prefp{thm:opT_mnoise}}
      \\&= \boxed{\frac{\Swyx(\omega)}{\Swxx(\omega)-\Swuu(\omega)}}
      ...
      \\&= \frac{ \Real\Swyx(\omega) +\Real\Swyu(\omega)}{\Swxx(\omega)}
         + \frac{i\Imag\Swyx(\omega)+i\Imag\Swyu(\omega)}{\Swxx(\omega)}
        && \text{by \prefp{thm:opT_mnoise}}
      \\&= \boxed{\frac{\Swyx(\omega)+\Swyu(\omega)}{\Swxx(\omega)}}
    \end{align*}
\end{enumerate}
\end{proof}
...
\thmbox{
  ...
  \implies
  \brbl{\begin{array}{F>{\ds}r>{\ds}c>{\ds}l}
      (1).&\argmin_{\estG}\fCostrq(\estG) &=& \frac{\Swyx(\omega)}{\Swxx(\omega)-\Swuu(\omega)}
...
\begin{enumerate}
  ...
  \item lemma: \label{ilem:G1LS_I}
    \begin{align*}
      ...
      \\&= 2\estG_I\Swpp + 2i(i\Imag\Swyp)
        && \text{by \prefp{cor:Swxy_sym}}
      \\&= 2\estG_I\Swpp + 2i(i\Imag\Swyx)
        && \text{by \prefp{thm:opT_mnoise}}
      \\&= 2\estG_I\Swpp - 2\Imag\Swyx
...
It follows immediately from \prefpp{thm:estHls_B} that, in the special case
...
\corbox{
  \brb{\begin{array}{FMD}
      (1). & hypotheses of \pref{thm:estHls_B} & and
    \\(2). & $\rvu(n)=0$
  \end{array}}
  \implies
...
Let $\opS$ be the \structe{system} illustrated in \prefpp{fig:addnoise_LTI}.
\thmboxt{
  $\estHkp(\omega)$ \xref{def:Hkp} is the \propb{optimal least squares estimate} if
  \\\indentx$\fCost(\estHls)=\Swuu$ and
  \\\indentx$\Swvv(\omega)=\kappa(\omega)\Swuu(\omega)$
  }
\end{theorem}
\begin{proof}
\begin{enumerate}
  \item
    \begin{align*}
      ...
      %Y-V &= Q = P = H(X-U)
      %\\HU-V &= HX-Y
      %\\\pE\brp{\brs{HU-V}\brs{HU-V}^\ast} &= \pE\brp{\brs{HX-Y}\brs{HX-Y}^\ast}
      %\\
      %\\\implies \abs{\estH}^2\Swuu - \estH\cancelto{}{\Swuv} - \estH^\ast\cancelto{}{\Swvu} + \Swvv
      ...
      \\&\mathrlap{\implies
        \boxed{
          \Swuu(\omega) = \frac{\abs{\estH(\omega)}^2\Swxx(\omega) - \estH(\omega)\Swxy(\omega) - \estH^\ast(\omega)\Swxy^\ast(\omega) + \Swyy(\omega)}
                ...
                \eqd\frac{F(\omega)}{G(\omega)}}}
    \end{align*}

  \item
    \begin{align*}
      0 &= \frac{1}{2}G^20 = \frac{1}{2}G^2\pderiv{}{\estH_R}\Swuu
      \\&= \frac{1}{2}G^2\frac{(F'G-G'F)}{G^2}
        && \text{by \thme{Quotient Rule}}
      \\&= \frac{1}{2}(F'G-G'F)
      \\&= \frac{1}{2}\brs{2\estH_R\Swxx - \Swxy - \Swxy^\ast}G - \frac{1}{2}2\estH_RF
      \\&= \estH_R\Swxx G - G\Real\Swxy - \estH_RF
      \\
      0 &= \frac{1}{2}G^20 = \frac{1}{2}G^2\pderiv{}{\estH_I}\Swuu
      \\&= \frac{1}{2}G^2\frac{(F'G-G'F)}{G^2}
        && \text{by \thme{Quotient Rule}}
      \\&= \frac{1}{2}(F'G-G'F)
      \\&= \frac{1}{2}\brs{2\estH_I\Swxx - i\Swxy + i\Swxy^\ast}G - \frac{1}{2}2\estH_IF
      \\&= \estH_I\Swxx G + G\Imag\Swxy - \estH_IF
      \\
      0
        &= 0 + i0
      \\&= \brs{\estH_R\Swxx G - G\Real\Swxy - \estH_RF}
         +i\brs{\estH_I\Swxx G + G\Imag\Swxy - \estH_IF}
      \\&= \estH\Swxx G - \Swyx G - \estH F
