...
All the other members of this family ($\estH_1$, $\estH_2$, $\estHv$, $\estHs$) are \prope{complex-valued},
...
\remboxt{
  $\Cxy \eqd \sqrt{\frac{\estH_1^\ast}{\estH_2}}$ whereas $\Twxy' \eqd \sqrt{\estH_1 \estH_2}$
  }
\end{remark}
\begin{proof}
\begin{align*}
...
%---------------------------------------
\begin{corollary}[Estimating H in communication additive noise system]
\label{cor:estH1}
\label{cor:estH2}
%---------------------------------------
Let $\opS$ be the \structe{system} illustrated in \prefpp{fig:addnoise} (A).
\corboxt{
  ...
  \brb{\begin{array}{F >{\ds}rc>{\ds}l D}
    %\mc{3}{M}{\scs\prope{biased}:}
      ...
      (1).&\estH_1(\omega) &=& \FH(\omega) &and
    \\(2).&\estH_2(\omega) &=& \frac{\Swvv(\omega)}{\FH^\ast(\omega)\Swxx(\omega)} 
...
\begin{align*}
  ...
  \estH_2(\omega)
    ...
    && \text{\xref{def:H2}}
  \\&= \frac{\Swyy(\omega)}{\FH(\omega)\Swxx(\omega)}
    && \text{by \prefp{thm:sysH_addnoise_lti}}
  \\&= \frac{\FH(\omega)\FH^\ast(\omega)}{\FH(\omega)}
  \\&= \FH(\omega)
\end{align*}
\end{proof}
...
\label{thm:estH2}
%---------------------------------------
Let $\opS$ be the \structe{system} illustrated in \prefpp{fig:addnoise} (B).
\thmboxt{
  $\brb{\begin{array}{FMMD}
      (A).& $\fH$ is                                &\prope{linear time invariant} & and
    \\(B).& $\rvx(n)$ is                            &\prope{wide-sense stationary} & and
  ...
  \brb{\begin{array}{F >{\ds}rc>{\ds}l DD}
    \\(1).&\estH_1(\omega) &=& \FH(\omega)\brs{\text{$\frac{1}{1+\frac{\ds\Swww(\omega)}{\ds\Swpp(\omega)}}$}}
...
\corbox{\begin{array}{lcl}
  \brb{\begin{array}{FMM}
      (A).& \mc{2}{M}{hypotheses of \pref{thm:estH1} \scs and}
