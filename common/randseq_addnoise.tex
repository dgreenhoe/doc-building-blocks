...
\begin{figure}[h]
  ...
  \begin{tabular}{|l|l|l|}
    ...
    \\(A) $\Rxy(m)=\Rxq(m)$         &(B) $\Rpq(m)=\Rxy(m)+\Ruy(m)$     &(C) $\Rxy(m)=\Rpy(m)$
    \\\xref{lem:sysT_addnoise_v}&    \xref{thm:sysT_addnoise_u}    &    \xref{thm:sysT_addnoise_w}
    \\\hline
  \end{tabular}
...
\begin{figure}[h]
  ...
  \begin{tabular}{|c|c|}
   ...
   \\\xref{thm:opT_cnoise}&\xref{thm:opT_mnoise}
   \\\hline
  \end{tabular}
\caption{\label{fig:opT_addnoise}}
\end{figure}
...
\begin{align*}
  \Rpq(m)
    &= \Rpy(m)
    && \text{by \prefp{lem:sysT_addnoise_v}}
  \\
  \Rpy(m)
