...
Despite the $\pE(a\rvX+b)=a\pE\rvX+b$ hope that \pref{thm:pE} may have given, 
note that in general the expectation operator $\pE$ is \prope{non-linear}.
In particular, in general it is \emph{not} true that $\pE(\rvX+\rvY)$
equals $\pE\rvX + \pE\rvY$.
However, in the special case that $\rvX$ and $\rvY$ are \prope{independent},
then this \emph{is} true (next theorem).
%---------------------------------------
\begin{theorem}
\label{thm:pE}
%---------------------------------------
...
\thmbox{
  \brb{\begin{array}{M}
    $\rvX$ and $\rvY$ are \prope{independent}
  \end{array}}
  \implies
  \brb{\begin{array}{>{\ds}rc>{\ds}lCD}
    \pEx(a\rvX+b\rvY)  &=& \brp{a\pEx\rvX} + \brp{b\pEx\rvY} & \forall a,b\in\R & (\prope{linear})
  \end{array}}
  }
\end{theorem}
\begin{proof}
\begin{align*}
  \pExy(a\rvX+b\rvY)
    &\eqd \int_{x\in\R}\int_{y\in\R}\brs{ax+by} \pdfxy(x,y)  \dy\dx
    && \text{by definition of $\pE$}
    && \text{\xref{def:pE}}
  \\&= \int_{x\in\R}\int_{y\in\R} ax \pdfxy(x,y)  \dy\dx
     + \int_{x\in\R}\int_{y\in\R} by \pdfxy(x,y)  \dy\dx
     && \text{by \prope{lineararity} of $\int\int\dy\dx$ operator}
     && \text{\xref{def:linop}}
  \\&= \int_{x\in\R}\int_{y\in\R} ax \pdfx(x)\pdfy(y)  \dy\dx
     + \int_{x\in\R}\int_{y\in\R} by \pdfx(x)\pdfy(y)  \dy\dx
     && \text{by \prope{independent} property}
  \\&= a \int_{x\in\R} x \pdfx(x)\dx \int_{y\in\R} \pdfy(y)  \dy
     + \int_{x\in\R} \pdfx(x) b\int_{y\in\R} y \pdfy(y)  \dy
  \\&= \brp{a\pEx\rvX} + \brp{b\pEy\rvY}
\end{align*}
\end{proof}
