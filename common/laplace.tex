...
\thmbox{\begin{array}{>{\ds}r c >{\ds}rc>{\ds}l}
  \brb{\lim_{x\to-\infty}\fg(x)=0} &\implies&
     \brb{\opLT \brs{\ddt \fg(x)}} &=& s \brs{\opLT\fg}(s)
    \\&&
...
\begin{proof}
  \begin{align*}
    \opLT \brs{\ddx \fg(x)}
      ...
      && \text{by definition of $\opLT$}
    \\&= \brlr{\mcom{e^{-sx}}{$u$} \mcom{\fg(x)}{$v$}}_{x=-\infty}^{x=+\infty}
        -\int_{x\in\R} \mcom{\fg(x)}{$v$} \mcom{(-s)e^{-s x}}{$\du$} \dx
      && \text{by \thme{Integration by Parts}}
    \\&= \cancelto{0}{e^{-s \infty}}\fg(\infty) - e^{s \infty}\cancelto{0}{\fg(-\infty)}
