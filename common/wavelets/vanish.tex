...
\thmboxt{
  ...
  $\brb{\begin{array}{>{\ds}rc >{\ds}rcl@{\qquad}D}
       ...
       \\&\iff&         \sum_{k\in\Z} (-1)^k k^n  \fh_k                    &=& 0 & (4)
...
\end{theorem}
\begin{proof}
This follows from the \thme{Strang-Fix condition} \xxref{lem:sfixc}{ex:sfixc5}.
\end{proof}
...
%=======================================
%--------------------------------------
\begin{example}[\exmd{3 coefficients and 2 vanishing moments}]
%\label{ex:pun_n=3}
%--------------------------------------
...
\exbox{
  \brb{\begin{array}{FMMD}
    ...
    \\     & \quad\prope{vanishing 0th moment}   &                         & and
    \\(4). & \prope{vanishing 1st moment}        & \xref{thm:gh_cqf}       &
  \end{array}}
  \implies
  \brb{\begin{array}{c|r}
      n            & \fh_{n}
...
\\\indentx\tbox{\includegraphics{../common/math/graphics/pdfs/n1_pz.pdf}}
     \quad\tbox{\includegraphics{../common/math/graphics/pdfs/n1_h.pdf}}
     \quad\tbox{\includegraphics{../common/math/graphics/pdfs/b1_g.pdf}}
\\The resulting scaling function is the \fncte{$1$st order B-spline} \xref{ex:bspline_N1}.
\end{example}
\begin{proof}
\begin{enumerate}
  \item Proof that (1) $\implies$ that only $\fh_0$, $\fh_1$, and $\fh_2$ are non-zero: by \prefp{thm:mra_support}.
  ...
  \item Note: Using the sufficient condition of placing two zeros at $z=-1$ \xref{lem:van_factor} yields the same result\ldots
    \begin{align*}
      \Zh(z)
        &= \sqrt{2} \brs{\frac{ \brp{z+1}^2 }
                              { \brp{1+1}^2 z^2 }}
         = \frac{\sqrt{2}}{4} \brs{\frac{ z^2 + 2z + 1 }
                                        { z^2          }}
         = \frac{\sqrt{2}}{4} \brs{ 1 + 2z^{-1} + z^{-2} }
    \end{align*}
\end{enumerate}
\end{proof}

%--------------------------------------
\begin{example}[\exmd{4 coefficients and 3 vanishing moments}]
%\label{ex:pun_n=3}
%--------------------------------------
Let $\wavsys$ be a \structe{wavelet system}.
\exbox{
  \brb{\begin{array}{FMMD}
    ...
    \\(4). & \prope{vanishing 1st moment}        & \xref{thm:gh_cqf}       & and
    \\(5). & \prope{vanishing 2nd moment}        & \xref{thm:gh_cqf}       &
  \end{array}}
  \implies
  \brb{\begin{array}{c|r}
      n            & \fh_{n}
    \\\hline
      0            & \frac{\sqrt{2}}{8}\cdot1
    \\1            & \frac{\sqrt{2}}{8}\cdot3
    \\2            & \frac{\sqrt{2}}{8}\cdot3
    \\3            & \frac{\sqrt{2}}{8}\cdot1
    \\\text{other} & 0
  \end{array}}
  }
\\\tbox{\includegraphics{../common/math/graphics/pdfs/n2_pz.pdf}}
  \tbox{\includegraphics{../common/math/graphics/pdfs/n2_h.pdf}}
  \tbox{\includegraphics{../common/math/graphics/pdfs/b2_g.pdf}}
\\The resulting scaling function is the \fncte{$2$nd order B-spline} \xref{ex:bspline_N2}.
\end{example}
\begin{proof}
\begin{enumerate}
  \item Proof that (1) $\implies$ that only $\fh_0$, $\fh_1$, $\fh_2$, and $\fh_3$ are non-zero: by \prefp{thm:mra_support}.
  \item Proof solving a system of equations:
    \begin{enumerate}
      \item System of Equations:
            \\\indentx$\ds\begin{array}{rcl MM}
  ...
  \item Note: Using the sufficient condition of placing three zeros at $z=-1$ \xref{lem:van_factor} yields the same result\ldots
    \begin{align*}
      \Zh(z)
...
%--------------------------------------
\begin{example}[\exmd{5 coefficients and 4 vanishing moments}]
%\footnote{
%  \citerp{strang89}{616}
%  }
%\label{ex:pun_n=3}
...
\exbox{
  \brb{\begin{array}{FMMD}
      (1). & $\support\fphi(x)=\intcc{0}{4}$     & \xref{thm:mra_support}  & and
    \\(2). & \prope{admissibility condition}     & \xref{thm:admiss}       & and
    \\(3). & \prope{partition of unity}          & \xref{thm:pun_zero}     & and
    \\(4). & \prope{vanishing 1st moment}        & \xref{thm:gh_cqf}       & and
    \\(5). & \prope{vanishing 2nd moment}        & \xref{thm:gh_cqf}       & and
    \\(6). & \prope{vanishing 3rd moment}        & \xref{thm:gh_cqf}       &
  \end{array}}
  \implies
  \brb{\begin{array}{c|r}
      n            & \fh_{n}
    \\\hline
      0            & \frac{\sqrt{2}}{16}\cdot1
    \\1            & \frac{\sqrt{2}}{16}\cdot4
    \\2            & \frac{\sqrt{2}}{16}\cdot6
    \\3            & \frac{\sqrt{2}}{16}\cdot4
    \\4            & \frac{\sqrt{2}}{16}\cdot1
    \\\text{other} & 0
  \end{array}}
  }
...
\\\indentx\tbox{\includegraphics{../common/math/graphics/pdfs/n3_h.pdf}}
  \indentx\tbox{\includegraphics{../common/math/graphics/pdfs/b3_g.pdf}}
\\The resulting scaling function is the \fncte{$3$rd order B-spline} \xref{ex:bspline_N3}.
\end{example}
\begin{proof}
\begin{enumerate}
  \item Proof that (1) $\implies$ that only $\fh_0$, $\fh_1$, $\cdots$, $\fh_4$ are non-zero: by \prefp{thm:mra_support}.
  \item Proof solving a system of equations:
    \begin{enumerate}
      \item System of Equations:
            \\\indentx$\ds\begin{array}{rcl MM}
  ...
  \item Note: Using the sufficient condition of placing four zeros at $z=-1$ \xref{lem:van_factor} yields the same result\ldots
    \begin{align*}
      \Zh(z)
...
In particular, note that for all of them $\sum_{n\in\Z} (-1)^n\fh_n = 0$ \xref{thm:pun_zero}.

Other examples of wavelets systems with $p$ vanishing moments the Daubechies-$p$ \xref{sec:examples_Dp} wavelet systems 
and Symlets \xref{sec:symlet_examples}.
...
%                 &\eqd& \sum_{n\in\Z} \fh_n \fkernea{n}{\omega}
%                 &      n \in\Z
%     \\
...
%   \\&= \inprod{\sum_{n\in\Z}  \fh_n  \opT^n\opD\vphi}{\fkern{x}{\omega}}
%     && \text{by dilation equation \prefpo{thm:dilation_eq}}
%   \\&= \sum_{n\in\Z}  \fh_n \inprod{\opT^n\opD\vphi}{\fkern{x}{\omega}}
%     && \text{by definition of $\inprod{\cdot}{\cdot}$ \prefpo{def:inprod}}
%   \\&= \sum_{n\in\Z}  \fh_n \inprod{\vphi}{\left(\opT^n\opD\right)^\ast \fkern{x}{\omega}}
%     && \text{by definition of operator adjoint \prefpo{def:op_adj}}
%   \\&= \sum_{n\in\Z}  \fh_n \inprod{\vphi}{\cwt \fkern{n}{\frac{1}{2}\omega}\fkern{x}{\frac{1}{2}\omega}}
...
%     essentially same as above but substitute $g_n$ for $\fh_n$.
%     }
%\\
