...
Let $\Rxx(m)$ be the \fncte{autocorrelation} \xref{def:Rxxm} of $\rvx(n)$.
\defboxt{
  $\rvx(n)$ is \propd{white} if \quad
  $\ds\Rxx(m)=K\kdelta(m)$\quad for some $K>0$.
  }
\end{definition}
...
the innerproducts \textbf{are} \prope{uncorrelated} even though $\rvx(n)$ is not white.
