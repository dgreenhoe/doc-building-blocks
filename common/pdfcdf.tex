...
random processes.
Before what a \fncte{random variable} \emph{is}, note two things that a \fncte{random variable} is \emph{not} (next remark).
%---------------------------------------
\begin{remark}
\citetbl{
...
%
%A \fncte{random variable} is actually a \hie{function} $\rvX(\omega)$, where
%$\omega\in\pso$ is an outcome of an experiment.
%More specifically, in a \structe{probability space} $\ps$,
%a \fncte{random variable} is a mapping from the set $\pso$ of outcomes to the set of real numbers.
...
\hie{cummulative distribution function} of the \structe{probability space}.
%---------------------------------------
\begin{definition}
\citetbl{
...
$\rvX:\pso\to\R$ a \fncte{random variable}.
\defbox{\begin{array}{MMrc>{\ds}l}
  A \fnctd{probability density function} 
  & (\fnctd{pdf}) $\ppx$ is defined as
  & \ppx(x) &\eqd& \lim_{\epsilon\to 0} \frac{1}{\epsilon}\psp\setn{x \le \rvX < x+\epsilon}
  \\
  A \fnctd{cummulative distribution function} 
  & (\fnctd{cdf}) $\pcx$ is defined as
  & \pcx(x) &\eqd& \psp\setn{\rvX < x }
\end{array}}
\end{definition}
...
$\rvX:\pso\to\R$ and $\rvY:\pso\to\R$ random variables. Then
...
\defbox{\begin{array}{rc>{\ds}lD}
  \pcxy(x,y) &\eqd& \pPa{\rvX\le x}{\rvY\le y}
          & (\fncte{joint cumulative distribution function})
          \\
  \ppxy(x,y) &\eqd& \ddy\ddx\pcxy(x,y)
          & (\fncte{joint probability density function})
  %\ppxy(x,y) &\eqd& \lim_{\varepsilon\to 0} \frac{1}{\varepsilon}\psp\brb{x \le\rvX < x+\varepsilon \land y \le\rvY < y+\varepsilon}
\end{array}}
\end{definition}
...
$\rvX:\pso\to\R$ be a \fncte{random variable}, and $\intoo{a}{b}$ a \structe{real interval}.
...
\begin{align*}
  ...
  %  &&    \text{by definition of $\ppx[x|y](x|y)$}
  %  &&    \text{\xref{def:conpdf}}
  %\\&\eqd \lim_{\varepsilon\to0} \frac{1}{\varepsilon} \frac{\pP{(x\le\rvX < x+\varepsilon) \land (Y=y)}}{\pP{Y=y}}
  %  &&    \text{by definition of $\pP{\rvX|\rvY}$}
  %  &&    \text{\xref{def:conP}}
  %\\&=    \lim_{\varepsilon\to0} \frac{1}{\varepsilon} \frac{\pP{(x\le\rvX < x+\varepsilon) \land (y\le Y<y+\varepsilon)}}{\pP{y\le Y<y+\varepsilon}}
  %\\&\eqd \frac{\ppxy(x,y)}{\ppy(y)}
  %  \qquad\mathrlap{\text{by definitions of $\ppxy$ \xref{def:jointpdf} and $\ppy$ \xref{def:pdf}}}
  %\\
  %\int_y\ppxy(x,y)\dy
  %  &\eqd \lim_{\varepsilon\to0} \frac{1}{\varepsilon} \int_y \pP{x\le X<x+\varepsilon \land y\le Y<y+\varepsilon} \dy
  %  &&    \text{by definition of $\ppxy$}
  %  &&    \text{\xref{def:jointpdf}}
  %\\&=    \lim_{\varepsilon\to0} \frac{1}{\varepsilon} \pP{x\le X<x+\varepsilon}
...
\begin{enumerate}
  ...
  %  \begin{align*}
  %    &\ppy(y)
  %    \\&\eqd \lim_{\varepsilon\to0}\frac{1}{\varepsilon}\pP{y \le Y < y+\varepsilon}
  %      && \text{by def. of $\ppy$ \xref{def:pdf}}
  %    \\&= \lim_{\varepsilon\to0}\frac{1}{\varepsilon}\pP{y \le \ff(\rvX)< y+\varepsilon}
  %      && \text{by hypothesis}
  %    \\&= \lim_{\varepsilon\to0}\frac{1}{\varepsilon}\sum_{n=1}^{\xN+1} \pPa{y \le \ff(\rvX)< y+\varepsilon}{\rvX\in\setA_n}
  %      && \text{by \thme{sum of products} \xref{thm:psp_sop}}
  %    \\&= \lim_{\varepsilon\to0}\frac{1}{\varepsilon}\sum_{n=1}^{\xN+1} \pPc{y \le \ff(\rvX)< y+\varepsilon}{\rvX\in\setA_n}\pP{\rvX\in\setA_n}
  %      && \text{by def. of $\pP{\setX|\setY}$ \xref{def:conP}}
  %    \\&= \lim_{\varepsilon\to0}\frac{1}{\varepsilon}\sum_{n=1}^{\xN+1} \pPc{y \le \fg_n(\rvX)< y+\varepsilon}{\rvX\in\setA_n}\pP{\rvX\in\setA_n}
  %      && \text{by def. of $\fg_n(x)$}
  %    \\&= \mathrlap{\lim_{\varepsilon\to0}\frac{1}{\varepsilon}\brs{\begin{array}{>{\ds}lM}
  %          \sum_{n=1}^{\xN+1} \pPc{\fgi_n(y) \le\rvX < \fgi_n(y+\varepsilon)}{\rvX\in\setA_n}\pP{\rvX\in\setA_n} & for $\ffp(x)\ge0$ \\
  %          \sum_{n=1}^{\xN+1} \pPc{\fgi_n(y+\varepsilon) \le\rvX < \fgi_n(y)}{\rvX\in\setA_n}\pP{\rvX\in\setA_n} & otherwise
  %         \end{array}}\quad\text{(by \pref{item:YfX_gn}}}
  %    \\&= \mathrlap{\lim_{\varepsilon\to0}\frac{1}{\varepsilon}\brs{\begin{array}{>{\ds}lM}
  %           \sum_{n=1}^{\xN+1} \pPc{\fgi_n(y)     \le\rvX < \fgi_n(y) + \frac{\varepsilon}{\ffp\brs{\fgi_n(y)}}}{\rvX\in\setA_n}\pP{\rvX\in\setA_n} & for $\ffp(x)\ge0$ \\
  %           \sum_{n=1}^{\xN+1} \pPc{\fgi_n(y) + \frac{\varepsilon}{\ffp\brs{\fgi_n(y)}} \le\rvX < \fg_n(y)}{\rvX\in\setA_n}\pP{\rvX\in\setA_n}     & otherwise
  %         \end{array}}\quad\text{by \pref{ilem:YfX_ffp}}}
  %    \\&= \mathrlap{\lim_{\varepsilon\to0}\frac{1}{\varepsilon}\sum_{n=1}^{\xN+1} \pPc{\fgi_n(y) \le\rvX < \fgi_n(y) + \frac{\varepsilon}{\abs{\ffp\brp{\fgi_n(y)}}}}{\rvX\in\setA_n}\pP{\rvX\in\setA_n}}
  %    \\&= \lim_{\varepsilon\to0}\frac{1}{\varepsilon}\sum_{n=1}^{\xN+1} \pPa{\fgi_n(y) \le\rvX < \fgi_n(y) + \frac{\varepsilon}{\abs{\ffp\brp{\fgi_n(y)}}}}{\rvX\in\setA_n}
  %      && \text{by def. of $\pP{\setX|\setY}$ \xref{def:conP}}
  %    \\&= \lim_{\varepsilon\to0}\frac{1}{\varepsilon}\sum_{n=1}^{\xN+1} \pP{\fgi_n(y) \le\rvX < \fgi_n(y) + \frac{\varepsilon}{\abs{\ffp\brp{\fgi_n(y)}}}}
...
%  \item This follows from \prefpp{thm:YfX}: 
  ...
  %    \\&=  \left\{\begin{array}{ll}
  %            \pP{\frac{y-b}{a}\le\rvX < \frac{y-b}{a}+\frac{1}{a}h} &\forall a>0 \\
  %            \pP{\frac{y-b}{a}\ge\rvX > \frac{y-b}{a}+\frac{1}{a}h} &\forall a<0
  %          \end{array}\right.
  %    \\&=  \left\{\begin{array}{ll}
  %            \pP{\frac{y-b}{a}\le\rvX < \frac{y-b}{a}+\frac{1}{|a|}h}  &\forall a>0 \\
  %            \pP{\frac{y-b}{a}-\frac{1}{|a|}h <\rvX \le \frac{y-b}{a}} &\forall a<0
  %          \end{array}\right.
...
%              \\&= \pP{y\le X^2 < y+h | X<0}\pP{\rvX<0} + \pP{y\le X^2 < y+h | X\ge0}\pP{\rvX\ge0}
%              \\&= \pP{-\sqrt{y}\le\rvX < -\sqrt{y+h} | X<0  } \pP{\rvX<0} +
%                   \pP{+\sqrt{y}\le\rvX < +\sqrt{y+h} | X\ge0} \pP{\rvX\ge0}
%              \\&= \pP{-\sqrt{y}\le\rvX < -\left(\sqrt{y}+\frac{1}{2\sqrt{y}}h\right) \land X<0   } +
%                   \pP{+\sqrt{y}\le\rvX <        \sqrt{y}+\frac{1}{2\sqrt{y}}h        \land X\ge0 }
...
\begin{align*}
  ...
  \\&=    \ddz\lim_{\varepsilon\to0}\sum_{n\in\Z} \pPa{\rvX+\rvY \le z}{y+n\varepsilon<\rvY\le y+(n+1)\varepsilon}
    &&    \text{by \thme{sum of products}}&&\text{\xref{thm:psp_sop}}
  %\\&=    \ddz\int_y \pPc{\rvX+y \le z}{y<\rvY\le y+\varepsilon}\pP{y<\rvY\le y+\varepsilon} \dy
...
%\begin{align*}
%  \ppz(z)
%    &\eqd \lim_{\varepsilon\to0}\frac{1}{\varepsilon} \pP{z \le Z < z+\varepsilon }
%    &&    \text{by definition of $\ppz$ \xref{def:pdf}}
%  \\&=    \lim_{\varepsilon\to0}\frac{1}{\varepsilon} \pP{z \le X+Y < z+\varepsilon }
%    &&    \text{by definition of $\rvZ$}
%  \\&=    \lim_{\varepsilon\to0}\frac{1}{\varepsilon} \int_y \pP{(z \le X+y < z+\varepsilon) \land (y\le Y<y+\varepsilon) } \dy
%    &&    \text{by \thme{sum of products} \xref{thm:psp_sop}}
%  \\&=    \lim_{\varepsilon\to0}\frac{1}{\varepsilon} \int_y \pP{(z-y \le\rvX < z-y+\varepsilon) | (y\le Y<y+\varepsilon) }\pP{y\le Y<y+\varepsilon} \dy
%  \\&=    \lim_{\varepsilon\to0}\frac{1}{\varepsilon} \int_y \pP{(z-y \le\rvX < z-y+\varepsilon) | Y=y }\pP{y\le Y<y+\varepsilon} \dy
%  \\&=    \lim_{\varepsilon\to0}\frac{1}{\varepsilon} \int_y \pP{(z-y \le\rvX < z-y+\varepsilon)}\pP{y\le Y<y+\varepsilon} \dy
