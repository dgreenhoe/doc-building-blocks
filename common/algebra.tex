...
\defboxt{
  The pair $\opair{\setX}{\Diamond}$ is a \hid{group} if
...
\defboxt{
  The triple $\otriple{\setX}{+}{*}$ is a \hid{ring} if
  \\\indentx
  $\begin{array}{F lcl @{\qquad}C @{\qquad}D @{\qquad}D}
     ...
     2. & x(yz)  &=& (xy)z
        & \forall x,y,z\in\setX
        & (\prop{associative} with respect to $*$)
        & and
        \\
     3. & x(y+z) &=& (xy)+(xz)
        & \forall x,y,z\in\setX
        & ($*$ is \prop{left distributive} over $+$)
        & and
        \\
     4. & (x+y)z &=& (xz)+(yz)
        & \forall x,y,z\in\setX
        & ($*$ is \prop{right distributive} over $+$).
  \end{array}$}
\end{definition}
...
%---------------------------------------
\defboxt{
  A triple $\otriple{\setX}{+}{*}$ is a \hid{commutative ring} if
  \\\indentx
  $\begin{array}{F M @{\qquad}C@{\qquad}D@{\qquad}D}
    1. & $\otriple{\setX}{+}{*}$ is a ring  &                     & (ring) & and \\
    2. & $xy=yx$                            & \forall x,y\in\setX & (commutative).
  \end{array}$}
\end{definition}
...
\defboxt{
  A function $\absn$ in $\clFrr$ is an \hid{absolute value} (or \hid{modulus}) if
  \\\indentx
