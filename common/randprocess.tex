...
\defboxt{
  ...
  \\\indentx$\begin{array}{FllMD}
   (1).& \pmeanx(t)     & is constant with respect to $t$ & (\prope{stationary in the mean})    & and \\
   (2).& \Rxx(t+\tau,t) & is constant with respect to $t$ & (\prope{stationary in correlation})
  \end{array}$
  }
\end{definition}
...
\defbox{\begin{array}{rclcl}
   \LSxx(s) &\eqd& \opL{\Rxx(\tau)} &\eqd& \ds \int_{\tau\in\R}\Rxx(\tau) e^{-s\tau} \; d\tau   \\
   \LSyy(s) &\eqd& \opL{\Ryy(\tau)} &\eqd& \ds \int_{\tau\in\R}\Ryy(\tau) e^{-s\tau} \; d\tau   \\
   \LSxy(s) &\eqd& \opL{\Rxy(\tau)} &\eqd& \ds \int_{\tau\in\R}\Rxy(\tau) e^{-s\tau} \; d\tau   \\
   \LSyx(s) &\eqd& \opL{\Ryx(\tau)} &\eqd& \ds \int_{\tau\in\R}\Ryx(\tau) e^{-s\tau} \; d\tau   \\
\end{array}}
\end{definition}
...
\defbox{\begin{array}{rc>{\ds}lc>{\ds}l}
   \Swxx(\omega) &\eqd& [\opFT{\Rxx(\tau)}](\omega) &\eqd& \ds \int_{\tau\in\R} \Rxx(\tau) e^{-i2\pi\omega\tau} \; d\tau   \\
   \Swyy(\omega) &\eqd& [\opFT{\Ryy(\tau)}](\omega) &\eqd& \ds \int_{\tau\in\R} \Ryy(\tau) e^{-i2\pi\omega\tau} \; d\tau   \\
   \Swxy(\omega) &\eqd& [\opFT{\Rxy(\tau)}](\omega) &\eqd& \ds \int_{\tau\in\R} \Rxy(\tau) e^{-i2\pi\omega\tau} \; d\tau   \\
   \Swyx(\omega) &\eqd& [\opFT{\Ryx(\tau)}](\omega) &\eqd& \ds \int_{\tau\in\R} \Ryx(\tau) e^{-i2\pi\omega\tau} \; d\tau
\end{array}}
\end{definition}
...
\defboxt{
  ...
  \\\indentx$\ds\pE\brs{\rvx(t)} = \lim_{T\to\infty}\frac{1}{2T}\int_{-T}^{+T}\rvx(t)\dt$
  }
\end{definition}
...
\thmbox{
  \brb{\begin{array}{FMD}
    ...
    \\(B).&$\rvx(t)$ is \prope{ergodic in the mean}
  \end{array}}
  \implies
  \brb{\begin{array}{M}
    The \hie{power} of $\rvx(t)$
    \\in the spectral interval $\intcc{\omega_1}{\omega_2}$ is
    \\\indentx$\ds\text{power } = \int_{\omega_1}^{\omega_2} \Swxx(\omega)\dw$ 
  \end{array}}
  }
\end{theorem}
\begin{proof}
\begin{align*}
   ...
   \\&=    \pE\brs{\rvx^\ast(t)e^{i\omega t} \ft{x}(\omega) }
     ...
     &&    \text{\xref{def:opFT}}
   \\&=    \pE\brs{\rvx^\ast(t)e^{i\omega t}} \ft{x}(\omega)
   \\&=    \brs{\lim_{T\to\infty}\frac{1}{2T}\int_{-T}^{+T} \rvx^\ast(t)e^{i\omega t} \dt} \ft{x}(\omega)
     &&    \text{by \prope{ergodic in the mean} hypothesis}
     &&    \text{\xref{def:ergomean}}
   \\&=    \lim_{T\to\infty}\frac{1}{2T}\brs{\int_{t\in\R}\rvx(t)e^{-i\omega t} \dt}^\ast \ft{x}(\omega)
   \\&=    \lim_{T\to\infty}\frac{1}{2T} \ft{x}^\ast(\omega) \ft{x}(\omega)
   \\&=    \lim_{T\to\infty}\frac{1}{2T} \left|\ft{x}(\omega)\right|^2
\end{align*}
