...
\begin{array}{rcll}
   ...
   \Rxy(t,u) &=& \Ryx^\ast(u,t) & \text{(\prope{conjugate symmetric})}.
\end{array}
}
\end{theorem}
...
\thmbox{\begin{array}{rlp{7cm}}
  ...
  4. & \norm{\psi_n(t)}^2>0, \inprod{\opR f}{f} > 0 \implies \lambda_n>0
     ...
     \\
  5. & \pE\brb{\left| x(t)-\sum_{n\in\Z}\inprod{x(t)}{\psi_n(t)} \psi_n(t) \right|^2} = 0
     & ($\{\psi_n(t)\}$ is a basis for $\rvx(t)$)
\end{array}}
\end{theorem}
\begin{proof}
\begin{enumerate}
...
Let $\rvx(t)$ and $\rvy(t)$ be random processes.\\
\defbox{\begin{array}{MlMlc>{\ds}l}
     The \fnctd{mean}                     & \pmeanx(n) & of $\rvx(n)$              is & \pmeanx(n)&\eqd& \pE\brs{\rvx(n)}                    %& \text{(\prope{mean functional})} \\
   \\The \fnctd{autocorrelation function} & \Rxx(n,m)  & of $\rvx(n)$              is & \Rxx(n,m) &\eqd& \pE\brs{\rvx(n)\rvx^\ast(n+m)}        %& \text{(\prope{auto-correlation bilinear functional})} \\
...
\thmboxt{
  ...
  \brb{\begin{array}{Frc>{\ds}lc>{\ds}lD}
     ...
     \\(B).&\Ryy(n,m) &=& \Rxy(n,m)\conv \fh(n)      &\eqd& \sum_{k\in\Z} \fh(k)      \Rxy(n-k,m) & and 
     \\(C).&\Ryy(n,m) &=& \Rxx(n,m)\conv \fh(n)\conv h^\ast(m)                                          
                      &\eqd& \mc{2}{l}{\ds\sum_{p\in\Z} \sum_{k\in\Z} \fh(k)h^\ast(p) \Rxx(n-k,m-p)}
  \end{array}}$
  }
\end{theorem}
...
\begin{definition}
%---------------------------------------
Let $\rvx(n)$ be a random process with \fncte{mean} $\pmeanx(n)$ and
\fncte{autocorrelation} $\Rxx(n,n+m)$.
\\
\defboxt{
  $\rvx(n)$ is \propd{wide sense stationary} (\propd{WSS}) if
  \\\indentx
    $\begin{array}{FlMMD}
       1. & \pmeanx(n)  & is \prope{constant} with respect to $n$ & (\prope{stationary in the mean})    & and\\
       2. & \Rxx(n,n+m) & is \prope{constant} with respect to $n$ & (\prope{stationary in correlation})
    \end{array}$
  }
\end{definition}
...
%---------------------------------------
Let $\rvx(n)$ be a random process with \fncte{mean} $\pmeanx(n)$ and
\fncte{autocorrelation} $\Rxx(n,n+m)$.\footnote{
  The $\pmeanx$ and $\Rxx(m)$ definitions \xref{def:mean_wss} are examples
...
\defbox{
  ...
  \end{array}}
  \implies
  \brb{\begin{array}{FlMrclD}
      (1). & \pmeanx & is defined as & \pmeanx &\eqd& \pmeanx(0) & and
    \\(2). & \Rxx(m) & is defined as & \Rxx(m) &\eqd& \Rxx(0,m)  & and
    \\(3). & \Rxy(m) & is defined as & \Rxy(m) &\eqd& \Rxy(0,m)  & .
  \end{array}}
  }
\end{definition}
...
Let $\rvy(n)$ be a \fncte{random process},
    $\rvx(n)$    a random process with \fncte{autocorrelation} $\Rxx(n,n+m)$,
...
WSS random process \xref{fig:d-linear-sys-WSS}.
%---------------------------------------
\begin{corollary}
\footnote{
  \citerp{papoulis}{323}
...
\corboxt{
  ...
  \brb{\begin{array}{Frc>{\ds}lc>{\ds}lD}
     ...
     \\(3).&\Ryy(m) &=& \Rxx(m)\conv \fh(m)\conv h^\ast(-m) &\eqd& %\mc{2}{l}{\ds\sum_{j\in\Z}\sum_{k\in\Z}\fh^\ast(-j)\fh(k-j) \Rxx(m-k-j)}
                                                                   \mc{2}{l}{\ds\sum_{p\in\Z}\sum_{k\in\Z}\fh(k)h^\ast(p) \Rxx(-k,m-p)}
  \end{array}}
  $
  }
\end{corollary}
...
%20181110%\begin{align*}
%20181110%   \Rxx(m) \conv \fh^\ast(-m)
%20181110%     &\eqd \sum_{k\in\Z} \fh^\ast(-k) \Rxx(m-k)
%20181110%     && \text{by definition of \ope{convolution}}
%20181110%     && \text{\xref{def:dsp_conv}}
%20181110%   \\&\eqd \sum_{k\in\Z} \fh^\ast(-k) \pE\brs{\rvx(n) \rvx^\ast(n-m+k)}
%20181110%     && \text{by definition of $\Rxx$}
%20181110%     && \text{\xref{def:Rxx_wss}}
%20181110%   \\&=    \pE\brs{ \sum_{k\in\Z} \fh^\ast(-k) \rvx(n) \rvx^\ast(n-m+k)      }
%20181110%     && \text{by \prope{linear} property of $\pE$}
%20181110%     && \text{\xref{thm:pE}}
%20181110%   \\&=    \pE\brs{\rvx(n) \sum_{k\in\Z} \fh^\ast(-k)  \rvx^\ast(n-m+k)      }
%20181110%   \\&=    \pE\brs{\rvx(n) \sum_{k^\prime\in\Z} \fh^\ast(k^\prime)  \rvx^\ast(n-m-k^\prime)}
%20181110%     && \text{where $k^\prime\eqd-k$}
%20181110%   \\&\eqd \pE\brs{\rvx(n) \brp{\fh^\ast(n) \conv \rvx^\ast(n-m)}}
%20181110%     && \text{by definition of \ope{convolution}}
%20181110%     && \text{\xref{def:dsp_conv}}
%20181110%   \\&=    \pE\brs{\rvx(n) \rvy^\ast(n-m)}
%20181110%     && \text{by \prope{LTI} hypothesis}
%20181110%     && 
%20181110%   \\&\eqd \Rxy(m)
%20181110%     && \text{by definition of $\Rxy$}
%20181110%     && \text{\xref{def:Rxx_wss}}
%20181110%\\
%20181110%\\
%20181110%   \Rxy(m) \conv \fh(m)
%20181110%     &\eqd \sum_{k\in\Z} \fh(k) \Rxy(m-k)
%20181110%     && \text{by definition of \ope{convolution}}
%20181110%     && \text{\xref{def:dsp_conv}}
%20181110%   \\&=    \sum_{k\in\Z} \fh(k) \E\brs{\rvx(n+m-k) \rvy^\ast(n)}
%20181110%     && \text{by definition of $\Rxy$}
%20181110%     && \text{\xref{def:Rxy_wss}}
%20181110%   \\&=    \pE\brs{\rvy^\ast(n) \sum_{k\in\Z}\fh(k) \rvx(n+m-k)}
%20181110%     && \text{by \prope{linear} property of $\pE$}
%20181110%     && \text{\xref{thm:pE}}
%20181110%   \\&=    \pE\brs{\rvy^\ast(n) \brp{\fh(n)\conv\rvy(n+m)} }
%20181110%     && \text{by definition of \ope{convolution}}
%20181110%     && \text{\xref{def:dsp_conv}}
%20181110%   \\&=    \pE\brs{\rvy^\ast(n) \rvy(n+m) }
%20181110%     && \text{by \prope{LTI} hypothesis}
%20181110%     && 
%20181110%   \\&=    \pE\brs{ \rvy(n+m) \rvy^\ast(n)}
%20181110%     && \text{by \prope{commutative} prop.}  % of $\fieldR$
%20181110%     && \text{\ifxref{algebra}{def:field}}
%20181110%   \\&\eqd \Ryy(m)
%20181110%     && \text{by definition of $\Ryy$}
%20181110%     && \text{\xref{def:Rxx_wss}}
%20181110%\\
%20181110%\\
%20181110%   \Ryy(m)
%20181110%     &= \Rxy(m) \conv\fh(m)
%20181110%     && \text{by previous result}
%20181110%   \\&= \Rxx(m) \conv\fh^\ast(-m) \conv \fh(m)
%20181110%     && \text{by previous result}
%20181110%   \\&= \Rxx(m) \conv\fh(m)  \conv \fh^\ast(-m)
%20181110%\end{align*}
\end{proof}
...
\label{def:csd}
%---------------------------------------
Let $\rvx(n)$ and $\rvy(n)$ be a \prope{wide sense stationary} random processes
...
\thmbox{
  ...
  \brb{\begin{array}{FrclD}
       (1).&\Sxy(z) &=& \ds \Sxx(z) \Zh^\ast\brp{\frac{1}{z^\ast}} & and 
     \\(2).&\Syy(z) &=& \ds \Sxy(z) \Zh(z)                         & and 
     \\(3).&\Syy(z) &=& \mc{2}{l}{\ds \Sxx(z) \Zh(z) \Zh^\ast\brp{\frac{1}{z^\ast}}}
  \end{array}}
  }
\end{theorem}
\begin{proof}
...
%    && \text{\xref{def:opZ}}
%   \\&=    \sum_{m\in\Z}\brs{ \Rxx(m) \conv h^\ast(-m) } z^{-m}
%   \\&=    \sum_{m\in\Z}\brs{ \sum_{k\in\Z}h^\ast(-k) \Rxx(m-k)  } z^{-m}
...
%   \\&=    \sum_{k\in\Z}h^\ast(k) z^{-(-k)}   \sum_{j\in\Z}\Rxx(j)z^{-j}
%   \\&=    \brs{\sum_{k\in\Z}h(k) \brp{\frac{1}{z^\ast}}^{-k} }^\ast  \sum_{j\in\Z}\Rxx(j)z^{-j}
%   \\&\eqd \Zh^\ast\brp{\frac{1}{z^\ast}} \Sxx(z)
%\\
%\\
   \Syy(z)
     &\eqd \opZ \Ryy(m)
