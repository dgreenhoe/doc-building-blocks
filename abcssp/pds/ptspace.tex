...
This paper introduces a new relation called the \rele{power triangle inequality} \xref{def:trirel}.
...
\defboxp{
  ...
  \\\indentx$\begin{array}{rclCC}
    \ptfD&\eqd& 2\sigma\brs{\frac{1}{2}\distancep{p}{x}{z} + \frac{1}{2}\distancep{p}{z}{y}}^\frac{1}{p} & \forall \opair{p}{\sigma}\in\Rx\times\Rnn, & x,y,z\in\setX
  \end{array}$
  }
\end{definition}
...
Let $\clRxxx$ be the set of all trinomial \structe{relation}s \xref{def:clRxy} on $\setX$.
\defboxp{
...
\\\defboxp{$\begin{array}{Fl@{\,}r@{\,}@{\,}c@{\,}@{\,}l@{\,}M}
  1. & \trirel(& \infty      ,&\sfrac{\sigma}{2}  ;\distancen&)&{\scs is the} \reld{\txsigma-inframetric inequality}
\\2. & \trirel(& \infty      ,&\frac{1}{2}        ;\distancen&)&{\scs is the} \reld{inframetric inequality}
\\3. & \trirel(& 2           ,&\sfrac{\sqrt{2}}{2};\distancen&)&{\scs is the} \reld{quadratic inequality}
\\4. & \trirel(& 1           ,&\sigma             ;\distancen&)&{\scs is the} \reld{relaxed triangle inequality}
\\5. & \trirel(& 1           ,&1                  ;\distancen&)&{\scs is the} \reld{triangle inequality}
\\6. & \trirel(& \sfrac{1}{2},&2          ;\distancen &)       &{\scs is the} \reld{square mean root inequality}
\\7. & \trirel(& 0           ,&\frac{1}{2};\distancen &)       &{\scs is the} \reld{geometric inequality}
\\8. & \trirel(&-1           ,&\frac{1}{4};\distancen &)       &{\scs is the} \reld{harmonic inequality}
\\9. & \trirel(&-\infty      ,&\frac{1}{2};\distancen &)       &{\scs is the} \reld{minimal  inequality}
\end{array}$}
\end{definition}
...
\corboxp{
  The function $\ptfD$
  is \prope{continuous} and \prope{strictly monotone} in $\omsR$ with respect to both the variables $p$ and $\sigma$.
  }
\end{corollary}
\begin{proof}
\begin{enumerate}
...
%---------------------------------------
Let $\ptfD$ be the \fncte{power triangle function} \xrefh{def:ptf} in the \structe{distance space} \xref{def:dspace} $\dspaceX$.
\\\corboxp{$
   \ptfD =
    \brbl{\begin{array}{lDlD}
      2\sigma\max\setn{\distancen(x,z),\,\distancen(z,y)}                                 &for& p=\infty,  & (\fncte{maximum}, {\scs corresponds to} \structe{inframetric space})\\
      2\sigma\brs{\sfrac{1}{2}\distancep{2}{x}{z} + \sfrac{1}{2}\distancep{2}{z}{y}}^\frac{1}{2} &for& p=2,       & (\fncte{quadratic mean})\\
...
\corbox{\begin{array}{rclclcl}
         2\sigma\min\setn{\distancen(x,z),\, \distancen(z,y)}
   &\le& 4\sigma     \brs{\frac{1}{\distancen(x,z)} + \frac{1}{\distancen(z,y)}}^{-1}
   &\le& 2\sigma     \sqrt{\distancen(x,z)}\,\sqrt{\distancen(z,y)}
 \\&\le&  \sigma\brs{\distancen(x,z) + \distancen(z,y)}
   &\le& 2\sigma\max\setn{\distancen(x,z),\,\distancen(z,y)}
\end{array}}
\end{corollary}
\begin{proof}
...
%  \gsize%
%  \centering%
%  %\input{../common/math/graphics/metric/trirel_psigma.tex}
...
%  \centering%
%  \gsize%
%  \psset{unit=2mm}\input{ballinball.tex}%
...
\begin{figure}[h]
  ...
  \includegraphics{pds/graphics/ballinopen.pdf}
