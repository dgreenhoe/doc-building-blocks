...
\defbox{\begin{array}{rc>{\ds}ll}
   \pmeanx(t)    &\eqd& \pEb{\rvx(t)}             & \text{(mean functional)} \\
...
\thmbox{\begin{array}{rlp{7cm}}
  ...
  4. & \norm{\psi_n(t)}^2>0, \inprod{\opR f}{f} > 0 \implies \lambda_n>0
     ...
     \\
  5. & \E{\left| x(t)-\sum_{n\in\Z}\inprod{x(t)}{\psi_n(t)} \psi_n(t) \right|^2} = 0
...
\thmbox{\begin{array}{rcl}
   \Rxx(n,n+m) &=& \Rxx^\ast(n+m,m)\\ 
   \Rxy(n,n+m) &=& \Ryx^\ast(n+m,m)
\end{array}}
\end{theorem}
\begin{proof}
\begin{align*}
...
Let $\rvx(n)$ be a random process with \fncte{autocorrelation} $\Rxx(n,n+m)$.
Let $\Rxy$ be the \fncte{cross-correlation} of $\rvx$ and $\rvy$.
\corbox{
  \brb{\begin{array}{M}
    $\rvx$ and $\rvy$ are\\
    \prope{wide sense stationary}
...
\corboxt{
  $\brb{\begin{array}{FMMD}
    (1). & $\opS$ is \prope{linear time invariant}             & (\prope{LTI}) & and\\
    (2). & $\rvx$ and $\rvy$ are \prope{wide sense stationary} & (\prope{WSS}) & 
  \end{array}}
  \quad\implies$
    \\\qquad$
  \brb{\begin{array}{Frc>{\ds}lc>{\ds}lD}
       (A).&\Rxy(m) &=& \Rxx(m)\conv \fh^\ast(m)            &\eqd& \sum_{k\in\Z} \fh^\ast(k) \Rxx(m-k)  & and 
     \\(B).&\Ryy(m) &=& \Rxy(m)\conv \fh(m)                 &\eqd& \sum_{k\in\Z} \fh(k) \Rxy(m-k)        & and 
     \\(C).&\Ryy(m) &=& \Rxx(m)\conv \fh(m)\conv h^\ast(-m) &\eqd& %\mc{2}{l}{\ds\sum_{j\in\Z}\sum_{k\in\Z}\fh^\ast(-j)\fh(k-j) \Rxx(m-k-j)}
...
%   \\&=    \sum_{k\in\Z}h^\ast(k) z^{-(-k)}   \sum_{j\in\Z}\Rxx(j)z^{-j}
%   \\&=    \brs{\sum_{k\in\Z}h(k) \left(\frac{1}{z^\ast}\right)^{-k} }^\ast  \sum_{j\in\Z}\Rxx(j)z^{-j}
