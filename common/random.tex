...
\corboxt{
  ...
  \quad\implies$
    \\\qquad$
  \brb{\begin{array}{Frc>{\ds}lc>{\ds}lD}
       (A).&\Rxy(m) &=& \Rxx(m)\conv \fh^\ast(-m)           &\eqd& \sum_{k\in\Z}k h^\ast(-k) \Rxx(m-k)  & and 
     \\(B).&\Ryy(m) &=& \Rxy(m)\conv \fh(m)                 &\eqd& \sum_{k\in\Z}k h(k) \Rxy(m-k)        & and 
     \\(C).&\Ryy(m) &=& \Rxx(m)\conv \fh(m)\conv h^\ast(-m) &\eqd& \mc{2}{l}{\ds\sum_{j\in\Z}\sum_{k\in\Z} h^\ast(-j) h(k-j) \Rxx(m-k-j)}
  \end{array}}
  $
  }
\end{corollary}
...
\\
   ...
   \\&=    \pE\brs{ \rvy(n+m) \rvy^\ast(n)}
     && \text{by \prope{commutative} prop.}  % of $\fieldR$
     && \text{\ifxref{algebra}{def:field}}
   \\&\eqd \Ryy(m)
...
\thmbox{
  ...
  \brb{\begin{array}{rclcl}
       (1).&\Sxy(z) &=& \ds \Sxx(z) \Zh^\ast\brp{\frac{1}{z^\ast}}
     \\(2).&\Syy(z) &=& \ds \Sxy(z) \Zh(z)
     \\(3).&\Syy(z) &=& \ds \Sxx(z) \Zh(z) \Zh^\ast\brp{\frac{1}{z^\ast}}
  \end{array}}
  }
\end{theorem}
\begin{proof}
