...
\thmbox{\begin{array}{l}
  \left.\begin{array}{Fl}
    (A). & \mcom{\opNa\opN=\opN\opNa}{$\opN$ is \hie{normal}}\\
    (B). & \text{$\opN$ is \hie{compact}}
  \end{array}\right\}
  \qquad\implies\qquad
...
\thmbox{
  \brb{\begin{array}{F>{\ds}lD}
    (A). & \mcom{\int_a^b \int_a^b \kappa(t,s) \ff(t) \ff^\ast(s) \ds\dt \ge 0}{positive} & and\\
    (B). & \kappa(t,s) \text{ is continuous on } [a,b]\times[a,b]
  \end{array}}
  \implies
  \brb{\begin{array}{F>{\ds}lD}
    (1). & \kappa(t,s) = \sum_n \lambda_n \fphi_n(t) \fphi_n^\ast(s) & and\\
    (2). & \kappa(t,s) \text{ converges absolutely and} \\
      & \text{uniformly on $\intcc{a}{b}\times\intcc{a}{b}$}
  \end{array}}
  }
\end{theorem}
