...
%  The \hid{summation} of $\seqxZ{x_n}$ from index $m$ to index $\xN$ over a \structe{field} $\fieldF$ is
%  \\\qquad$\ds
%  \sum_{n=m}^\xN x_n \eqd 
%    \brbl{\begin{array}{>{\ds}l@{\qquad}M}
%      0                            & for $\xN<m$\\
%     %x_\xN                          & for $N=m$ \\
%    \brp{\sum_{n=m}^{\xN-1} x_n}\addf x_\xN & for $\xN\ge m$
%    \end{array}}
%  $
%\end{array}}
%\end{definition}
...
\thmbox{
  \brb{y_n = x_{(\xN n})}
  \qquad\implies\qquad
  \brb{\Zy(z)=\sum_{m=0}^{\xN-1}\Zx\left( e^{i\frac{2\pi m}{\xN}} z^{\frac{1}{\xN}} \right)}
  }
\end{theorem}
\begin{proof}
\begin{align*}
...
%  \psset{unit=10mm}\input{../common/wavelets/graphics/d2_phi_h.tex} &
%  \setlength{\unitlength}{0.10mm}\input{../common/wavelets/graphics/D2_pz.tex}  \\
