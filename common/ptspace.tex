...
%metric.tex \begin{figure}[h]
%metric.tex   \center
%metric.tex   \includegraphics{../common/math/graphics/pdfs/ballinopen.pdf}
%metric.tex   \caption{\structe{open set} (see \prefp{lem:tri_open}) \label{fig:ms_open} }
%metric.tex \end{figure}
\pref{lem:tri_open} (next) demonstrates that every point in an open set is contained in an open ball that is 
contained in the original open set \xref{fig:ms_open}.
%---------------------------------------
\begin{lemma}
\label{lem:tri_open}
%---------------------------------------
...
\label{thm:convergent_cauchy_pdspace}
%---------------------------------------
Let $\pdspaceX$ be a \structe{power distance space}.
...
However \pref{thm:xn_to_xy_pdspace} (next) demonstrates that, in a \structe{power distance space},
...
%  4. & $\seqn{x_n}$ is \prope{convergent} in $\dspaceX$ &\implies& limit is \prope{unique}                        & \xref{thm:xn_to_xy_pdspace}.\\
...
\begin{enumerate}
  ...
  \item In $\dspaceX$, the limits of \prope{convergent} sequences are \prope{unique}.
        ...
        and by \prefp{thm:xn_to_xy_pdspace}.

  \item In $\dspaceX$, \prope{convergent} sequences are \prope{Cauchy}.
        This follows directly from the fact that $\pdspace{\R}{\distancen}{1}{2}$ is a \structe{power distance space} \xref{item:pdspace_21}
        and by \prefp{thm:convergent_cauchy_pdspace}.
\end{enumerate}
\end{example}
