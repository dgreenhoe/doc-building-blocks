...
\pref{ex:msr_counting} (next) is useful for converting an integral $\int$ into a summation $\sum$.
%---------------------------------------
\begin{example}
...
\begin{align*}
  ...
  \\&= \sum_{n\in\Z} a_n \seto{\set{n\in\Z}{\ff(n)=a_n}}
  \\&= \sum_{n\in\Z} \ff(n)
\end{align*}
...
%======================================
Important measure spaces include \structb{probability spaces}.
See \prefpp{sec:probspace_examples} for examples.
...
%---------------------------------------
\exboxt{
  The \structd{Jordan measure space} is $\ms$ where
  \\\indentx$\begin{array}{>{\ds}ll>{\ds}l}
    \msx                &=& \R                      \\
    \mss                &=& \set{(a,b]}{a,b\in\R}   \\
    \msm(\intoc{a}{b})  &=& \inf\set{\sum_{n=1}^\xN (b_n-a_n)}
