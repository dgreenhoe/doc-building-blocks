...
\label{def:ring}
%---------------------------------------
Let $+:\setX\times \setX\to \setX$ and $\cdot:\setX\times \setX\to \setX$
be operations on a set $\setX$.
Furthermore, let the operation $\cdot$ also be represented by
juxtapostion as in $a\cdot b\equiv ab$.\\
\defboxt{
  The triple $\otriple{\setX}{+}{\cdot}$ is a \structd{ring} if
  \\\indentx
  $\begin{array}{F lcl @{\qquad}C @{\qquad}D @{\qquad}D}
     ...
     2. & x(yz)  &=& (xy)z
        & \forall x,y,z\in\setX
        & (\prope{associative} with respect to $\cdot$)
        & and
        \\
     3. & x(y+z) &=& (xy)+(xz)
        & \forall x,y,z\in\setX
        & ($\cdot$ is \prope{left distributive} over $+$)
        & and
        \\
     4. & (x+y)z &=& (xz)+(yz)
        & \forall x,y,z\in\setX
        & ($\cdot$ is \prope{right distributive} over $+$).
  \end{array}$}
\end{definition}
...
%---------------------------------------
\defboxt{
  A triple $\otriple{\setX}{+}{\cdot}$ is a \structd{commutative ring} if
  \\\indentx
  $\begin{array}{FMCD}
      1. & $\otriple{\setX}{+}{\cdot}$ is a \structe{ring}  &                     &  and
...
\defboxt{
  ...
  \\\indentx$\begin{array}{FlCDD}
   1. & \otriple{\setX}{+}{\cdot} \text{ is a ring}
      &
      & (ring)
      & and
      \\
   2. & xy=yx
      & \forall x,y\in\setX
      & (commutative with respect to $\cdot$)
      & and
      \\
   3. & (\setX\setd\setn{0},\cdot) \text{ is a group}
      &
      & (group with respect to $\cdot$).
\end{array}$}
\end{definition}
...
\label{def:algebra}
%---------------------------------------
Let $\spV=(F,+,\cdot)$ be a \structe{vector space} and
