...
%The \fncte{indicator function} (next) is a convenient
%``indicator" of whether or not a particular element is in a set.
%%and has several interesting properties (next theorem).
%%---------------------------------------
%\begin{definition}
%\footnote{
%  \citerp{ab}{126},
...
%    $
%  \\
%  The indicator function is also called the \fnctd{characteristic function}.
%\end{array}}
%\end{definition}
%}
...
\end{definition}
\end{minipage}
\hfill\tbox{\includegraphics{graphics/fstep.pdf}}
...
\begin{figure}
  ...
  \begin{tabular}{|c|c|c|}
    ...
    \hline
    \mc{2}{|c|}{\includegraphics{graphics/n5.pdf}}&\includegraphics{graphics/n6.pdf}\\
    \mc{2}{|c|}{$\fN_5(x)$}&$\fN_6(x)$\\
    \hline
    \mc{3}{|c|}{\includegraphics{graphics/n7.pdf}}\\
    \mc{3}{|c|}{$\fN_7(x)$}\\
    \hline
  \end{tabular}
...
\begin{enumerate}
  ...
  \item inductive step\ldots proof that $n$ case $\implies$ $n+1$ case:
    \begin{align*}
      &\fN_{n+1}(x) 
...
Let $\fN_3(x)$ be the \fncte{$3$rd order B-spline} \xref{def:Bspline}.\footnote{%
  ...
  \fN_3(x) = \frac{1}{6}\brb{\begin{array}{*{4}{@{\hspace{2pt}}c@{\hspace{2pt}}r}M}
    ...
    -& x^3 &+& 12x^2 &-& 48x &+& 64  & for $3\le x\le 4$ \\
     &               & &     & &  0 & &               & otherwise
  \end{array}}
  \tbox{\includegraphics{graphics/n3.pdf}}
  }
\end{example}
\begin{proof}
...
\exbox{
  \fN_4(x) = \frac{1}{24}\brb{\begin{array}{*{5}{@{\hspace{2pt}}c@{\hspace{2pt}}r}M}
               ...
               -&4x^4 &+& 60x^3 &-& 330x^2 &+& 780x &-& 655  & for $3 \le x \le= 4$\\
                ...
                &     & &       & &        & &      & &   0  & otherwise             
  \end{array}}
  %\tbox{\includegraphics{graphics/n4.pdf}}
  }
\end{example}
\begin{proof}
This expression can be calculated ``by hand" using \prefpp{thm:Nnx}
...
\exbox{
  \fN_5(x) = \frac{1}{120}\brb{\begin{array}{*{6}{@{\hspace{2pt}}c@{\hspace{2pt}}r}M}
               ...
               -&   x^5 &+&  30x^4 &-&  360x^3 &+& 2160x^2 &-&  6480x &+&  7776  & for $5 \le x \le= 6$\\
                &       & &        & &         & &         & &        & &     0  & otherwise             
  \end{array}}
  %\tbox{\includegraphics{graphics/n5.pdf}}
  }
  \\
...
\exboxt{%
  ...
  $\brb{\begin{array}{*{8}{@{\hspace{1pt}}>{\scy}c@{\hspace{1pt}}r}D}
    ...
    -&    x^7 &+&   56 x^6 &-&  1344 x^5 &+&  17920 x^4 &-& 143360 x^3 &+&  688128 x^2 &-& 1835008 x &+& 2097152  & for $ 7 \le x \le 8$ \\
     &        & &          & &           & &            & &            & &             & &           & &       0  & otherwise
  \end{array}}$
  %\tbox{\includegraphics{graphics/n7.pdf}}
  }
  \\
...
Note: For information about differentiation of an integral, see
  \citer{flanders1973},
  \citer{talvila2001}, and
  \citerpgc{knappb2005}{389}{0817632506}{Chapter VII}.
  %\item \url{http://planetmath.org/encyclopedia/DifferentiationUnderIntegralSign.html}
\end{proof}
...
\begin{enumerate}
  \item Proof that $\fN_n(x) \ge 0$ (proof by induction): \label{item:bspline_Nprop_ge0}
    \begin{enumerate}
      ...
      \item inductive step\ldots proof that $\brb{\fN_n(x)\ge0}\implies\brb{\fN_{n+1}(x)\ge0}$:
        \begin{align*}
          \fN_{n+1}(x)
...
%=======================================
\subsection{Projection of B-splines onto complex exponentials}
%=======================================
...
\\\indentx\begin{tabular}{cl}
    \imark & The complex exponential is an eigenvalue of any \prope{linear time invariant} (\prope{LTI}) operator
         \\& \xref{thm:Le=he}.
         \\
    \imark & The complex exponential generates a \fncte{continuous point spectrum} for the \ope{differential operator}. 
\end{tabular}\\
...
to be useful. 
Such a hunch would be demonstrated correct because it is useful for proving that 
\\\indentx\begin{tabular}{cl}
...
\end{example}
\end{minipage}%
\hfill\tbox{\includegraphics{graphics/Fpulse.pdf}}
...
\end{example}
\end{minipage}%
\hfill\tbox{\includegraphics{graphics/Ftent012.pdf}}%
...
\end{example}
\end{minipage}%
\hfill\tbox{\includegraphics{graphics/FN2.pdf}}%
    ...
    \begin{enumerate}
      ...
      \item Alternatively, this theorem can be proved by \thme{induction}:
        \begin{enumerate}
          ...
          \item Inductive step \ldots proof that $\sum_{k\in\Z}\fN_n(x-k)=1 \implies \sum_{k\in\Z}\fN_{n+1}(x-k)=1$:
            \begin{align*}
...
\end{example}
\end{minipage}
\hfill\tbox{\includegraphics{graphics/n0punity.pdf}}
...
\end{example}
\end{minipage}
\hfill\tbox{\includegraphics{graphics/n1punity.pdf}}
...
\end{example}
\end{minipage}
\hfill\tbox{\includegraphics{graphics/n2punity.pdf}}
...
%  \]
%\end{minipage}%
%\indentx\tbox{\includegraphics{graphics/cencubs.pdf}}
%
%%\begin{minipage}{\tw/2-2mm}
%%  \psset{unit=8mm}
...
%\end{minipage}
%\hfill\tbox{\includegraphics{graphics/cencubs_pun.pdf}}
%
%%\psset{unit=8mm}
...
%---------------------------------------
  \centering%
  \includegraphics{graphics/Snn_bspline.pdf}
  \caption{
  ...
  \item Proof that $\Swn(\pi)$ converges to some value $>0$: \label{item:Nriesz_Gpi}
    \begin{enumerate}
      ...
      \item Being important for the $n=0$ case, note that\footnote{
            ...
            \citerpgc{bailey2013}{334}{3319323776}{Catalan's Constant},
            \citePc{bailey2011}{(15)},
...
is illustrated to the right.
\end{example}%
\end{minipage}\tbox{\includegraphics{graphics/n0dyadic.pdf}}
\footnotetext{
...
is illustrated to the right.
\end{example}%
\end{minipage}\tbox{\includegraphics{graphics/n1dyadic.pdf}}
\footnotetext{
...
is illustrated to the right.
\end{example}%
\end{minipage}\tbox{\includegraphics{graphics/n2dyadic.pdf}}
...
\begin{figure}
  ...
  $\begin{array}{|*{3}{>{\ds}c|}}
     \hline
      \includegraphics{graphics/n0_h.pdf}
     &\includegraphics{graphics/n1_h.pdf}
     &\includegraphics{graphics/n2_h.pdf}
    %&\includegraphics{graphics/bspline_phi_h.pdf}
     \\
     ...
     \\\hline
       \mc{2}{|c|}{\includegraphics{graphics/n3_h.pdf}}
      &\includegraphics{graphics/n4_h.pdf}
     \\
