...
Let $\pc(x)$ be a \fncte{function} in $\psPcl$ 
and $\pp(x)$ a function in $\clF{\psE}{\R}$.
\defbox{\begin{array}{MM}
  $\pc(x)$ is a \fnctd{cummulative distribution function} &(\fnctd{cdf}) on $\ps$ if 
    \\\mc{2}{l}{\indentx\ds\pcx(x) \eqd \psP\setn{\rvX < x }}
    \\
  $\pp(x)$ is a \fnctd{probability density function}      &(\fnctd{pdf}) on $\ps$ if 
    \\\mc{2}{l}{\indentx\ds\ppx(x) \eqd \ddx\pcx(x) \eqd \ddx\psP\setn{\rvX < x }}
\end{array}}
\end{definition}
