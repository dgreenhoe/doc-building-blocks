...
\begin{figure}
  ...
  \begin{tabular}{|c|}
    \hline
     \tbox{\includegraphics{../common/math/graphics/pdfs/pdf_Poisson.pdf}}%
    \\\hline
  \end{tabular}
  \caption{
    Poisson pdfs \xref{def:pdf_poisson} for $\lambda=\seqn{1,e,4,5}$
    \label{fig:pdf_poisson}
    }
\end{figure}
%---------------------------------------
\begin{definition}
\label{def:pdf_poisson}
%---------------------------------------
\exboxt{
  The function $\ppx(k;\;\lambda)$ is the \fncte{Poisson distribution} if
  \\\indentx$\ds
    \ppx(k,\lambda) \eqd \frac{1}{k!} \; \lambda^k\;e^{-\lambda}
  $
  }
\end{definition}
...
\defbox{
  \ppx(x) \eqd
  \brbl{\begin{array}{lM}
            \ds \frac{1}{2\sigma^2\Gamma(n/2)}
                \brp{\frac{x}{2\sigma^2}}^{\frac{n}{2}-1}
                \exp\brp{-\frac{x}{2\sigma^2}}
            & for $y\ge 0$
          \\0
            & otherwise
        \end{array}}
...
\thmboxt{
  ...
  \\\indentx$\begin{array}{FMD}
      (1).& \fncte{chi-squared distribution with $1$ degree of freedom} & and
    \\(2).& distribution of $\rvX^2$ where $\rvX\sim\pN{0}{1}$          & and
    \\(3).& Gamma distribution $\Gamma\brp{\frac{1}{2},\frac{1}{2}}$    &
  \end{array}$
  }
