...
Let $\fF(s)\eqd \brs{\opLT\ff(x)}(s)$ 
and $\fG(s)\eqd \brs{\opLT\fg(x)}(s)$.
\thmbox{\begin{array}{>{\ds}rc>{\ds}lM}
  \opLT \brs{\fg(x-\alpha)}(s)             &=& e^{-\alpha s} \;\fG(s)         & (translation)\\
  \brs{\opLT\brp{e^{-\alpha x} \fg(x)}}(s) &=& \fG(s+\alpha)                  & (modulation)\\
  \brs{\opLT\brp{\fg(\alpha x)}}(s)        &=& \frac{1}{\abs{\alpha}}\fG\brp{\frac{s}{\alpha}} & (dilation)  \\
  \brs{\opLT\brp{\ff(x)+\fg(x)}}(s)        &=& \fF(s) + \fG(s)                & (\prope{additive})     \\
  \brs{\opLT\brp{\alpha\fg(x)}}(s)         &=& \alpha\fG(s)                   & (\prope{homogeneous})
\end{array}}
\end{theorem}
\begin{proof}
\begin{align*}
