...
\corbox{
  ...
  \implies
  \brb{
  \ppy(y) = \mcoml{\frac{1}{\abs{a}} \ppx\brp{\frac{y-b}{a}}}
...
\begin{proof}
  \begin{enumerate}
    \item lemma: \label{ilem:YaXb_ffp}
          $\ff(x)=ax+b$ is a \prope{differentiable function} with $\xN=0$ \prope{critical point}s
          and $\ffp(x)=a$.
    \item lemma: \label{ilem:YaXb_ffi}
          The inverse of $y=\ff(x)=ax+b$ is $x=\fg_1(y)=\ffi(y)=\frac{y-b}{a}$.
...
\corbox{
  \brb{
    \tbox{\includegraphics{../common/math/graphics/pdfs/1_over_x.pdf}}
    \rvY=\frac{1}{\rvX}
    }
  \implies
...
\begin{proof}
  \begin{enumerate}
    \item lemma: The roots of $z=\tan\theta$ are $\set{\theta_n=\atan(z)+n\pi}{n\in\Z}$. \label{ilem:ppztan_roots}
    \item lemma: The derivative of $z=\tan\theta$ is  $\ff'(\theta)=\sec^2 \theta$. \label{ilem:ppztan_derivative}
    \item It follows that
      \begin{align*}
        \ppz(z)
