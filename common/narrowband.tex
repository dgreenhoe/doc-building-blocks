...
\thmbox{
  \brb{\begin{array}{M}
    $\fx$ is \propb{real-valued}
  \end{array}}
  \implies
  \brb{\begin{array}{FrcllD}
      (1). & \Fx(f)       &=& \Fx^\ast(-f)    & \text{ (hermitian symmetric )} & and 
    \\(2). & \Reb{\Fx(f)} &=& \Reb{\Fx(-f)}   & \text{ (symmetric           )} & and 
    \\(3). & \Imb{\Fx(f)} &=& -\Imb{\Fx(-f)}  & \text{ (anti-symmetric      )} & and 
    \\(4). & \abs{\Fx(f)}     &=& |\Fx(-f)|       & \text{ (symmetric       )} & and
    \\(5). & \angle\Fx(f) &=& -\angle\Fx(-f)  & \text{ (anti-symmetric      )} &
  \end{array}}
  }
\end{theorem}
\begin{proof}
