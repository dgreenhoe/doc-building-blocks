...
and the set $\Psi=\setn{\psi_1(t),\psi_2(t),\ldots,\psi_N(t)\}}$ 
...
that $\ds\opR \psi_n = \lambda_n \psi_n$,
then by \prefpp{thm:Rxx_prop}, the set $\setn{\psi_n(t)}$ are \prope{orthogonal} and
...
and $\seq{\fpsi_n}{n\in\Z}$ are the eigenfunctions of $\opR$ such that
    $\opR \psi_n = \lambda_n \psi_n$.
\thmbox{
  \mcom{\pE\brp{\abs{ x(t)-\sum_{n\in\Z}\inprod{x(t)}{\psi_n(t)} \psi_n(t) }^2} = 0}
       {\prope{convergence in probability}}
    \qquad($\setn{\psi_n(t)}$ is a \structe{basis} for $\rvx(t)$)
  }
\end{theorem}
\begin{proof}
\begin{enumerate}
\item Proof that $\setn{\psi_n(t)}$ is a \structe{basis} for $\rvx(t)$ such that
      \\$\ds\pE\brb{\left| x(t)-\sum_{n\in\Z}\dot{x}_n \psi_n(t) \right|^2} = 0$
        \qquad where \quad$\dot{x}_n\eqd \inprod{x(t)}{\psi_n(t)}$
    \begin{align*}
