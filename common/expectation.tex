...
\fncte{probability density function} $\ppx$ \xref{def:pdf}.
\defboxt{
...
A \fncte{random variable} $\rvX$ is neither random nor a variable,
...
\defbox{\begin{array}{FM>{\ds}rc>{\ds}l}
  (1).&The $n$th \fnctd{moment} $\moment_n$ of $\rvX$ is
      & \moment_n(\rvX)  &\eqd& \pEx\brp{\rvX^n}
  \\
  (2).&The $n$th \fnctd{central moment} $\cmom_n$ of $\rvX$ is
      & \cmom_n(\rvX) &\eqd& \pEx\brs{\brp{\rvX - \pEx\rvX}^n}
\end{array}}
\end{definition}

Two common statistics that are conveniently expressed in terms of the
expectation operator are the \fncte{mean} and \fncte{variance}.
...
%---------------------------------------
Let $\rvX$ be a \fncte{random variable} on the \structe{probability space} $\ps$
with 1st \fncte{moment} $\moment_1$ and $n$th \fncte{central moment} $\cmom_n$ \xref{def:cmom}.
\defbox{\begin{array}{FM>{\ds}rc>{\ds}l}
  (1).&The \fnctd{mean} $\pmeanx$ of $\rvX$ is
      & \pmeanx  &\eqd& \moment_1(\rvX)
  \\
  (2).&The \fnctd{variance} $\pVar(\rvX)$ or $\pvarx$ of $\rvX$ is
      & \pVar(\rvX) &\eqd& \cmom_2(\rvX)
  \\
  (3).&The \fnctd{standard deviation} $\psdevX$ of $\rvX$ is
      & \psdevX &\eqd& \sqrt{\pvarx}
  \\
  (4).&The \fnctd{skewness} $\skewness(\rvX)$ is
      & \skewness(\rvX) &\eqd& \frac{1}{\psdevX^3}\cmom_3(\rvX)
  \\
  (5).&The \fnctd{kurtosis} $\kurtosis(\rvX)$ is
      & \kurtosis(\rvX) &\eqd& \frac{1}{\psdevX^4}\cmom_4(\rvX)
\end{array}}
\end{definition}
...
\thmbox{\begin{array}{>{\ds}rc>{\ds}lCD}
    \pExy(a\rvX+b\rvY+c)&=& \brp{a\pEx\rvX} + \brp{b\pEy\rvY} + c& \forall a,b,c\in\R & (\prope{linear})
\end{array}}
\end{theorem}
\begin{proof}
\begin{align*}
...
\begin{corollary}
\footnote{
  \citerp{hernandez2016}{4}
  }
\label{cor:pVar}
%---------------------------------------
Let $\rvX$ be a \fncte{random variable} on a \structe{probability space} $\ps$.
\corbox{\begin{array}{>{\ds}rc>{\ds}lC}
    \pVar(\psdev\rvX+\mu) &=& \psdev^2\pVar(\rvX)           & \forall \psdev,b  \in\R
...
\exbox{\begin{array}{>{\ds}rc>{\ds}lM}
      \brp{\pE\rvX}^{-1}  &<  & \pE\brp{\rvX^{-1}} & ( $\ff(x)\eqd\sfrac{1}{x}$ is \prope{convex}  )
   \\ \log\brp{\pE\rvX}   &>  & \pE\brp{\log\rvX}  & ( $\ff(x)\eqd\log(x)$      is \prope{concave} )
   \\ e^{-\pE\rvX}        &\le& \pE\brs{e^{-\rvX}} & ( $\ff(x)\eqd\exp(x)$      is \prope{convex}  )
   \\ \sin\brp{\pE\rvX}   &\le& \pE\brp{\sin\rvX}  & for $0\le\rvX\le\pi/2$
   \\ \sin\brp{\pE\rvX}   &\ge& \pE\brp{\sin\rvX}  & for $\pi/2\le\rvX\le\pi$
\end{array}}
\\
Colloquially, ``If $\ff$ is \prope{convex}, then $\ff$ on the outside of $\pE$ is less than $\ff$ on the inside."
\end{example}
