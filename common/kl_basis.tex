...
%============================================================================
%=======================================
\chapter{Sufficient Statistics Estimation}
%=======================================
%=======================================
...
%---------------------------------------
Let $\Psi\eqd\set{\fpsi_n}{n=1,2,\ldots,\xN}$ be an \structe{orthonormal basis} for a parameterized function $\rvx(t;\theta)$
with parameter $\theta$.
Let $\fy(t)$ be $\fx(t;\theta)$ plus random noise $\rvv(t)$ such that $\rvy(t)\eqd\rvx(t;\estT)+\rvv(t)$.
Let the set $\setY$ be defined as $\setY\eqd\set{\fdoty_n}{1,2,\ldots,\xN}$ 
where each $\fdoty_n$ is the projection of $\fy(t)$ onto the \fncte{basis vector} $\fpsi_n(t)$ such that 
\\\indentx$\ds
  \fdoty_n \eqd \inprod{\fy(t)}{\fpsi_n(t)}
           \eqd \int_{t\in\R}\fy(t)\fpsi_n(t)\dt
$.\\
...
%---------------------------------------
\begin{proposition}[\thme{Additive noise projection statistics}]
\label{prop:an_stats}
...
\thmbox{
\mcom{\opC=\opCagn}{additive \prope{Gaussian} channel}
\implies
...
\begin{proof}
Because the noise is additive (see \prefpp{prop:an_stats})
\begin{align*}
   \pE\fdotv_{n}           &= 0  \\
...
\brbl{\begin{array}{rclD}
   ...
   \fdoty_n|\theta                     &\sim& \pN{\fdotx_n(\theta)}{\sigma^2}      & (receiver projections are \prope{Gaussian}     ) \\
   \cov{\fdotv_n}{\fdotv_m}            &=   & \sigma^2 \kdelta_{nm}                & (noise projections are \prope{uncorrelated}    ) \\
   \cov{\fdoty_n}{\fdoty_m }           &=   & \sigma^2 \kdelta_{nm}                & (receiver projections are \prope{uncorrelated} ) \\
   \psp\{\fdotv_n=a \land \fdotv_m=b\} &=   & \psp\{\fdotv_n=a\}\psp\{\fdotv_m=b\} & (noise    projections are \prope{independent}  ) \\
   \psp\{\fdoty_n=a \land \fdoty_m=b\} &=   & \psp\{\fdoty_n=a\}\psp\{\fdoty_m=b\} & (receiver projections are \prope{independent}  )
\end{array}}
}
\end{theorem}

\begin{proof}
The distributions follow because they are \prope{linear} operations on
\prope{Gaussian} processes.
...
\end{align*}
Because the processes are \prope{Gaussian},
\prope{uncorrelatedness} implies \prope{independence}.
\end{proof}
