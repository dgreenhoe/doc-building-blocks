...
%\defbox{\begin{array}{M}
%  The space of \hid{Lebesgue square-integrable functions} $\spLLRBu$ (or $\spLLR$) is defined as
...
\cntboxt{%
  ...
  \qquad
  \begin{tabular}{ccc}
     \includegraphics{graphics/tentfx.pdf}%
    &\includegraphics{graphics/TD_tentfx.pdf}%
    &\includegraphics{graphics/DT_tentfx.pdf}%
  \end{tabular}%
  }
\end{counterex}
...
every operator has an \hie{adjoint} \ifxref{operator}{prop:op_adjoint}
...
\begin{align*}
  ...
  \\&= \inprod{\opTrn\fg(u)}{\ff(u)}
    && \text{by definition of adjoint $\opTrna$}             && \text{\xref{def:adjoint}}
  \\&= \inprod{\fg(u-1)}{\ff(u)}
    && \text{by definition of $\opTrn$}                      && \text{\xref{def:opT}}
  \\&= \inprod{\fg(x)}{\ff(x+1)}
  ...
  \\&= \inprod{\opDil\fg(u)}{\ff(u)}
    && \text{by definition of $\opDila$}                     && \text{\xref{def:adjoint}}
  \\&= \inprod{\sqrt{2}\fg(2u)}{\ff(u)}
    && \text{by definition of $\opDil$}                      && \text{\xref{def:opD}}
  \\&= \int_{u\in\R} \sqrt{2}\fg(2u) \ff^\ast(u) \du
    && \text{by definition of $\inprodn$}                    && \text{\xref{def:inprod}}
...
%%=======================================
%=======================================
%\subsection{Bases for function spaces}
%=======================================
...
\footnote{
  \citerpg{higgins1996}{2}{0198596995} %{1.1 General introduction}
  }
\label{ex:TD_flinear}
%--------------------------------------
...
\exbox{\begin{array}{Frc>{\ds}lCD}
  ...
  3. & \alpha_n &\eqd& \cft \int_0^{2\pi} \ff(x) \opDil_n e^{-ix} \dx         & \forall\ff\in\spL\intoo{0}{2\pi}
\end{array}}
\end{example}
\begin{proof}
See \prefp{thm:opFSi}.
...
\exbox{\begin{array}{Frc>{\ds}lCD}
  ...
  3. & \Ff(\omega) &\eqd& \cft \int_\R \ff(x) \opDil_\omega e^{-ix} \dx  & \forall\ff\in\spLLR
\end{array}}
\end{example}
...
\exbox{\begin{array}{Frc>{\ds}lCD}
  ...
  3. & \alpha_n  &\eqd& \int_\R \ff(x) \opDil^k\opTrn^n\fpsi^\ast(x) \dx & \forall\ff\in\spLLR
\end{array}}
\end{example}
