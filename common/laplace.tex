...
\begin{enumerate}
  ...
  \item Region of Convergence of $\opLT\brs{\cos\brp{\omega_o x} \fg(x)}$ and $\opLT\brs{\sin\brp{\omega_o x} \fg(x)}$:
    {\begin{align*}
      \opROC\opLT\brs{\cos \brp{\omega_o x} \fg(x)}
  ...
  \item Region of Convergence of $\opLT\brs{\cosh\brp{\omega_o x} \fg(x)}$ and $\opLT\brs{\sinh\brp{\omega_o x} \fg(x)}$:
    {\begin{align*}
      ...
      \\&= \opROC\brp{\opLT\brs{\frac{e^{ \omega_o x}}{2} \fg(x)}
        ...
        &&  \text{by \prope{linear} properties}
        &&  \text{\xref{thm:opLT_linear}}
...
\corbox{\begin{array}{>{\ds}lc>{\ds}c     D     l                             C }
    \mc{3}{H}{Mapping}                  & \mc{2}{|H}{Region of Convergence} & \mc{1}{|H}{Domain}
    \\\hline
           \opLT\brs{e^{-\alpha x}\step( x)} &=& \frac{1}{s+\alpha}  & for & \Real(s) \geq-\Real(\alpha) & \forall x\in\R;\,\alpha\in\C
    \\[2ex]\opLT\brs{e^{-\alpha x}\step(-x)} &=& \frac{1}{s+\alpha}  & for & \Real(s) \leq \Real(\alpha) & \forall x\in\R;\,\alpha\in\C
\end{array}}
\end{corollary}
\begin{proof}
\begin{align*}
  \opLT\brs{e^{-\alpha x}\step(x)}(s)
    ...
    && \text{\xref{thm:opLT_mod}}
  \\&= \frac{1}{s+\alpha}
    && \forall\Real(s) \in \intcc{0-\Real(\alpha)}{\infty-\Real(\alpha)}
    && \text{by \prope{causality}}
    && \text{\xref{thm:opLT_step}}
  \\&= \frac{1}{s+\alpha}
    && \forall\Real(s) \geq -\Real(\alpha)
    && 
    && 
  \\\\
  \opLT\brs{e^{-\alpha x}\step(-x)}(s)
    ...
    && \text{\xref{thm:opLT_mod}}
  \\&= \frac{-1}{s+\alpha}
    && \forall\Real(s) \in \intcc{-\infty-\Real(\alpha)}{0-(-\Real(\alpha))}
    && \text{by \prope{anti-causality}}
    && \text{\xref{thm:opLT_step}}
  \\&= \frac{-1}{s+\alpha}
    && \forall\Real(s) \leq \Real(\alpha)
    && \text{by \prope{anti-causality}}
    && \text{\xref{thm:opLT_step}}
\end{align*}
\end{proof}
...
\corbox{\begin{array}{F>{\ds}lc>{\ds}c                                                     D     l                             C@{\hspace{0pt}}}
  ...
  %\\[2ex](2).& \opLT\brs{\sinh(\omega_o x)\step( x)} &=& \frac{\omega_o}{s^2+\omega_o^2}  & for & \Real(s) > 0 & x,\omega_o\in\R
  %\\[2ex](3).& \opLT\brs{\cosh(\omega_o x)\step(-x)} &=& \frac{-s}{s^2+\omega_o^2}        & for & \Real(s) < 0 & x,\omega_o\in\R
  %\\[2ex](4).& \opLT\brs{\sinh(\omega_o x)\step(-x)} &=& \frac{-\omega_o}{s^2+\omega_o^2} & for & \Real(s) < 0 & x,\omega_o\in\R
\end{array}}
\end{corollary}
\begin{proof}
\begin{align*}
