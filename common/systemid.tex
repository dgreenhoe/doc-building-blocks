...
\defboxt{
  ...
  \\\indentx$
  \estHgm(\omega) \eqd
    \sqrt{\frac{\ds\Swyy(\omega)\Swxy^\ast(\omega)}
               {\ds\Swxx(\omega)\Swxy(\omega)}}
  $}
\end{definition}
...
Let $\opS$ be a \structe{system} with input $\rvx(n)$ and output $\rvy(n)$.
\thmbox{
  \estHa(\omega) \le \estHhm(\omega) \le \estHgm(\omega) \le \estHam(\omega) \le \estHb(\omega)
  }
\end{theorem}
\begin{proof}
...
%20190211 make proposition?%% _______________________________________________________________________
%20190211 make proposition?%%| |                                   2 Syx(w)                          |
%20190211 make proposition?%%|D| Hv(w) = ------------------------------------------------------------|
%20190211 make proposition?%%|E|                               ____________________________________  |
%20190211 make proposition?%%|F|                           \  /[               ]2      |        |2   |
%20190211 make proposition?%%| |          Sxx(w) - Syy(w) + \/ [Sxx(w) - Syy(w)]   + 4 | Sxy(w) |    |
%20190211 make proposition?%%|_|_____________________________________________________________________|
%20190211 make proposition?%\begin{definition}                                                       %
%20190211 make proposition?%\footnote{                                                               %
%20190211 make proposition?%  \citePpc{white2006}{679}{(6)},                                         %
%20190211 make proposition?%  \citerpgc{shin2008}{294}{0470725648}{(9.69)}                           %
%20190211 make proposition?%  }                                                                      %
%20190211 make proposition?%\label{def:Hv}                                                           %
%20190211 make proposition?%%------------------------------------------------------------------------%
%20190211 make proposition?%\defboxt{
%20190211 make proposition?%    The \fnctd{Total Least Squares transfer function estimate $\estHv(\omega)$}
%20190211 make proposition?%    of $\estHa(\omega)$ and $\estHb(\omega)$ is defined as
%20190211 make proposition?%  \\\indentx$
%20190211 make proposition?%  \estHv(\omega) \eqd
%20190211 make proposition?%  \frac{\ds2\Swyx(\omega)}
%20190211 make proposition?%       {\ds\Swxx(\omega)-\Swyy(\omega) + \sqrt{\brs{\Swxx(\omega)-\Swyy(\omega)}^2 + 4\abs{\Swxy(\omega)}^2}}
%20190211 make proposition?%  $}
%20190211 make proposition?%\end{definition}
...
%------------------------------------------------------------------------%
\defboxt{
    The \fnctd{Total Least Squares transfer function estimate $\estHv(\omega)$}
    of $\estHa(\omega)$ and $\estHb(\omega)$ is defined as
  \\\indentx$
  \estHv(\omega)
...
\label{prop:Hs_lim}              %
%--------------------------------%
Let $\estHs(\omega)$ be defined as in \pref{def:Hs} (previous).
\propbox{\begin{array}{F>{\ds}rc>{\ds}l}
    (1). & \brlr{\estHs(\omega)}_{s=0}     &=& \estHb(\omega)
  \\(2). & \brlr{\estHs(\omega)}_{s=1}     &=& \estHv(\omega)
  \\(3). & \lim_{s\to\infty}\estHs(\omega) &=& \estHa(\omega)
\end{array}}
\end{proposition}
\begin{proof}
\begin{align*}
