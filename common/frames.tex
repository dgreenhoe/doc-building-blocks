...
Without a topology, it is not possible to determine whether an \structe{infinite sum} \xref{def:suminf} 
...
Let $\Ff(\omega)$ and $\Fg(\omega)$ be the \ope{Fourier transform}s \xref{def:ft} of the
...
\begin{proof}
\begin{enumerate}
  \item Proof using \thme{Pythagorean theorem} \ifxref{vsinprod}{thm:pythag}:
...
The \thme{Best Approximation Theorem} (next) shows that
\begin{liste}
...
Let $\Ff(\omega)$ be the \ope{Fourier Transform} \xref{def:ft} of a function $\ff\in\spLLR$.
\thmbox{
...
\exboxt{%
  ...
  \end{array}$\tbox{\includegraphics{graphics/frameR2N.pdf}}
  \\
  Moreover,\quad
...
\exbox{%
  ...
  \end{array}%
  \tbox{\includegraphics{graphics/mframe01.pdf}}
  }
\end{example}
\begin{proof}
\begin{enumerate}
...
\exboxt{%
  ...
  \end{array}$
  \tbox{\includegraphics{graphics/frameR2pq.pdf}}
