...
%---------------------------------------
Let %$\opSys$ be an additive White \prope{Gaussian} noise system and
$\Psi$ be an orthonormal basis for $\rvx(t;\estT)$ such that
...
\item lemma: $\setY$ and $\rvv'(t)$ are \prope{uncorrelated}:\label{ilem:sstat_uncorrelated}
\begin{align*}
...
\begin{align*}
   ...
   \\&= \psP\set{\rvx(t;\estT)}{\setY}
     && \text{by definition of \fncte{conditional probability} \xref{def:conprob}}
\end{align*}
...
\begin{align*}
   ...
   \\&=    \argmax_{\estT} \psP\set{\sum_{n=1}^\xN\fdoty_n\psi_n(t)+\rvv'(t)}{\rvx(t;\estT)}
