...
\defboxt{
  ...
  \\\indentx$\ds\opR\ff  \eqd \int_{u\in\R}\Rxx(t,u)\ff(u) \du$
  }
\end{definition}
...
not white.
This criterion is called the \fncte{Karhunen-Lo\`{e}ve criterion} for $\rvx(t)$.
...
\thmbox{
  \brb{\begin{array}{rl>{\ds}l}
    \inprod{\rvx}{\rvy} &\eqd& \int_{t\in\R} \rvx(t) \rvy^\ast(t) \dt \\
  \end{array}}
  \implies
...
\intertext{3. Proofs under hypothesis (B): substitute $\ds\sum_{n\in\Z}$ operator for $\ds\int_{t\in\R}\dt$ operator in above proofs.}
\end{align*}
\end{proof}
