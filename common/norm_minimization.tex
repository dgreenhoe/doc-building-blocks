...
\defbox{\setlength{\arraycolsep}{3pt}\begin{array}{lclM | lclM}
...
$\begin{array}{|rcl|rcl|}
   \hline
     \vye(\vtheta)   &\eqd& \opU^H\vtheta                          & \estMS          &\eqd& \argmin_\vtheta \fCost(\vtheta)
   \\\ve(\vtheta)    &\eqd& \vye-\vy                               & \opR            &\eqd& \pE\brs{\opU\opU^H}            
   \\\fCost(\vtheta) &\eqd& \pE\norm{\ve}^2 \eqd \pE\brs{\ve^H\ve} & \opW            &\eqd& \pE\brs{\opU\vy}               
   \\\hline
\end{array}$,
then
\thmbox{\begin{array}{rcl}
     \estMS                          &=& (\Real{\setY})^{-1}(\Real{W})
   \\\fCost(\vtheta)                 &=& \vtheta^H\opR\vtheta -2\Real\opW^H\vtheta + \pE\brs{\vy^H\vy}
