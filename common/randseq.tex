...
\thmbox{
  ...
  \brb{\begin{array}{Frc>{\ds}l>{\ds}lD}
       (1).&\pmeany(n)&=& \sum_{k\in\Z} \fh(k)      \pmeanx(m-k)  &= \fh(n)\conv \pmeanx(n) & and
...
\begin{align*}
   ...
   \Rxy(n,m)
     ...
     && \text{by definition of $\Rxy(n,m)$}
     && \text{\xref{def:Rxynm}}
   \\&= \pE\brs{\rvx(n+m) \brp{\fh(n)\conv\rvx(n)}^\ast}
     && \text{by \prope{LTI} hypothesis}
   ...
   \Ryy(n,m)
     ...
     && \text{by definition of $\Rxy(n,m)$}
     && \text{\xref{def:Rxynm}}
   \\&= \pE\brs{\rvy(n+m) \brp{\fh(n)\conv\rvx(n)}^\ast}
     && \text{by \prope{LTI} hypothesis}
...
\defboxt{
  ...
  \brb{\begin{array}{FMlMlc>{\ds}l}
   ...
   \\(3).&The \fnctd{auto-correlation}  & \Rxx(m)  & of $\rvx(n)$               is & \Rxx(m) &\eqd& \brlr{\Rxx(m)}_{\rvy=\rvx}
  %\\The \fnctd{auto-correlation operator} & \opR\ff    & of $\ff(t)$               is & \opR f    &\eqd& \int_{u\in\R}\Rxx(t,u) f(u) \du     %& \text{(\prope{auto-correlation operator})}
\end{array}}$
  }
\end{definition}
