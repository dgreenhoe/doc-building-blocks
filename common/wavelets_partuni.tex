...
\defbox{\begin{array}{M}
  {A function $\ff\in\spRR$ forms a \propd{partition of unity} if}
...
%=======================================
\section{Examples}
\label{sec:pounity_examples}
%=======================================
Unlike the counter examples of \pref{sec:pounity_counter}, this section provides examples of systems that
generate a \prope{partition of unity} \xref{def:pun} \textbf{and} an \prope{MRA} \xref{def:mra}.
Arguably the simplest of these examples is the \fncte{Order 0 B-spline} $\fN_0(x)$,
which naturally results from a very weak set of constraints (next example).
...
\begin{enumerate}
  ...
  \item Proof for values of $h_0$ and $h_1$:
        ...
        a scaling function design is fully constrained using the \prope{admissibility equation} \xref{thm:admiss}
        and the \prope{partition of unity} constraint\ifsxref{partuni}{def:pun}.
        The partition of unity formed by $\fphi(x)$ is illustrated in \prefpp{ex:n0_pounity}.
  \item Here are the equations:
        ...
        \\\fh_0 -\fh_1 &=& 0           & (partition of unity       & \pref{thm:pun_zero} & \prefpo{thm:pun_zero})
        \end{array}$
        \\
