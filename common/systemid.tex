...
\defboxt{
    ...
    of $\estHam(\omega)$ and $\estHb(\omega)$ is defined as
  \\\indentx$
  \estHam(\omega) \eqd
...
\defbox{\begin{array}{Mrc>{\ds}>{\ds}l}
  ...
  & \estHgm(\omega) &\eqd& \sqrt{\frac{\Swyy(\omega)\Swxy^\ast(\omega)}{\Swxx(\omega)\Swxy(\omega)}}
\end{array}}
\end{definition}

%---------------------------------------
\begin{proposition}[\fnctd{Geometric mean estimator}]
\footnote{
...
\defboxt{
  ...
  \\\indentx$
  \estHhm(\omega) \eqd
    \frac{\ds\Swyy(\omega)\Swxy^\ast(\omega)}
...
Let $\opS$ be a \structe{system} with input $\rvx(n)$ and output $\rvy(n)$.
\propbox{\begin{array}{rc>{\ds}lM}
  \estHhm(\omega) &=& \frac{1}{\frac{1}{\ds\estHa(\omega)} + \frac{1}{\ds\estHb(\omega)}}
...
\begin{align*}
  \estHhm(\omega)
    &\eqd \frac{\ds\Swyy(\omega)\Swxy^\ast(\omega)}
               {\ds\Swxx(\omega)\Swyy(\omega) + \abs{\Swxy(\omega)}^2}
    && \text{by definition of $\estHhm$}
    && \text{\xref{def:Hhm}}
  \\&= \frac{1}{\frac{\ds\Swxx(\omega)\Swyy(\omega) + \abs{\Swxy(\omega)}^2}
                     {\ds\Swyy(\omega)\Swxy^\ast(\omega)}}
  \\&= \frac{1}{                                     %
         \frac{\ds\Swxx(\omega)}                     %            1
              {\Swxy^\ast(\omega)}                   %  -------------------
         +                                           %   Sxx(w)     Sxy(w)
         \frac{\ds\Swxy(\omega)}                     %  -------- + --------
  ...
  \\&= \frac{1}{                                     %           1
    ...
    && \text{by definitions of $\estHa$ and $\estHb$}%    ------ + ------
    && \text{\xxref{def:Ha}{def:Ha}}                 %     H1(w)    H2(w)
  \\&= \text{\fnctb{Harmonic mean} of $\estHa(\omega)$ and $\estHb(\omega)$}
\end{align*}
\end{proof}
