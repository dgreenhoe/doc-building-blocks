...
\label{def:opRnm}
%---------------------------------------
Let $\rvx(m)$ be a \fncte{random process} 
discrete \fncte{auto-correlation} $\Rxx(n,m)$ \xref{def:Rxxnm}.
\defboxt{
  The \opd{auto-correlation operator} $\opR$ of $\rvx(t)$ is defined as
  \\\indentx$\ds\mcom{\opR\ff \eqd \sum_{n\in\Z} \Rxx(n,m) \ff(m)}{(discrete case)}$
  }
\end{definition}
...
\label{def:white_seq}
%---------------------------------------
Let $\rvx(m)$ be a \fncte{random process} with \fncte{auto-correlation} $\Rxx(m)$ \xref{def:Rxxnm}.
\defboxt{
...
\intertext{2. Proof that $\opR$ is \prope{self-adjoint}:}
...
Let $\seq{\lambda_n}{n\in\Z}$ be the \fncte{eigenvalue}s and
    ...
    $\opR \fpsi_n = \lambda_n \fpsi_n$ for all $n\in\Z$.
...
\label{thm:kle}
%---------------------------------------
Let $\opR$ be the \ope{auto-correlation operator} \xref{def:opRnm}
...
\section{Quasi-basis}
%=======================================
The \ope{auto-correlation operator} $\opR$ \xref{def:opRnm} in the discrete case can be approximated using 
