...
\defboxp{
  A function $\hxs{\metricn}\in\clF{\setX\times\setX}{\Rnn}$ \xref{def:clFxy} is a \fnctd{quasi-metric} on a \structe{set} $\setX$ if
...
%The property of \prope{boundedness} (next definition) is fundamentally a metric space concept, 
...
%%---------------------------------------
%\begin{example}[\exmd{discrete metric}]
%\label{def:dmetric}
%\label{def:dmetric}
%\footnote{
...
%\exbox{\begin{array}{rcl@{\qquad}M}
%    \metric{x}{y} &\eqd& \brb{\begin{array}{lM}
%                           0 & for $x=y$\\
%                           1 & otherwise
%                         \end{array}}
%    & is a \fncte{metric} on $\setX$, called the \fnctd{discrete metric}. %\xref{def:metric}.
%\end{array}}
%\end{example}
...
%---------------------------------------
\begin{definition}[\fnctd{usual metric}]
\label{def:d_usual}
\footnote{
...
\index{metrics!usual}
%---------------------------------------
Let $\absn\in\clF{R}{\Rnn}$ be an \fncte{absolute value} %\xref{def:abs} 
function on a \structe{ring} $R$. % \xref{def:ring} $R$.
\\\defboxt{
  The function
  \quad
  $\metric{x}{y} \eqd \abs{x-y}$
  \quad
  is a \fncte{metric} on $\R$,
  called the \fnctd{usual metric}.}
...
%%---------------------------------------
%\begin{proposition}[\propd{Fr{\'e}chet product metric}]
%\footnote{
...
\begin{figure}[th]
  \center
  \input{../common/math/graphics/ballinball.tex}
...
\thmbox{
  \brb{\begin{array}{FMD}
    \cnto & $\metrican$ is a metric on $\setY$ & and 
    \cntn & $\fg$ is \prope{injective}
...
\\\begin{tabular}{>{\qquad\scs}rll}
  \cnto & It is \emph{not} necessary for $\fphi$ to be \prope{continuous}   & (see \prefp{ex:mpf_01}).
  \cntn & It is \emph{not} necessary for $\fphi$ to be \prope{nondecreasing}& (see \prefp{ex:mpf_11x1}).
  \cntn & It is \emph{not} necessary for $\fphi$ to be \prope{monotonic}    & (see \prefp{ex:mpf_nonmonotonic}).
\end{tabular}
...
\thmbox{
  ...
  \quad\implies\quad
  \brb{\begin{array}{FlD}
    \cnto & \fphi^{-1}(0) = \setn{0}             &  and
    \cntn & \oppR\fphi \subseteq \Rnn            &  and
    \cntn & \fphi(x+y) \leq \fphi(x) + \fphi(y)  & ($\fphi$ is \prope{subadditive})
  \end{array}}
  }
\end{theorem}
\begin{proof}
...
\thmbox{
  ...
  \brbr{\begin{array}{DlCDD}
    \cnto & x\ge y \implies \fphi(x) \ge \fphi(y)                &\forall x,y\in\Rnn   & (\prope{nondecreasing})     & and 
    \cntn & \fphi(0) = 0                                         &                   &                             & and 
    \cntn & \fphi(x+y) \le \fphi(x) + \fphi(y)                   &\forall x,y\in\Rnn & (\prope{subadditive}).      & 
    %\cntn & \fphi(x) \ge 0                                       &\forall x  \in\R   & (\prope{positive definite}) & and 
  \end{array}}
...
\thmbox{\begin{array}{>{\ds}l}
  \brbr{\begin{array}{FMD}
    \cnto & \text{$\metrican$ is a \structe{metric} on $\setX$} & and
    \cntn & $\ds\sum_{n=1}^\xN \lambda_n = 1$
  \end{array}}
  \implies
  \brbl{\begin{array}{M}
...
%\prefpp{def:d_usual} gives an example of a \structe{metric space} \xref{def:metric} on rings 
...
\begin{proof}
\prefpp{def:dmetric} already showed that $\metrican$ is a metric.
...
%  \put(   0,   0){\circle*{15}}
%
%  \put(   0, 310){\makebox(0,0)[b] { $\setn{x,y,z}$ }}
%  \put( 100, 200){\makebox(0,0)[bl]{ $\setn{  y,z}$ }}
%  \put(   0, 200){\makebox(0,0)[bl]{ $\setn{x,  z}$ }}
%  \put(-100, 200){\makebox(0,0)[br]{ $\setn{x,y  }$ }}
%  \put( 100, 100){\makebox(0,0)[tl]{ $\setn{    z}$ }}
%  \put(   0, 100){\makebox(0,0)[tl]{ $\setn{  y  }$ }}
%  \put(-100, 100){\makebox(0,0)[tr]{ $\setn{x    }$ }}
