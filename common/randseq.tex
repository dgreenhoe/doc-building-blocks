...
\defboxt{
  ...
  \brb{\begin{array}{FMlMlc>{\ds}l}
   ...
   \\(4).&The \fnctd{cross-correlation} & \Rxy(m)  & of $\rvx(n)$ and $\rvy(n)$ is & \Rxy(m) &\eqd& \pE\brs{\rvx(n+m)\rvy^\ast(0)}
...
\corbox{
  \brb{\begin{array}{FMMD}
      (A).&$\rvx$ and $\rvy$ are \prope{WSS} & \xref{def:wss} & and
    \\(B).&$\opS$ is \prope{LTI}             &                &
  \end{array}}
  \implies
...
\corbox{
  ...
  \brb{\begin{array}{Frc>{\ds}lc>{\ds}lD}
       (1).&\Rxy(m) &=& \Rxx(m)\convd \fh^\ast(-m)           &\eqd& \sum_{k\in\Z} \fh^\ast(k) \Rxx(m+k)  & and
     \\(2).&\Ryy(m) &=& \Ryx(m)\convd \fh^\ast(-m)           &\eqd& \sum_{k\in\Z} \fh^\ast(k) \Rxy(m+k)  & and
     \\(3).&\Ryy(m) &=& \Rxx^\ast(m)\convd \fh(-m)\convd\fh(-m) %&\eqd& \mc{2}{l}{\ds\sum_{p\in\Z}\sum_{k\in\Z}\fh(k)h^\ast(p) \Rxx(m-p-k)}
  \end{array}}
  }
\end{corollary}
\begin{proof}
\begin{align*}
  \Rxy(m)
     &\eqd \Rxy(0,m)
     && \text{by \prefp{prop:Rxynmm}}
   \\&= \sum_{k\in\Z} \fh^\ast(k) \Rxx(n-k,m+k)
