...
$\estHs\to\estHb$ as $s\to\infty$. % \xref{thm:Hs}.
%---------------------------------------
...
\label{thm:estHls}
%---------------------------------------
Let $\opS$ be the \structe{system} illustrated in \prefpp{fig:estHls} (A) or (B).
...
\thmbox{
  ...
  \implies
  \brbl{\begin{array}{c}
    \ds\argmin_{\estH}\norm{\FW(\omega)-\FQ(\omega)}^2 = \frac{\Swyp(\omega)}{\Swpp(\omega)}
    \\
    \brp{\begin{array}{M}
      \scs The optimal \prope{LTI} \prope{least-squares} estimate of $\FH$ is
      \\$\estHls=\ffrac{\Swyp}{\Swpp}$
    \end{array}}
  \end{array}}
  }
\end{theorem}
...
\label{cor:H1LSa}
%---------------------------------------
Let $\opS$ be the \structe{system} illustrated in \prefpp{fig:estHls} (A).
\corbox{
  \brb{\begin{array}{FMD}
      &hypotheses of \prefpp{thm:estHls} & 
...
\label{cor:H1LSa}
%---------------------------------------
Let $\opS$ be the \structe{system} illustrated in \prefpp{fig:estHls} (B).
\corbox{
  \brb{\begin{array}{FMD}
      &hypotheses of \prefpp{thm:estHls} & 
    %\\(B).&$\rvu(n) = 0$\quad\text{\scs(zero input noise)}
  \end{array}}
  \implies
  \estHls(\omega) = \frac{\Swyx(\omega)}{\Swxx(\omega)-\Swww(\omega)}
  }
\end{corollary}
\begin{proof}
  \begin{align*}
...
\corbox{
  ...
  \end{array}}
  \implies
  {\mcom{\argmin_{\estH}\norm{\FW(\omega)-\FQ(\omega)}^2 = \estHa(\omega)}
       {$\estHa$ is the optimal \prope{least-squares estimator} of $\FH$}}
  }
\end{corollary}
\begin{proof}
  \begin{align*}
