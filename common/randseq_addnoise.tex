...
The following three theorems demonstrate:
\begin{listi}
  \item Adding in zero-mean uncorrelated noise \textbf{after} a transform (operation) $\opT$
        has \textbf{no effect} on cross-correlation across input $\rvx(n)$ and output $\rvy(n)$ \xref{thm:sysT_addnoise_v}.
  \item Adding zero-mean uncorrelated noise \textbf{before} $\opT$ \textbf{does} \xref{thm:sysT_addnoise_u}.
        \textbf{However}, if $\opT$ is \prope{LTI}, then the cross-correlation is 
        simply $\Rxy=\fh(n)\conv\Rxx$ \xref{thm:sysH_addnoise_lti}.
  \item Zero-mean uncorrelated input measurement noise \textbf{does not} \xref{thm:sysT_mnoise}.
\end{listi}
