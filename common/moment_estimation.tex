...
\label{def:varest}
%---------------------------------------
\defbox{\begin{array}{rc>{\ds}l | rc>{\ds}l}
    \varest_B &\eqd& \frac{1}{\xN}  \sum_{n=1}^{\xN} \brp{\rvx_n - \meanest}^2
   &\varest   &\eqd& \frac{1}{\xN-1}\sum_{n=1}^{\xN} \brp{\rvx_n - \meanest}^2
\end{array}}
\end{definition}

The factor difference between the two is known as ``\vale{Bessel's correction}".
%---------------------------------------
\begin{theorem}
\label{thm:varest}
\footnote{
...
\begin{enumerate}
  ...
  \item Proof for \prope{unbiased} result: 
        \begin{align*}
          ...
          \\&= \frac{\xN}{\xN-1}\brs{\frac{\xN-1}{\xN}\sigma^2}
