...
random processes.
Before defining a random variable formally, note two things that a random variable is \emph{not}:\citetbl{
...
%
%Before defining what a random variable is, here are two things that
%it is not:
%\begin{enume}
...
\label{def:rvX}
%---------------------------------------
Let $\ps$ be a \structe{probability space} \xref{def:ps} on a 
\structe{orthocomplemented lattice} $\psL\eqd\latocX$.
A \fnctd{random variable} $\rvX$ is a function $\rvX:\setX\to\R$.
\end{definition}
...
\hie{cumulative distribution function} of the probability space.
%---------------------------------------
\begin{definition}
\citetbl{
...
a \fnctd{probability density function} (\fnctd{pdf})      $\ppx:\pse\to\intcc{0}{1}$ and
a \fnctd{cumulative distribution function} (\fnctd{cdf}) $\pcx:\pse\to\intcc{0}{1}$
are defined as
\defbox{\begin{array}{rc>{\ds}lD}
  \pcx(x) &\eqd& \psp\brb{\rvX \le x }
...
Conversely, the probability measure $\psp\brb{a\le X<b}$
