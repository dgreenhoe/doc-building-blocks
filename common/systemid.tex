...
\begin{figure}
  ...
  \begin{tabular}{|c|c|}
    ...
    &\includegraphics{graphics/sysH_mnoise.pdf}
    \\
    ...
    \\\hline
    \\\mc{2}{|c|}{The two models are equivalent under the relation $\rvu(n)=-\rvu'(n)$.}
    \\\mc{2}{|c|}{In each model, $\rvx(n)$ and $\rvy(n)$ are ``known", and $\rvu(n)$ and $\rvv(n)$ are not.}
    \\\hline
  \end{tabular}
...
from geophysical activity), $\rvu'$ is measurement noise, $\rvx$ is the measured input contaminated by noise,
and $\fH$ is the device under test (a piece of equipment, a building, or the entire earth).
\end{liste}

Note that the two models are an equivalent system $\opS$ under the relation $\rvu=-\rvu'$.
...
\\\indentx$\ds\Sw{u'u'} = \opFT\pE\brs{\rvu'(m)\rvu'^\ast(0)}
                        = \opFT\pE\brs{\brp{-\rvu(m)}\brp{-\rvu^\ast(0)}}
                        = \opFT\pE\brs{\brp{\rvu(m)}\brp{\rvu^\ast(0)}}
                        = \Swuu$
\\
So the sign difference is not that big of a difference after all.
Rather, the key difference between the two models is simply the interpretation of what the ``input signal" is
and by extension, what the \fncte{signal-to-noise ratio} (\fncte{SNR}) is:
\begin{liste}
  ...
  \item In the measurement model (on the right), the ``input signal" is $\rvp(n)$
      and the frequency-domain input \fncte{signal-to-noise ratio} (\fncte{SNR}) is 
         $\ffrac{\Swpp(\omega)}{\Sw{u'u'}(\omega)} = \ffrac{\Swpp(\omega)}{\Swuu(\omega)}$.
\end{liste}
...
\begin{align*}
  ...
  \\&= \sqrt{\Twxy(\omega)}\sqrt{e^{-i2\theta}}
  \\&= \sqrt{\Twxy(\omega)}\,e^{-i\theta}
\end{align*}
...
\defbox{\begin{array}{Mrc>{\ds}>{\ds}l}
  ...
  & \estHharm(\omega) &\eqd& \frac{1}{\frac{1}{\estH_1(\omega)} + \frac{1}{\estH_2(\omega)}}
\end{array}}
\end{definition}
...
%---------------------------------------
%\begin{minipage}{\tw-70mm}
\begin{theorem}[LTI no input noise case]
\footnote{
...
\label{thm:estH1}
%---------------------------------------
Let $\opS$ be the \structe{system} illustrated in \prefpp{fig:addnoise}. %to the right.
...
\thmbox{
  ...
  \brbl{\begin{array}{F>{\ds}rc>{\ds}lDD}
    ...
    \\(2).&\estH_2(\omega) &=& \FH(\omega)\brs{1 + \frac{\Swvv(\omega)}{\Swqq(\omega)}} & \mc{2}{D}{\begin{tabular}{@{}l}(\prope{biased})\\(\prope{over estimated})\end{tabular}}
  \end{array}}
  }
\end{theorem}
\begin{proof}
\begin{align*}
  ...
  \\&= \frac{\Swpp(\omega)\FH(\omega)}{\Swpp(\omega)+\Swuu(\omega)}
    && \text{by hypotheses (B)--(C)}
...
\label{thm:estH2}
%---------------------------------------
Let $\opS$ be the \structe{system} illustrated in \prefpp{thm:estH1}.
\thmbox{
  \brbr{\begin{array}{FMMD}
      (A).& The system $\fh$ is         &\prope{LTI}          & and
    \\(B).& $\rvx(n)$ and $\rvy(n)$ are &\prope{WSS}          & and
  ...
  \implies
  \brbl{\begin{array}{F>{\ds}rc>{\ds}lDD}
      (1).&\estH_1(\omega) &=& \frac{\FH(\omega)}
                                    {1 + \frac{\ds\Swuu(\omega)}
                                              {\ds\Swxx(\omega)}
                                    }
          & \begin{tabular}{@{}l}(\prope{biased})\\(\prope{under estimated})\end{tabular}
          &and
    \\(2).&\estH_2(\omega) &=& \FH(\omega)
          & (\prope{unbiased}) &
  \end{array}}
  }
\end{theorem}
