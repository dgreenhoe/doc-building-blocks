...
\begin{liste}
  ...
  \item This criterion is called the \fncte{Karhunen-Lo\`{e}ve criterion} for $\rvx(t)$ \xref{thm:kle_eigen}.
\end{liste}
...
\begin{proof}
\begin{enumerate}
  \item Define $\ds\dotx_n\eqd \inprod{\rvx(t)}{\fpsi_n(t)}$ \label{idef:kle_xdot_eigen}
 %\item Define $\ds\opRx\rvx(t) \eqd \int_{u\in\R} \Rxx(t,u)\rvx(u)\du$ \label{idef:kle_R}

  \item lemma: \label{ilem:kle_mercer_eigen}
  ...
  \item lemma: \label{ilem:kle_1_eigen}
    \begin{align*}
  ...
  \item lemma: \label{ilem:kle_2_eigen}
    \begin{align*}
...
\item Proof that $\setn{\fpsi_n(t)}$ is a \structe{basis} for $\rvx(t)$:
  \begin{align*}
    ...
    \\&= \mcom{\sum_{n\in\Z}\lambda_n \abs{\fpsi_n(t)}^2}{by \pref{ilem:kle_mercer_eigen}}
       - \mcom{\sum_{n\in\Z}\lambda_n \abs{\fpsi_n(t)}^2}{by \pref{ilem:kle_1_eigen}}
       - \mcom{\brs{\sum_{n\in\Z}\lambda_n \abs{\fpsi_n(t)}^2}^\ast}{by \pref{ilem:kle_1_eigen}}
       + \mcom{\sum_{n\in\Z}\lambda_n \abs{\fpsi_n(t)}^2}{by \pref{ilem:kle_2_eigen}}
    \\&= 0
  \end{align*}
\end{enumerate}
\end{proof}
