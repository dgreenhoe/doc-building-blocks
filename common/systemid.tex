...
\chapter{Transfer Function Estimation}
\label{chp:systemid}
%============================================================================
\qboxnpq{
...
\fncte{sine sweep}. That is, they drove the input with a sine wave with slowly increasing (or decreasing)
...
\begin{figure}
  ...
  \begin{tabular}{|c|c|}
    \hline
     \tbox{\includegraphics{../common/math/graphics/pdfs/opT_cnoise.pdf}}
    &\tbox{\includegraphics{../common/math/graphics/pdfs/opT_mnoise.pdf}}
    \\
    ...
    \\\hline
  \end{tabular}
  \caption{Additive noise systems with \prope{linear}/\propb{non-linear} operator $\opT$\label{fig:addnoise_sysid}}
\end{figure}

Consider the additive noise systems illustrated in \prefpp{fig:addnoise_sysid}.
\begin{liste}
...
\defboxt{
  The \fnctd{transmissibility} function $\Twxy(\omega)$ is defined as
  \qquad$\ds
...
\end{minipage}
\hfill\tbox{\includegraphics{../common/math/graphics/pdfs/sysH_xy0v--.pdf}}
\\
%---------------------------------------
...
\footnote{
  ...
  \citerpgc{shin2008}{293}{0470725648}{
    ...
    which differs from \pref{def:H1}, but see \citeP{greenhoe2019} %\prefp{app:xcordef}
    },
...
\end{minipage}
\hfill\tbox{\includegraphics{../common/math/graphics/pdfs/sysH_xyu0--.pdf}}
\\
...
\footnote{
  \citerpgc{shin2008}{293}{0470725648}{$H_2(f)=\ffrac{\Swyy(f)}{\Swyx(f)}$ (9.65);
    which differs from \pref{def:H2}, but see \citeP{greenhoe2019} %\prefp{app:xcordef}
    },
...
\rembox{\begin{array}{Mc>{\ds}l}
  The \fnctd{complex transmissibility} function $\Twxy'(\omega)$ &\eqd& \estHgm(\omega)
\end{array}}
\end{remark}
...
A bit of review reveals $\estHa$ at the low end of the estimation problem,
...
\begin{listi}
  \item has $\estHa$ and $\estHb$ as limiting cases \xref{thm:Hs_lim},
  \item is optimal in the least squares sense \xref{thm:estHk}, and
...
\begin{liste}
  \item In the case of constant $\kappa$, $\estHkp$ simplifies to
        the \fncte{Scaling transfer function estimate} $\estHs$ \xref{def:Hs}.
  \item In the case of $\kappa=1$, $\estHkp$ and $\estHs$ simplify to
        the \fncte{Total least squares transfer function estimate} $\estHv$ \xref{def:Hv}.
\end{liste}
...
\defboxt{
  ...
  with \vald{scaling parameter} $s\in\intco{0}{\infty}$
  \\is defined as
  \indentx$\ds
  \estHs(\omega;s) \eqd \estHkp(\omega;\kappa)\quad\text{with $\ds\kappa(\omega)\eqd s^2$}
      %20190322%\frac{\Swyy(\omega)- s^2 \Swxx(\omega) +
      %20190322%      \sqrt{\brs{\Swyy(\omega) -  s^2 \Swxx(\omega)}^2 + 4 s^2 \abs{\Swxy(\omega)}^2}
      %20190322%     }{2\Swxy(\omega)}
  $}
\end{definition}
...
\defboxt{
  ...
  \\\indentx$\ds
  \estHv(\omega) \eqd \estHkp(\omega;\kappa)\qquad\text{with $\ds\kappa(\omega)=1$}
...
%20190322%The estimate $\estHv$ and it's refusal to assume zero input or output noise
%20190322%was definitely a step in the right direction.
%20190322%But it still has the limiting assumption that the input and output noise power are equal.
%20190322%And what if it's not?
%20190322%Enter Wicks and Vold who in 1986 introduced such an estimator (next definition)
%20190322%that makes no such assumption.
%20190322%It features a \vale{scaling parameter} $s$ in the range $\intco{0}{\infty}$
%20190322%that can be adjusted depending on the ratio of output and input noise.
%20190322%And as it turns out, $\estHs = \estHb$ when $s=0$, $\estHs=\estHv$ when $s=1$, and
%20190322%$\estHs\to\estHa$ as $s\to\infty$  \xref{thm:Hs_lim}.
%20190322%Wicks and Vold's $\estHs$ estimates $\FH$ using a single parameter $s$
%20190322%that is \prope{constant} with respect to frequency.
%20190322%This is suitable for additive white noise $\rvw(n)$, which has constant spectral density
%20190322%$\Swww(\omega)=\pvar_w$.
%20190322%However, not all noise is white noise.
%20190322%And later in 2006, White, Tan and Hammond proposed a generalization (next definition) of $\estHs$ with a scaling
%20190322%function $\kappa(\omega)$---that is, one that is not in general constant with respect to frequency
%20190322%and is thus suitable in the presence of \prope{colored} noise.
...
\begin{figure}[h]
  \begin{tabular}{|c|c|}
    \hline
     &
    \\\tbox{\includegraphics{../common/math/graphics/pdfs/transferHs_p.pdf}}
     &\tbox{\includegraphics{../common/math/graphics/pdfs/transferHs_ddp.pdf}}
    \\
...
%|O|                              \  /[                  ]2         |        |2   |
%|P|           k Sxx(w) - Syy(w) + \/ [ k Sxx(w) - Syy(w)]   + 4 k  | Sxy(w) |    |
%|_|______________________________________________________________________________|
\begin{proposition}                                                               %
\footnote{                                                                        %
  \citerpgc{shin2008}{293}{0470725648}{(9.67)},                                   %
...
\label{prop:Hs}                                                                   %
%---------------------------------------------------------------------------------%
Let $\estHkp(\omega; \kappa)$ be defined as in \prefpp{def:Hkp}.
\propbox{\begin{array}{rc>{\ds}l}
  \estHkp(\omega; s)
    &=&
  \frac{\ds 2\kappa(\omega)\Swyx(\omega)}
       {\ds  \kappa(\omega)\Swxx(\omega)-\Swyy(\omega) +
        \sqrt{\brs{\kappa(\omega)\Swxx(\omega)-\Swyy(\omega)}^2 + 4\kappa(\omega)\abs{\Swxy(\omega)}^2}
       }
  \\&=& \frac{2 \Swxy^\ast}
             {\Swxx - \frac{1}{\kappa(\omega)}\Swyy  + \sqrt{\brs{\Swxx - \frac{1}{\kappa(\omega)}\Swyy}^2 + \frac{4}{\kappa(\omega)} \abs{\Swxy }^2}}
\end{array}}
\end{proposition}
\begin{proof}
We can transform $\estHkp$ from that found in \prefpp{def:Hs} to the forms in this proposition by the
...
Let $p\eqd \kappa$.
%Let $\ds\fG(s)\eqd
%  \frac{\ds 2\kappa\Swyx }
%       {\ds  \kappa\Swxx -\Swyy  +
%        \sqrt{\brs{\kappa\Swxx -\Swyy }^2 + 4\kappa\abs{\Swxy }^2}
%       }$.
\begin{align*}
...
%=======================================
\qboxns{
  Per Bak, in \emph{``how nature works\ldots"}
  \index{Bak, Per}
  \index{quotes!Bak, Per}
  \footnotemark
  }
  ...
  \footnotetext{\citerpgc{bak2013}{29}{1475754264}{{\textsection} Systems in Balance Are Not Complex}}

\begin{figure}[h]
  \centering
  \begin{tabular}{|c|c|}
    \hline
     \tbox{\includegraphics{../common/math/graphics/pdfs/opT_estG_estH_cnoise.pdf}}
    &\tbox{\includegraphics{../common/math/graphics/pdfs/opT_estG_estH_mnoise.pdf}}
...
\end{minipage}
\hfill\tbox{\includegraphics{../common/math/graphics/pdfs/sysHw_xy.pdf}}

The defintion of ``best" or ``optimal" is given by a \fncte{cost function} $\fCost(\estH)$.
There are several possible cost functions.
\pref{def:fCost} provides some.
%The answer is the estimator $\estHa$ \xref{def:H1}, as demonstrated in the next theorem.
\pref{thm:estHls} then demonstrate optimal solutions with respect to these definitions.

%---------------------------------------
\begin{definition}
\label{def:fCost}
%---------------------------------------
Let $\opS$ be a system defined as in \prefpp{fig:estHls} (A) or (B).
Define the following \fncte{cost function}s for spectral \prope{least-squares} estimates:
\defbox{\begin{array}{>{\ds}r*{3}{c>{\ds}l}}
      \fCostrq(\estG) &\eqd& \opFT\norm{\rvr(n)-\rvq(n)}^2 &\eqd& \opFT\inprod{\rvr(n)-\rvq(n)}{\rvr(0)-\rvq(0)} &\eqd& \opFT\pE\brp{\brs{\rvr(n)-\rvq(n)}\brs{\rvr(0)-\rvq(0)}^\ast}
    \\\fCostsy(\estH) &\eqd& \opFT\norm{\rvs(n)-\rvy(n)}^2 &\eqd& \opFT\inprod{\rvs(n)-\rvy(n)}{\rvs(0)-\rvy(0)} &\eqd& \opFT\pE\brp{\brs{\rvs(n)-\rvy(n)}\brs{\rvs(0)-\rvy(0)}^\ast}
  \end{array}}
\end{definition}
...
\label{lem:H1LS_cost}
%---------------------------------------
Let $\fCostrq(\estG)$ and $\fCostsy(\estH)$ be defined as in \prefpp{def:fCost}.
...
\thmbox{
  \brbr{\begin{array}{FMMD}
    ...
    \\(C).& $\pE\rvu=\pE\rvv=0$            & (\prope{zero-mean})  & and
    \\(D).& $\estGls$ and $\estHls$ are    & \prope{LTI}
  \end{array}}
  \implies
  \brbl{\begin{array}{F>{\ds}r>{\ds}c>{\ds}l}
      (1).&\argmin_{\estG}\fCostrq(\estG) &=& \frac{\Swyp(\omega)}{\Swpp(\omega)}
    \\(2).&\argmin_{\estH}\fCostsy(\estH) &=& \frac{\Swyx(\omega)}{\Swxx(\omega)}
    \\
  \end{array}}
  }
\end{theorem}
...
Using \pref{thm:estHls} (previous) we can see that the optimal \propb{least-squares} operators $\estGls$ and
$\estHls$ for the \propb{non-linear} operator $\opT$ in \prefpp{fig:estHls} (A) and (B) are (next two corollaries)
\\\indentx$\ds\boxed{
...
\thmbox{
  \brb{\begin{array}{O}
    hypotheses of \pref{thm:estHls}
    \\\prefpo{thm:estHls}
  \end{array}}
  \implies
  \brb{\begin{array}{F>{\ds}r>{\ds}c>{\ds}c}
      (1).&\argmin_{\estGls}\fCostrq(\estGls) &=& \frac{\Swyx(\omega)}{\Swxx(\omega)+\Swuu(\omega)}
    \\(2).&\argmin_{\estHls}\fCostsy(\estHls) &=& \frac{\Swyx(\omega)}{\Swxx(\omega)}
    \\
  \end{array}}
  }
\end{corollary}
...
It follows immediately from \prefpp{cor:estHls_A} and \prefpp{cor:estHls_B} that, in the special case
...
\corbox{
  \brb{\begin{array}{FMD}
      (1). & hypotheses of \pref{thm:estHls} & and
    \\(2). & $\rvu(n)=0$
  \end{array}}
  \implies
  \brb{\estGls(\omega) = \estHls(\omega) = \estHa(\omega)}
  }
\end{corollary}
...
\footnote{
  ...
  \citePpc{mitchell1982}{277}{$\tilde{H}_1(f)=\ffrac{H_0(f)}{\brp{1+\ffrac{G_{nn}}{G_{uu}}}}$}
  \citePpc{mitchell1982}{278}{$\tilde{H}_2(f)=H_0(f)\brp{1+\ffrac{G_{mm}}{G_{vv}}}$}
  }
...
\begin{enumerate}
  ...
  \item Development of Wicks and Vold (\citeP{wicks1986}):
    \begin{align*}
...
\begin{enume}
  ...
  \item The \fncte{coherence} of $\rvx$ and $\rvy$ is a measure of how reliable
        ...
        estimates.\footnote{\citerppgc{liang2015}{363}{365}{1498702341}{7.4.2\scshape Coherece Function}}
\end{enume}
