...
\thmbox{\begin{array}{rlp{7cm}}
  ...
  3. & \norm{\fpsi_n(t)}^2>0 \implies \lambda_n\ge0
     ...
     \\
  4. & \norm{\fpsi_n(t)}^2>0, \inprod{\opR f}{f} > 0 \implies \lambda_n>0
...
and $\seq{\fpsi_n}{n\in\Z}$ are the eigenfunctions of $\opR$ such that
    $\opR \fpsi_n = \lambda_n \fpsi_n$.
\thmbox{
  \mcom{\pE\brp{\abs{ x(t)-\sum_{n\in\Z}\inprod{\fx(t)}{\fpsi_n(t)} \fpsi_n(t) }^2} = 0}
...
\begin{proof}
\begin{enumerate}
  \item Define $\ds\dotx_n\eqd \inprod{\fx(t)}{\fpsi_n(t)}$ \label{idef:kle_xdot}
  \item Define $\ds\opR\fx(t) \eqd \int_{u\in\R} \Rxx(t,u)\fx(u)\du$ \label{idef:kle_R}

  \item lemma: \label{ilem:kle_1}
    \begin{align*}
       \pE\brs{\fx(t) \brp{\sum_{n\in\Z}\dotx_n \fpsi_n(t)}^\ast}
         &\eqd \pE\brs{\fx(t) \brp{\sum_{n\in\Z}\int_{u\in\R}x(u)\fpsi_n^\ast(u)\du \fpsi_n(t)}^\ast}
         && \text{by definition of $\dotx$}
         && \text{\xref{idef:kle_xdot}}
       \\&= \sum_{n\in\Z}\brp{\int_{u\in\R}\pE\brs{\fx(t)x^\ast(u)}\fpsi_n(u)\du} \fpsi_n^\ast(t)
  ...
  \item lemma: \label{ilem:kle_2}
    \begin{align*}
       \pE\brs{\sum_{n\in\Z}\dotx_n \fpsi_n(t)\brp{\sum_{m\in\Z}\dotx_m \fpsi_m(t)}^\ast}
         &\eqd \pE\brs{\sum_{n\in\Z}\int_{u\in\R}x(u)\fpsi_n^\ast(u)\du   \fpsi_n(t)\brp{\sum_{m\in\Z}\int_v x(v)\fpsi_m^\ast(v)\dv \fpsi_m(t)}^\ast}
         && \text{by definition of $\dotx$}
         && \text{\xref{idef:kle_xdot}}
       \\&= \sum_{n\in\Z}\sum_{m\in\Z}\int_u\brp{\int_v \pE\brs{\fx(u)x^\ast(v)}\fpsi_m(v)\dv} \fpsi_n^\ast(u)\du   \fpsi_n(t)   \fpsi_m^\ast(t)
         && \text{by \prope{linearity}}
         && \text{\xref{thm:pE_linop}}
...
\item Proof that $\setn{\fpsi_n(t)}$ is a \structe{basis} for $\rvx(t)$:
  \begin{align*}
    &\pE\brp{\abs{ x(t)-\sum_{n\in\Z}\dotx_n \fpsi_n(t) }^2}
    \\&= \pE\brp{\brs{ x(t)-\sum_{n\in\Z}\dotx_n \fpsi_n(t) }\brs{ x(t)-\sum_{m\in\Z}\dotx_m \fpsi_m(t) }^\ast}
    \\&= \pE\brp{
         \fx(t)x^\ast(t)
       - \fx(t)\brs{\sum_{n\in\Z}\dotx_n \fpsi_n(t)}^\ast
       - \fx^\ast(t)\sum_{n\in\Z}\dotx_n \fpsi_n(t)
       + \brs{\sum_{n\in\Z}\dotx_n \fpsi_n(t)} \brs{\sum_{m\in\Z}\dotx_m \fpsi_m(t) }^\ast
       }
    \\&= \pE\brp{\fx(t)x^\ast(t)}
       - \pE\brs{\fx(t)\brs{\sum_{n\in\Z}\dotx_n \fpsi_n(t)}^\ast}
       - \pE\brs{x^\ast(t)\sum_{n\in\Z}\dotx_n \fpsi_n(t)}
