...
\begin{align*}
  ...
  \Ruy(m) 
    &= \Ruq(m)
    && \text{by previous result}
    && \text{(swap $\rvx$ and $\rvu$)}
  \\
  \Rpy(m)
    &\eqd \pE\brs{\rvp(m)\rvy^\ast(0)}
    && \text{by definition of $\Rpy$}
    && \text{\pref{def:Rxym}}
  \\&\eqd \pE\brs{\rvp(m)(\rvq(0)+\rvv(0))^\ast}
    && \text{by definition of $\opS$}
  \\&= \pE\brs{\rvp(m)\rvq^\ast(0)+\rvp(m)\rvv^\ast(0)}
  \\&= \pE\brs{\rvp(m)\rvq^\ast(0)} + \pE\brs{\rvp(m)\rvv^\ast(0)}
  \\&= \pE\brs{\rvp(m)\rvq^\ast(0)} + \brs{\pE\rvp(m)}\brs{\pE\rvv^\ast(0)}
    && \text{by \prope{uncorrelated} hypothesis}
    && \text{(B)}
  \\&= \pE\brs{\rvp(m)\rvq^\ast(0)} + \brs{\pE\rvp(m)}\cancelto{0}{\brs{\pE\rvv^\ast(0)}}
    && \text{by \prope{zero-mean} hypothesis}
    && \text{(E)}
  \\&= \Rpq(m)
    && \text{by definition of $\Rpy$}
