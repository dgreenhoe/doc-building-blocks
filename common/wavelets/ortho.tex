...
%      \\&=  \Dh(\omega)
%            \brs{s(-1)^\xN e^{-i\xN\omega} \Dh^\ast(\omega+\pi) }^\ast
%          \\&\indentx+ \Dh(\omega+\pi)
%            \brs{s(-1)^\xN e^{-i\xN(\omega+\pi)} \Dh^\ast(\omega+2\pi) }^\ast
...
\exbox{
  \brb{\begin{array}{FMMD}
    ...
    \\(3). & $\setn{\opTrn^n\fphi}$ is \prope{orthonormal}                & \xref{lem:ows_quadcon} &
   %\\(4). & $\fg_n  = \pm (-1)^n \fh_{\xN-n}^\ast$ $\scy\forall n\in\Z$  & \xref{thm:ortho_qmr}   &
  \end{array}}
  \implies
  \brb{\begin{array}{c|r|r}%
      n            & \fh_{n}            & \fg_n
    \\\hline
...
\\\indentx\includegraphics{../common/math/graphics/pdfs/D1_pz.pdf}
     \quad\includegraphics{../common/math/graphics/pdfs/d1_phi_h.pdf}
     \quad\includegraphics{../common/math/graphics/pdfs/d1_psi_g.pdf}
\end{example}
\begin{proof}
\begin{enumerate}
...
\exbox{
  \brb{\begin{array}{FMMD}
      (1). & $\support\fphi(x)=\intcc{0}{3}$                              & \xref{thm:support}     & and
    \\(2). & \prope{admissibility condition}                              & \xref{thm:admiss}      & and
    \\(3). & \prope{partition of unity}                                   & \xref{thm:pun_zero}    & and
    \\(4). & \prope{vanishing 1st moment}                                 & \xref{thm:gh_cqf}      & and
    \\(5). & $\setn{\opTrn^n\fphi}$ is \prope{orthonormal}                & \xref{lem:ows_quadcon} & and
    \\(6). & $[\fh_n]$ is \prope{minimum phase}                           & \xref{def:minphase}    &
   %\\(7). & $\fg_n  = \pm (-1)^n \fh_{\xN-n}^\ast$ $\scy\forall n\in\Z$  & \xref{thm:ortho_qmr}   &
  \end{array}}
  \implies
  \brb{\begin{array}{c|r|r}%
        n            & \fh_{n}                            &\fg_n
      \\\hline
        0            & \frac{\sqrt{2}}{2}\brs{1+\sqrt{3}} &  \fh_3
      \\1            & \frac{\sqrt{2}}{2}\brs{3+\sqrt{3}} & -\fh_2
      \\2            & \frac{\sqrt{2}}{2}\brs{3-\sqrt{3}} &  \fh_1
      \\3            & \frac{\sqrt{2}}{2}\brs{1-\sqrt{3}} & -\fh_0
      \\\text{other} & 0                                  &  0
  \end{array}}
  }%
\\\indentx\includegraphics{../common/math/graphics/pdfs/D2_pz.pdf}
     \quad\includegraphics{../common/math/graphics/pdfs/d2_phi_h.pdf}
     \quad\includegraphics{../common/math/graphics/pdfs/d2_psi_g.pdf}
\end{example}
\begin{proof}
\begin{enumerate}
  ...
  \item This same wavelet system is identical to the \hie{Daubechies-$p2$} wavelet system
