...
\defbox{
  \begin{array}{MlMlc>{\ds}l}
     ...
     \\The \fnctd{auto-correlation operator} & \opR\ff    & of $\ff(t)$               is & \opR f    &\eqd& \int_{u\in\R}\Rxx(t,u)\ff(u) \du     %& \text{(\prope{auto-correlation operator})}
  \end{array}
  }
\end{definition}
...
%---------------------------------------
Let $\opR$ be the \ope{auto-correlation operator} of a \fncte{random process} $\rvx(t)$.
\\
\thmboxt{
  $\brb{\begin{array}{FMD}
